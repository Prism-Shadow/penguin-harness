...
- A row-initiated check reports exactly one outcome as a toast — found (download starts by itself), already downloaded and waiting, already up to date, unsupported, or failed — matching the manual-check contract of System settings → Updates. A failure toast carries the shell's own reason where the updater gave one, so a download rejected by its checksum or its signature does not read as a network hiccup. Unsupported forms (dev run, non-AppImage Linux install) render the row disabled, with the reason as the tooltip of a wrapper rather than of the disabled control itself — a disabled button is dispatched no pointer events and would never open one.
- The native menu flow (Check for Updates…, its dialogs, the restart prompt on download) was left unchanged.
- The Web App guide gained a paragraph on the row, in both languages.
