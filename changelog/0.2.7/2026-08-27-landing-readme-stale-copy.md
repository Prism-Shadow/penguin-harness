...
- **The Skill lists are now pinned to the shipped library.** `packages/landing/test/skills-sync.test.ts` asserts that each dictionary's Skills section names exactly the directories under `packages/skills/skills/`, and that both dictionaries put each Skill on the same card. `packages/skills/test/skills.test.ts` extended its existing README guard to cover the two root READMEs alongside the package's own, matching every table row against the group its heading names in `SKILL_GROUPS`. Both compare membership rather than order, and name the offending Skill or group in the failure.
