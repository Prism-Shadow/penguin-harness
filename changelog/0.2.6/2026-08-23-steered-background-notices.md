...
- Legacy Traces (recorded before the stamp existed) get a positional fallback: a stamp-less report sitting between a turn's paired tool outputs and the continuation request is provably in-task — a task can never end in that gap — so the chat reducer, the message-window scanner and trace analysis all treat it as injected, keeping the four turn implementations in agreement on old data. A stamp-less report after a no-tool turn keeps its independent round (there an idle launch is genuinely possible, and only the stamp — written by new cores — can tell the two apart). The extra field line is ignored by older parsers.
