# Plugin versions are spelled `2026.09.10.1`
...
- `PLUGIN_VERSION_PATTERN` is `/^\d{4}\.\d{2}\.\d{2}\.\d+$/`, and a library `plugin.json` must
  match it: the manifest check now rejects anything else with `version must be YYYY.MM.DD.N`.
- All twelve built-in manifests were rewritten in place, each keeping its date and sequence
  number (`2026-07-20.1` → `2026.07.20.1`). `continual-learning` additionally moves to
  `2026.09.10.1`, a real content bump: its stop-hook prompt now asks the agent to bump a
...
- The zip export filename suffix stays `-v<version>`: `penguin-sdk-v2026.09.10.1.zip`.
...
`2026-09-02.1` and `2026.09.02.1` compare equal. Frontmatter and manifest parsing of installed
...
The plugin card's metadata line still prints the version behind a display `v` and reads the
relative date off the new form. The hook-import prompt tail asks the agent for a `hooks.json`
`version` in the `YYYY.MM.DD.N` format, identically in both dictionaries, and the plugin
