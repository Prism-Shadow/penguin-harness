...
## 插件版本改写作 `2026.09.10.1`，旧写法读作同一版本

库内清单里的日期版本改写作 `YYYY.MM.DD.N`——`2026.09.10.1`——原为 `YYYY-MM-DD.N`（这是
`plugin.json` 里插件自身的内容版本，与该插件包跟随发行版本的 npm 版本是两回事）。已经装进 Agent
`agent_state/` 的副本带的仍是旧写法：Skill 的 `SKILL.md` frontmatter，以及钩子包生成的
`hooks.json`。loader 两种都读：`parsePluginVersion` 把 `2026-09-02.1` 与 `2026.09.02.1` 解析成
...
词现在要求 Agent 按新格式提升 SKILL.md 的版本，因此该插件版本升到 `2026.09.10.1`、其已装副本被报
