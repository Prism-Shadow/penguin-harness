...
找到全部答案。本版本后来的一处改动又添了第七样：插件的版本号格式。
...
## 插件版本改写作 `v2026.09.10.1`，旧写法读作同一版本

库内清单里的日期版本改写作 `vYYYY.MM.DD.N`——`v2026.09.10.1`——原为 `YYYY-MM-DD.N`（这是
`plugin.json` 里插件自身的内容版本，与该插件包跟随发行版本的 npm 版本是两回事）。已经装进 Agent
`agent_state/` 的副本带的仍是旧写法：Skill 的 `SKILL.md` frontmatter，以及钩子包生成的
`hooks.json`。loader 两种都读：`parsePluginVersion` 把 `2026-09-02.1` 与 `v2026.09.02.1` 解析成
同一个日期与序号，因此本版本之前盖章的已装副本与库内同一版本比较**相等**，改写格式本身不会在任何
地方产生可更新提示。

容忍只在读的一侧，磁盘上不改写任何东西：已装副本保持它被盖章时的写法，直到从库重装该插件——而重装
本身就是写入新写法的那一步。只有库内 `plugin.json` 必须用新写法，清单校验会拒绝其余形式。**用户
无需做任何事。**（`continual-learning` 在本版本确有变动，但出于它自己的原因：它的 stop hook 提示
词现在要求 Agent 按新格式提升 SKILL.md 的版本，因此该插件版本升到 `v2026.09.10.1`、其已装副本被报
为可更新——这是普通的内容更新，不是改名带来的。）

这一项**确实**是带有效期的垫片。等到不再有早于 0.2.10 的已装副本即可移除——最早是 0.3 之后的某个
版本——届时要删的只有一处：`packages/core/src/plugins/index.ts` 中 `parsePluginVersion` 的旧写法
分支（`LEGACY_PLUGIN_VERSION_PATTERN`）。删除之后，旧写法的版本读作空字符串，插件库把这样的安装各
报一次可更新，与今天对待旧的自然数版本完全一样。
...
不受影响。以上只有一项是带有效期的垫片——loader 读取插件版本的旧写法，等到不再有早于 0.2.10 的已装
副本即可移除；其余各项后续版本都没有要删的东西。
