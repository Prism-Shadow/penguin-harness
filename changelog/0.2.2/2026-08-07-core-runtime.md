...
Running against a small-window OpenAI-compatible endpoint (e.g. a local vLLM with `--max-model-len 32768`) used to fail three ways: requests 400'd because the configured `max_tokens` went on the wire regardless of how much of the window the input already occupied, and the compaction threshold (default 128000) was unreachable inside the window. Now ([#218](https://github.com/Prism-Shadow/penguin-harness/issues/218)):
