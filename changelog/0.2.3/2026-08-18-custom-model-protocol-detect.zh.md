...
- **自定义分组不再从 model id 推断任何东西。** 在自定义分组里填 `claude-sonnet-5`，对话框的 API Key 提示不再因此指向 Anthropic 客户端与 `ANTHROPIC_*`：Custom 与自建分组一律回退到兼容客户端。厂商分组仍按 id 路由——它们本来就是这么工作的。
- **保存时检测失败不再阻止保存。** 改为回退到 `openai-chat` 继续保存，并以 toast 说明发生了什么——检测是提升准确度的手段，不是关卡；一味拒绝保存会把那些端点本就无法探测的用户卡死。两种情况下写入的都是可用客户端，不会产生起不来的条目。
- **视觉能力检测**，位于视觉开关旁边：发送一张 1x1 的 PNG 和一句一个词的提示，模型能作答就打开开关。若模型明确回答不接受图片，则关闭开关(这是确定的答案，不是错误)；因鉴权或网络失败的探测则不改动该设置，只显示同一句「请检查 API Key 与 base URL」。始终可点击、单次并发、仅用 toast 反馈——与协议控件同一套做法。新建自定义模型默认关闭视觉。**与协议探测不同，这是一次真实计费的补全**(图片请求无法做成零成本)，因此只在主动点击时运行，保存时不会触发。服务端为 `POST /api/projects/:p/models/detect-vision`(仅 owner)，凭据链与连通性测试一致。
  `ModelEntry.vision` 保持普通布尔值：在 core、CLI 与 Web 中，该字段缺省本就表示「支持」，改成三态会让所有既有的未标注条目被悄悄降级。
