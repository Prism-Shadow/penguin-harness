...
- Web：应用中心页（`/apps`，导航与收起图标栏中位于智能体与插件库之间）：搜索框、状态分段、带类型图标 / 元信息行 / 状态胶囊的行、打开 / 重启 / 停止、含跳转到会话 / 编辑 / 取消注册的菜单、每 20 秒重新探测，以及组件包的两个创建按钮——「用 AI 创建」把描述交给 Project 默认 Agent（附构建应用的固定尾巴），提示词预填进新对话的输入框，发送由用户决定；「手动创建」用表单登记到 Project 最近的某个会话。对话页把 `[app_center]` 块折叠成一行「应用中心：重启 …」提示，输入历史跳过这类消息。
