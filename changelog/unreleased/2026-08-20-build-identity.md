...
- Every other build has its git position — description, commit, branch, dirty flag — inlined into the artifact by the bundler that produced it, from one shared `scripts/build-git-stamp.mjs` wired into core, cli and desktop's tsup configs and into `scripts/deploy.mjs`'s esbuild. A built artifact carries no path back to its origin: a bundle pushed into `<root>/hmr/store/` sits outside any checkout, and the working directory would answer about the user's own repository. The stamp holds nothing that varies between two builds of identical source, so the HMR store's content addressing still collapses a re-push of unchanged code onto one entry.
- The inlined stamp wins over asking git at run time, because it describes the code that is executing rather than whatever the surrounding tree has moved on to: a stale `dist/` in a checkout ten commits further along reports the revision it was built at.
- An installed penguin never runs git; it reads those constants. Asking git is the fallback for an un-bundled run — `tsx`, `vitest`, `pnpm dev` — and asks only about the checkout the running module sits in, located by walking up from its own directory for a directory holding both `.git` and `pnpm-workspace.yaml`. Starting from the module rather than the working directory is what makes `penguin version` report the harness's revision when it is run inside another repository; requiring the workspace marker is what keeps an install that merely sits under an unrelated repository — a home directory that is itself a dotfiles repo — from reporting that repository's commits. A linked worktree, where `.git` is a file, is recognized.
