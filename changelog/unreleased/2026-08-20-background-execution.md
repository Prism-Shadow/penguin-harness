...
- User-role `text` payloads gained an optional `sender` field (`"user" | "agent" | "harness"`): subagent prompts written by the parent agent are recorded as `sender: "agent"` in the child's Trace, harness injections (completion reports, scheduled-task triggers) as `sender: "harness"`. The field is additive — absent means the human user, which is also the correct reading of every Trace written before it existed — so no migration and no compatibility code is involved, and it never reaches the provider wire.
