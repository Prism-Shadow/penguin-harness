...
- The Trace page folds a completion notice's automatic round into the round that launched the background work (display-layer only — file and analysis untouched; tokens/duration sum, the later context snapshot wins), and each round's timeline gained a user lane: point marks for user-side messages, a filled dot for the human, a ring for machine injections, named by tooltip.
