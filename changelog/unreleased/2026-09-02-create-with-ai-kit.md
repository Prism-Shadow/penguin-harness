...
- `AiCreateModal` takes `primaryExit`: `"send"` by default, `"edit"` moves both the accent and the rightmost footer slot onto **Edit in a new conversation** — for a surface whose own copy warns about what the prompt will carry. Both exits stay available either way.
