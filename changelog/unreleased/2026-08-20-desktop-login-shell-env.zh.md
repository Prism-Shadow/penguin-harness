...
- shell 的簿记变量（`_`、`SHLVL`、`PWD`、`OLDPWD`、`TERM`）与 `ELECTRON_RUN_AS_NODE` 永不导入；`PENGUIN_WEB_DIST`、`PENGUIN_CLI_ENTRY` 同样排除——它们决定内嵌 server 提供哪份前端代码、从哪个入口自更新，为仓库检出写下的 profile 行不该改写已安装应用的指向。
