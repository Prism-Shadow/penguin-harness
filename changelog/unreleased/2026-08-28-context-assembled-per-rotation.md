...
prompt cache holds; the vault, per-tool `r`/`rw` permissions and the command policy never
reach the model but are read once per context, on the same rotation schedule), **soft** (the
thinking level: changeable mid-context, effective from the next request, at the cost of the
provider's cached context — the pickers advise compacting first), and **unrestricted** (the
approval mode: re-read from the DB per decision, effective immediately).
...
- Per-tool `r`/`rw` permissions and the command policy are strict-tier as well:
  ...
  `.project_config.toml` once per context open — no approval decision reads a file. The
  unrestricted tier is the approval mode alone, re-read from the DB on every decision.
- Fixed for the Session's lifetime: its id, Workspace, model entry (credentials, window and
  per-model annotations included), origin, and the Environment's process host — background
  commands, subagent child sessions and their listeners survive the rotation.
...
- SDK: `SessionConfig.commandPolicy` takes a source function instead of a static config — the
  ...
  signature and answers from the running context's toolset.
