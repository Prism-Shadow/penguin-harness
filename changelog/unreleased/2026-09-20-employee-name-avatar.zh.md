...
- 头像按聊天产品的尺寸绘制，而不再沿用图标的尺寸：侧栏工位列表与频道成员列表为 22px（原 14），折叠侧栏为 24px（原 18），频道里一段消息开头的头像为 36px（原 28，旁边的发送者名字也大一号）。工位行在名字后面写出职务——`小明 (CEO)`——备注用较淡的颜色，行内放不下时备注先让位；没有自己名字的员工以职务示人，职务在前、id 作备注：`Developer (acme_dev_a)`。
- 频道里的 @ 胶囊在名字后带上职务：`@小明 (CEO)`，职务的色调浅一档。消息原文不变——键入与存储的仍只是名字。
- CLI：`penguin org employee avatar [<agent_id>] (--file <image> | --clear)` 从 PNG、JPEG 或 WebP 文件设置或清除头像，发送前先检查格式与大小。id 缺省为 `PENGUIN_AGENT_ID`，员工在工位里即可给自己设；员工 Skill 列出了这条命令，`agent-company` 为 `2026.09.25.1`。
