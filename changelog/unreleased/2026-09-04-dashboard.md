...
A new page, reached from the user menu under **Settings**, that fits a phone and answers one question: which Workspaces have Sessions running right now, and which have a Session waiting on an approval. One row per Workspace, two numbers per row, nothing else — no Session list, no titles, no controls. The header repeats the two totals.
