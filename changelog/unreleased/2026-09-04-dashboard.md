...
A new page, reached from the user menu under **Settings**, that fits a phone and answers one question: which Workspaces have Sessions running right now, and which have a Session finished since you last opened it — the sidebar's green dot, counted. One row per Workspace, two numbers per row, nothing else — no Session list, no titles, no controls. The header repeats the two totals.
...
- The server answers `GET /api/projects/:projectId/sessions/overview` with every non-archived Session of the Project over every Agent — its Workspace, status, whether it ever ran, when it last did. The page counts from those the way the sidebar draws its glyphs: running while the status is not idle, to review while a settled Session has run since this browser last opened it. Read versus unread is a per-browser marker, so the counting is the page's, not the server's.
- The page is not in the sidebar's nav group; its row sits directly under Settings in the user menu.
