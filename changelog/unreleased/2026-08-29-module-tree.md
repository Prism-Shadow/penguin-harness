...
The root has fourteen children, not seventy-four: `RuntimeModule`, `SettingsModule`, `IdentityModule`, `ProjectsModule`, `SessionRuntimeModule`, `ObservabilityModule`, `TracesModule`, `AgentsModule`, `WorkspaceModule`, `MessagingHubModule`, `ApiModule`, plus the sandbox, terminal and machines modules and `Startup`. A group is `@Module({ children, exports })`: its `exports` are the interfaces its children offer the rest of the tree, forwarded from the child that declares each; everything else in the group — the repos, the seams a test replaces, the route components — is visible only inside it (the kernel's visibility is lexical: siblings and the ancestors' siblings, never an ancestor). A whole group is replaceable, and so is any node in one. Ordering follows the exported child, not the group, so two groups may need each other's children without a cycle. The generator resolves every requirement to the module that will provide it and records it as `from` — the table now says which *module* a node depends on, and the page shows who depends on whom.
