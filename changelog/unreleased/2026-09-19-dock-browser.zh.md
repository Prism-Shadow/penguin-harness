...
- **弹窗与对话框可用。** 页面打开的窗口（`window.open`、指向站点自身的 `target="_blank"` 链接——OAuth 登录、打印视图）是站点主机上的真实窗口，不受 iframe 沙箱约束：`window.opener` 与回页面的 `postMessage` 可用，弹窗可自由导航。`alert`、`confirm`、`prompt` 同样允许。桌面端里这样的弹窗作为 App 自己的窗口打开（实例端口上的 `<label>.localhost` 主机属于它的本地面），而不是交给系统浏览器——那会丢掉 opener 关系。
...
- 顶部一行为后退、前进、重新加载与地址栏。停靠栏标题栏的「弹出」按钮（与终端同一个）把页面弹到网页浏览器自己的标签页里（它的 `<label>.localhost` 地址，Cookie 与 storage 随之保留；桌面端里是 App 自己的窗口），那个标签页关闭后停靠栏标签自动回来。标签所示地址随停靠栏布局一并保存，刷新后回到原页面。
