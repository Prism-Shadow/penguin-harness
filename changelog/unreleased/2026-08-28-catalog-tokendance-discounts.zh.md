...
- **模型卡片改为三行**：第一行是名称（字号小一号）与其后的上游 id，第二行是全部常驻标记独占一行——默认、
  视觉、视觉代理、快速、免费、折扣，顺序固定，好让视线学会去哪里找——第三行仍是原来的价格行。这些标记
  此前挤在标题那一行，每一枚都是模型**名字**让出来的宽度。标记行即使为空也会渲染，高度就是标记自身的
  高度：多数模型一枚标记也没有，而两列网格中同一行的卡片会被撑到最高者，因此一个会塌陷的行表现出来是
  参差的留白，而不是一张更矮的卡片。
