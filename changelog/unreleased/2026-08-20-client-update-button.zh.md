...
在桌面 shell 自身窗口的侧边栏用户菜单中新增了客户端更新行——正是服务端更新提醒行在桌面模式下空出的那个位置，桌面模式的系统设置里没有更新页。该行端到端驱动 shell 的 electron-updater:检查、带进度的后台下载、经确认的重启安装(与 shell 原生提示同样的「打断运行中任务」警告),并同排展示当前安装的客户端版本。
...
- 由该行发起的检查以 toast 精确报告一次结果——发现新版本(自动开始下载)、已下载待装、已是最新、不支持自更新、或检查失败——与系统设置「更新」页手动检查的约定一致。更新器给出了原因时,失败 toast 会带上 shell 自己的说法,因此被校验和或签名拒绝的下载不会被读成一次网络抖动。不支持自更新的形态(开发运行、非 AppImage 的 Linux 安装)渲染为禁用行,原因挂在外层元素的 tooltip 上而非禁用控件本身——禁用按钮收不到指针事件,自身的 tooltip 永远不会弹出。
- 原生菜单流程(Check for Updates…、其对话框、下载完成后的重启提示)保持不变。
- Web App 指南新增了介绍该行的段落,中英文各一份。
