...
## An `out` forward is cleared before it is carried

An `out` forward's listener is on the machine, and where it binds is the machine's sshd's call, not ours: a `-R` asks for `127.0.0.1`, and a sshd with `GatewayPorts yes` binds every interface instead and says so only in a debug message (measured on OpenSSH 10: the client reports success either way; `no` and `clientspecified` keep the loopback, and under `no` a client cannot widen it either). So before an `out` forward is ever handed to a session, the server asks the machine — one probe connection that requests a listener of sshd's own choosing, bound as every `out` forward asks and delivering to a closed port, lists the machine's listeners while it is up, and ends — and reads the answer: sshd's own "overridden by server GatewayPorts" first, the listing's address for that port second (`ss`, or `netstat` in its Linux, macOS and Windows spellings). The forward is carried on its own only when the answer is `loopback`. `exposes` and `unknown` — the connection failed, the port was not reported, the listing has no tool or no such port — are refused alike: a machine that cannot be read is not assumed safe. A definite verdict stands for ten minutes; an unknown one is asked again.

- **`POST /api/port-forwards`** answers such an `out` forward `409` `forward_exposes` or `forward_exposure_unknown`; the Ports panel turns that into a dialog that says what would happen and offers to open Settings.
- **Settings › Ports** (admin, server-global) lists every machine with its last verdict in words, a Probe button, and a switch — "Forward even when exposed" — that consents to that machine (`GET /api/port-forwards/exposure`, `PUT …/exposure/:machineId {allowed}`, `POST …/exposure/:machineId/probe`; the consent is one list of machine ids in `server_settings`). Consent hands the machine's withheld forwards over at once; withdrawing it takes them back off.
- **An `out` forward on record whose machine is not cleared** — a build before this, or consent withdrawn — is held back rather than carried, reads `exposure-refused` (red, with which of the two reasons), and its `in` neighbours go as before. One that was `unknown` is tried again from the panel's poll once the machine is connected, at most every thirty seconds.
