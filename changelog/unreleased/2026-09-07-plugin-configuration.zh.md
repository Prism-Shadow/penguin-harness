...
- **「插件」页。** 在设置弹窗的服务器分组里，仅管理员，排在「沙盒」之后：每个声明了选项的已加载插件一张表单，按其 schema 画出——密钥字段留空、下方是存储值的掩码与清除勾选，Project 字段是 Project 选择器，布尔是开关。每个插件各自保存、一次 PUT；服务端拒绝的字段在该字段下标红。插件列表页的页头另有一个齿轮图标按钮（仅管理员），直接打开设置弹窗的这一页。
