...
- **Skill 清单改为由测试锁定到实际发布的库。** `packages/landing/test/skills-sync.test.ts` 断言两份词典的 Skill 区块恰好列出 `packages/skills/skills/` 下的全部目录；`packages/skills/test/skills.test.ts` 把原有的 README 校验扩展到两份根 README，与该包自身的 README 一并覆盖。两者都只比较集合而不比较顺序，并在失败信息中点名缺失的 Skill。
