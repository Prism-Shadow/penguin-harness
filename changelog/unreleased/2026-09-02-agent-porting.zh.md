...
- Agents 页页头新增**导入智能体**——弹窗含**从文件导入**（包或单独的定义文件，id 按文件名预填；id 已占用时就地提示以便重试）与**让 AI 导入**（「让 AI 创建」面板，示例覆盖本机 Claude Code、Codex、Pi 的设置与导出包，固定尾注点名 `agent-porting` 技能与目标 Project）。每张卡片的**导出智能体**打开与之对称的弹窗，分三段：**导出 API 调用示例**、**导出 Docker 文件**、**让 AI 导出**。导入与导出因此同形——两侧都既有服务端应答的表单路径，也有交给 Agent 的提示词路径。设置页概览在快照的一对操作旁显示同一导出，命名区分。
