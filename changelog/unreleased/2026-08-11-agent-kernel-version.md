...
## The overview settles into cards

The kernel controls started it, and the rest of the overview follows (#261): version display, Update and Restore group into a **Kernel** card, and a sibling **Agent State** card carries the State version with the snapshot import/export actions beside it and the state path below — now click-to-copy, the chat details card's optimistic "Copied" convention, with the full path on hover. The name/description form keeps its Save beside the fields it actually saves, and the active-session statistic is dropped from the overview.
