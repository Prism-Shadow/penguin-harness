# Backward compatibility: legacy baked templates and the section placeholders

Scope: the prompt-injection change (#257). `system_config.yaml` is baked at agent creation and never auto-upgraded, so every agent created before this change carries the old template text — hardcoded `# Vault` and `# Skills` sections with inline `{{VAULT_KEYS}}` / `{{SKILL_METADATA}}` placeholders, and no schedules section.
