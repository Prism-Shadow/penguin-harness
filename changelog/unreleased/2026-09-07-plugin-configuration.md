...
- **A backend declares its own group** with `parent: "sandbox"`, beside its `SandboxModule.providers` contribution, and reads it through `PluginConfig` at each spawn. bwrap declares its program and a probe timeout of 1–30 seconds, and Seatbelt its program. A changed program is probed again.
