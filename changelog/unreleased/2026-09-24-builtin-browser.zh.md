...
- 每个标签页都是一个 Electron `<webview>`，位于自己的持久化分区 `persist:penguin-browser` 中，因此内置浏览器的登录状态和网站数据与应用本身、与系统浏览器都相互隔离。页面看到的是普通 Chrome 的 user agent。侧栏关闭时，标签页仍以桌面尺寸保持挂载，Agent 读到的内容不受窗口布局影响。
