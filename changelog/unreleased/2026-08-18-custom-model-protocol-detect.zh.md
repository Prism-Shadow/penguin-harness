...
- 「检测协议」按钮位于 base URL 输入框右上角、与其标签同一行——与 API Key 字段「获取 API key」链接的摆放方式一致——并且始终可点击：不需要填 API Key，探测凭据由服务端自行解析（见下）。检测从此只有两个入口：用户主动点这个按钮，以及下面保存时的兜底。此前 base URL 失焦时的隐式触发已移除——反馈改走弹窗之后，被动触发要么在用户填表途中打断他，要么失败了也说不清原因。
...
- **成功与失败都会弹窗**（新增 `AlertModal`：沿用 ConfirmModal 的紧凑卡片，只保留一个关闭按钮）。成功时说明应用了哪种协议；所有失败情形——连不上、超时、网关杂讯、三条路径都没提供、URL 不可用——统一显示同一条简短提示，只讲用户能动手改的两件事：API Key 与 base URL。不再罗列协议名，也不再描述端点做了什么：「端点有响应，但……」会被读成成功。逐个协议的探测结果仍保留在检测接口的响应里，便于排查。成功弹窗只属于手动点击「检测协议」；保存路径上检测成功则静默继续，否则会打断用户已经确认的保存动作。
