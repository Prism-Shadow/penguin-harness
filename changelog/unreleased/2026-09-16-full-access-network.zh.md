...
- **完全访问且断网或屏蔽路径**时，现在会走到一个后端去执行这一维度，同时不限制文件系统。这是对沙盒契约的小幅放宽：provider 现在可能收到 `danger-full-access`，此时它必须放开文件、但仍要断网或隐藏路径。bwrap 把根目录以读写方式绑定并仍 `--unshare-net`；Seatbelt 不拒绝任何写入但仍 `(deny network*)`。
