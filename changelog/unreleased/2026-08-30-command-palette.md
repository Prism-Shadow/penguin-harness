...
**Rollback.** The runtime's store keeps one rollback copy of each artifact; the platform keeps the whole last five versions under `<root>/harness-history/versions/` — each as the upgrade body a hand-over forwards (bundles, web archive, native assets with their exec bits, provenance), read by the same helper the Machines page uses. Any kept version has a *Roll back to this version* button (admin; two clicks): the platform hands the kept body to the runtime's upgrade channel in-process — no network hop, bind address, HTTPS gate or token involved — and answers before the swap. A push that lands reloads every tab, as a push from outside does; one the runtime refuses leaves the platform running, and the refusal is shown on the history in the runtime's words (`lastRollback` on `GET /api/version/history`). `POST /api/version/history/rollback { id }`.

The record itself is written once the runtime's commit has landed, never at boot: on a push the runtime commits after the boot succeeds, so a line written earlier would carry the previous version's bundles under the new platform's table. Writes are atomic and serialized, and a read of the history writes nothing.
