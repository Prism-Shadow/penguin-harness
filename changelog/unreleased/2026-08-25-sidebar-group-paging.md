...
- One click reveals one page more, and fetches the next server page only when the reveal runs past
  what is already loaded. In Workspace mode a fetch fans out to every Agent contributing to the
  group and its rows may land in other groups entirely, so a request that would show nothing here
  is no longer spent.
