...
- 模型 id 按端点返回顺序保留。当某条 id 与已占用的 id 重复（列表内部重复，或与已配置的 `(provider, model_id)` 对撞），或配置根本无法承载（为空、超过 200 字符的 id 长度上限、含控制字符）时，该条会被跳过并计入成功提示，单个坏条目不会让整次导入失败。
- 端点那边只取 id：价格、上下文窗口与显示名一律留空，不把供应商自报的数字写进遵循内置目录定价约定的字段；导入的模型在被探测或手动打开之前不声明视觉能力——与手动往自建分组添加模型完全一致。
