...
- 公告可以链接到站外。`announcement-bar.tsx` 中每条公告恰好带 `to` 与 `href` 之一：`to` 是经路由跳转的博客路径，`href` 是 `https://` 地址，渲染为在新标签页打开的普通链接（`target="_blank"`、`rel="noopener noreferrer"`），样式、箭头以及焦点与 `tabIndex` 处理与博客链接一致。两者都带或都不带的条目通不过条目类型的检查。
