...
- `ApproveFn` 的拒绝现在可以不返回裸的 `"deny"`，而返回标注决定者的 `ApprovalRefusal`。两种拒绝的工具输出都是固定的 `aborted` 一句——`Tool call denied by user.` 或 `Tool call denied by policy.`——`approval_decision` 事件新增可选 `source`（`"human"` / `"policy"`，只落非缺省的 `"policy"`），模型看到的文案、Trace 查看器与工具卡片的决策标签由此都点名拒绝者；字段出现之前的事件一律按 human 理解。返回 `"allow"` / `"deny"` 的既有回调行为完全不变。
...
存量与新建 Project 都会开始拒绝出厂集里的毁灭性命令，即使在全部放行模式下——依赖 `rm -rf` 的日常操作（如删除 `node_modules`）会收到固定拒绝文案 `Tool call denied by policy.`，无论是经 `exec_command` 启动，还是用 `input_command` 敲进已运行的 shell。要恢复某个具体行为，可在 Project 设置 → 安全策略中停用或编辑对应那一条规则（或整体关闭策略）；不涉及数据迁移。本改动之前创建的项目没有 `[command_policy]` 块，按出厂集生效，首次保存编辑时列表才写入 `.project_config.toml`。
