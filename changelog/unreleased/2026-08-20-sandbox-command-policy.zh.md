...
- `ApprovalDecision` 新增第三个取值：`"forbidden"` 是命令策略的拦截，由 `Session.run` 的包装作答、从不出自宿主。两种拒绝的工具输出都是固定的 `aborted` 一句——`Tool call denied by user.` 或 `Tool call denied by policy.`——`approval_decision` 事件携带的就是决定值本身，模型看到的文案、Trace 查看器与工具卡片的决策标签由此都点名拒绝者，无需额外字段。返回 `"allow"` / `"deny"` 的既有回调行为完全不变。
