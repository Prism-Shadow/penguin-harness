...
- [2026-08-07] Memory: an agent keeps long-term notes between Sessions under `agent_state/memory/` — a user scope (`memory/user/`, read by every Session) plus one directory per Workspace, each with its own `MEMORY.md` index; only the indexes enter the context (fenced by `[user_memory_index]` / `[workspace_memory_index]` markers), bodies read on demand. A temporary Workspace gets the user scope only. `memory.enabled` is the only on/off switch — a template without `{{MEMORY}}` still injects, spliced before `# Environment` — so existing agents need no migration. Agent settings gain a Memory tab: memories grouped by scope with view / delete (index lines pruned) / edit-via-chat. ([details](2026-08-07-memory.md))
