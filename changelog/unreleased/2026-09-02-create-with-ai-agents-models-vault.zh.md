...
- 密钥保险柜标签页在**添加**旁新增魔法棒按钮（仅 owner），弹窗如实提醒写进提示词的值会发给模型服务商、进入对话记录（Trace），也会出现在智能体执行的命令里，建议只让 AI 建键名；尾巴在每条 `penguin config vault set` 上带上目标 Agent、Project 与数据根目录，禁止复述值或读取 `.vault.toml`，最后 `penguin config vault list`。
