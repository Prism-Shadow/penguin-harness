...
The Windows `rest` shard failed for its own reason: the Claude Code surface test asserted the searched paths as one string, separator and all, which reads differently on Windows. It asserts the message rather than the separator now.
