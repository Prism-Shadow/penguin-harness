...
- 内置示例 Benchmark 有了版本：`benchmark_config.toml` 多一个只有示例才写的 `example_version`。装载 `default_agent` 时，旧版本留下的示例（config 没有版本号或版本号更小）整目录换成当前示例，追加在它上面的评估一并丢弃；删掉的示例不会写回，版本相同的示例原样保留，旁边用户自己的 Benchmark 一概不动。
