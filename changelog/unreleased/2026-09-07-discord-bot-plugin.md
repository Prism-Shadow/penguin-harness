...
- **Configuration is the Plugins page of the Settings dialog, and nothing else.** The
  plugin's module declares its settings group beside its route — `bot_token` (a secret),
  `project` (the Project's id), `agent` (default `default_agent`) and `enabled` — and an admin
  fills them in there; the plugin reads its own group, watches it and restarts the bot on a
  save, without a server restart. See
