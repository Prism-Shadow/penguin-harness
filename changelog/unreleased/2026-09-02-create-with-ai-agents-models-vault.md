...
- The Vault tab gained a wand beside **Add** (owner only) whose dialog warns that a value typed into the prompt reaches the model provider, the conversation's Trace and the agent's own command line. The rest of the surface follows that warning: the examples lead with creating the key names an agent's skills need and filling the values in by hand afterwards, none of them pastes a secret into the prompt, and it is the one surface whose emphasised, rightmost exit is **Edit in a new conversation** rather than **Send to agent**. Its tail names the target agent, the Project and the data root on every `penguin config vault set`, forbids echoing values or reading `.vault.toml`, and ends with `penguin config vault list`.
