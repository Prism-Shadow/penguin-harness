...
新建 Agent 的 `compaction.max_context_length` 初始值由 `128000` 改为 `512000`。真正约束压缩
的是模型自身的 `context_window`：生效阈值在每次使用时都会被收敛到
`context_window − COMPACTION_HEADROOM`（2048）以内，配置得再大也无法把压缩推到会撑爆窗口
之后。把初始值提上去，大窗口模型才用得上它实际拥有的上下文，而小窗口模型的触发点与此前
完全一致。
...
- 采用新初始值后，32768 的窗口仍在 30720 触发压缩，200000 的窗口在 197952 触发。只有窗口
  大于 512000 时，触发的才是配置阈值本身。
...
- 存量 Agent 不迁移。Agent 始终按其磁盘上的 `system_config.yaml` 原样运行，所以已在磁盘上
  ...
  128000。只有此后新建的 Project 与 Agent 才以 512000 初始化。
...
- 内置默认发生变化，`KERNEL_VERSION` 因此前进到 `2026-08-20` 一代，其唯一变化的叶子就是
  ...
  于某一代已记录默认的存量值会跟进到 512000，被用户改过的值则保留并在结果中列出。

## 文档

- 双语的配置页与 Agent 循环页记录了新的缺省值，并写明实际触发的是模型窗口给出的数字，以及
  未配置 `context_window` 的条目按什么推导。
