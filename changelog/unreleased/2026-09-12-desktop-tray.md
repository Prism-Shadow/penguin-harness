...
- The icon is rendered by `scripts/render-icon.mjs` and committed under `build/tray/`: a 32px colour image (with a 64px `@2x`) for Windows and Linux, taken from the app's own artwork, and a 16px monochrome template image (with a 32px `@2x`) that the macOS menu bar inverts with its own appearance. The menu bar image is drawn for its size in `scripts/menu-bar-glyph.svg` rather than taken from the brand mark: an illustration with gradients, separate flippers and a beak has more in it than 16 pixels can hold, and shrinking it produced a mark covering 19% of the canvas, nearly all of it antialiasing, that read as a smear beside the solid marks around it. The drawn one keeps only a head, sloping shoulders and a notch for the feet, and covers 45%. Two assertions hold that: the template images carry no colour, and neither falls below 40% coverage. `scripts/build-assets.mjs` stages the set into `dist/tray/`, where the shell looks it up relative to the app directory, so a source run and a packaged app read the same layout. The tooltip is the app name, dev suffix included.
