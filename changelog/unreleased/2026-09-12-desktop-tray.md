...
The desktop app took a place in the system tray — the Windows notification area, the macOS menu bar, the Linux tray — and closing the main window came to hide it there by default, leaving the embedded server and its background tasks running with a one-click way back.
...
- **Keep running in the tray when the window closes** is on by default, and closing the window then only hides it, on all three platforms. Turning it off restores the previous behaviour: on macOS the window closes and the app stays in the Dock, on Windows and Linux the app quits. The preference is stored in `userData/tray.json`; a missing or malformed file reads as on, and a write that fails is logged and nothing more.
...
- Deliberately not included: a recent-sessions submenu and a running-activity indicator on the icon. Both need the shell to ask the server what is going on, and the two exchange nothing but a shutdown token and update status today.
