...
- API：`/api/projects/:projectId/organizations` 及其子路由——员工树、员工、工位、手册、日程、工单（移列、阻塞、解除、进展、发起、挂接）、频道（成员、消息与已读游标）、财务与组织会话列表；用户级事件 `org_run`、`org_channel`、`org_ticket`、`org_budget`。Project 成员可读写，仅 owner 可删除。迁移 6 新增七张组织表。
...
- 工单的变化不再启动任何一轮运行。指派负责人、标记阻塞、阻塞解除、完成、拒绝，仍照旧记录——写进工单文件、该变化在全员频道有系统消息时写进全员频道、并发出 `org_ticket` 事件——同时排入相关员工的队列；每人的下一条日历事件在正文的 `## Since your last sweep` 一节里带上它们：一条变化一行，写明工单、标题、发生了什么，以及工单自身带的理由或阻塞方（文件已不在时写 `(ticket removed)`），最后一行说明该怎么决定——发起工单会话、核验后解除阻塞，或者放着不动。驱动工位会话的只有日程项、频道里的提及与直接找它说话的人，CEO 创建时的初始化运行是唯一例外。组织或员工处于暂停时队列照样写入，由此后真正触发的那次巡检送达；员工离职时，尚未送达的行随之删除。迁移 8 新增存放该队列的 `org_desk_notices` 表。`ticket_notice` 仍留在 `OrgTriggerKind` 中，因为它存在期间写下的 Trace 带着它；现在已没有任何地方再写出这种触发。Skill、组织手册模板、公司模式指南与服务端 API 参考都已照此叙述。
...
- 公司现在可以镜像一家现实公司。`agent-company` 插件新增的 `company-mirror` Skill 是这样一家组织的协议——它的员工是现实同事的数字分身：CEO 的初始化运行向董事会索要现实公司的组织图（姓名、头衔、汇报线，以及哪个机器人归谁），然后就结束本轮；董事会确认花名册之后，才为每人招募一个分身，`--reports-to` 照搬现实汇报线，并写下每个分身的简介与每个人的 `people/<姓名>.md` 手册页，最后汇报董事会需要去绑定的那些工位会话。这样的公司没有任何周期性驱动：不排日程、不开工单、不按工作线开频道。分身只在自己的同事通过绑定的机器人写来消息时运行——能从手册里答就答，答不了就用 `@` 把问题转给相关同事的分身，对方把这个问题作为本轮的最后一句回复，由机器人送达真人——带回来的答复一律写进手册，下次同样的问题就能自己答掉。`company-ceo` 的初始化清单与 `company-employee` 的工位一节都会把镜像使命引到这里，CEO 的初始化运行也在第一句就这么说，中英皆然。
- 新建组织对话框与公司模式的空状态落地页新增第四个使命示例「员工数字分身公司」（英文 “Digital-twin company”），其使命正是这样一家镜像公司。公司模式指南新增一节讲它建出来的是什么；`company-setup` 在用户谈的是自己的同事、而不是要产出的活时，会主动提议它。
...
### 用户试用后的公司页面（2026-09-15）

- **标题即链接。** 卡片、行、面板整体从来不是点击目标，它的标题才是——一枚文字按钮，点它
  展开或跳转，其余表面惰性。工单卡片点标题开抽屉、拖动其他任何地方即移动；概览的收件箱、
  今日日程、预算告警都从标题跳转；没有标题的行（KPI 卡）保留角落小按钮。
  `test/company-click-targets.test.ts` 解析公司模式各模块，非控件表面上的点击处理、
  `role="button"` 或 `cursor-pointer` 一律判失败。
- **工单显示得更少、说得更清楚。** 卡片只有标题、优先级 / 阻塞 / 截止角标、一行「父工单：…」
  与负责人——不写会话数、不写成本，工单界面任何地方都不再显示会话的运行状态。抽屉把成本叫
  本单成本 / 总成本，父工单是带「打开」按钮的字段，其下三节折叠：子工单（行尾打开按钮，节标题
  带数量与总成本）、关联工单会话（行尾打开按钮；发起与挂接的入口移出抽屉，API 与 CLI 仍在）、
  操作历史（只有时间、主体、操作类型）。
- **财务台账折叠子工单。** 子工单缺省藏在父工单之下，点 chevron 才展开（折叠行写明几张）；
  上卷列改称「总成本」。
- **日历给空格子起名。** 空的日格或小时格悬停 / 聚焦时以强调色铺底，并在最上层显示
  「新建日程」；格子是位于日程之下的真按钮，点日程永远不会误开新建表单。按某位员工筛选后，
  图例里其余员工加删除线。
- **手册目录就是共享的目录树**——与对话页 Workspace 浏览、插件库同一个 `FileTree` 组件，
  索引固定为第一行，文档的大小与修改时间在行尾。
- **公司模式标明自己是内测功能。** 组织切换器的名字旁多了一枚「内测版」标签（折叠侧栏则在
  「切换到公司模式」的提示后加同样的后缀）；管理员总开关下方常驻一行「内测功能：可能有不稳定
  的现象，遇到问题请反馈。」，同一句也进了该页的「?」；第一次在某个浏览器里切到公司模式时，
  这句话以 toast 弹一次，之后不再出现（记在 localStorage 的 `penguin.companyBetaNoticeShown`）。
- **工单详情是原地打开的对话框。** 居中窗口（同设置界面的形态），不再是侧栏抽屉；挂在公司壳上
  只有一个，看板标题、财务台账、概览收件箱、频道消息里的工单引用都从原地打开它；父 / 子工单
  在同一对话框内切换、带「返回」；工单页保留 `?ticket=<id>` 供直达。切页只从菜单发生：财务台账与
  频道工单引用不再跳页；概览保留跨菜单跳转（它是仪表盘），打开会话仍去对话页。P0 / P1 / P2
  角标紧跟标题同一行、字号小一号。
- **裸 URL 止于结尾的强调标记。** `请**打开 http://127.0.0.1:4321/**查看` 过去会把 `**`（连同后面
  的中文）吞进链接；autolink 边界处理现在裁掉尾部的 `*`、`~`、`` ` `` 串、段落里开过的 `_` 串，
  以及无配对的 `)` / `]` / `>`，交还给正文。
- **「内测版」角标**从组织切换器移到「开发 | 公司」切换里「公司」二字的右上角，小号上标、不改变
  控件尺寸；该选项的可访问名为「公司 · 内测版」。
- **员工自受雇起就有工位会话。** `hire` 在写组织图的同一把锁里把工位开出来——开的是会话、
  不是运行轮，因此不派发任何触发——开失败也不影响招募成功（原因记为
  `org_desk_unavailable`）。此后每轮对账都会为缺工位的员工补开：手工写进 `org_chart.yaml`
  的人、此前招募的人，以及工位会话记录已被删掉的人（手工删会话，或随 Agent 一起删）。
  暂停的组织与关掉的总开关同样照补。在此之前，工位只在该员工第一次被日程触发、被提及或
  被点开侧栏时才打开，于是新招的员工的「工位」行没有会话可指，而指着已删会话的行在绑定
  聊天机器人时会答「该 Session 不存在」。
