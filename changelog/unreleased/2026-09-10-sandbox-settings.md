...
- **What enforces it, said plainly — and what does not, with why.** The card lists the backends in use and the isolation each implements. A backend that FAILED is named with its reason: bwrap, Seatbelt and MXC now check at load that they can serve on this host — an installed SDK, a runner that passes its probe, and for MXC a real shell started inside a container, since MXC confines with an AppContainer and an MSYS2 shell (Git Bash, the harness's first choice on Windows) dies at STATUS_DLL_INIT_FAILED before running anything — an AppContainer denies the global object namespace its `\BaseNamedObjects\msys-*` directory needs, and no policy or file permission reaches that; the reason says so and points at `PENGUIN_SHELL` — and refuse to load, with the reason, when they cannot, so no backend is ever silently absent (nor does a Windows host route a policy to bwrap). A backend for another platform simply declines: nothing is wrong with a deployment that installs one per platform, so it is named only when nothing serves, where it explains why. When the saved mode needs isolation no usable backend implements, the card warns that every agent command will be refused; with no usable backend at all it says a mode confines nothing.
