...
TokenDance 分组头部新增**自动获取密钥**动作：经供应商的授权页在用户的 TokenDance 账户下新建一个 key，并写入该分组下的每一个模型，首次配置不必再去控制台手动复制。
...
- 分组头部的五个动作改用短标签——**添加模型**、**自动获取密钥**、**手动设置密钥**、**测速**、**前往密钥管理**——并调整顺序，自动授权排在手动设置之前。
