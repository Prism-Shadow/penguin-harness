...
- **保存在 `web.db`**（表 `port_forwards`，migration 15 `port-forwards`，swap-safe）。`in` 转发的本地端口缺省取与远端端口同号且本地空闲的端口，否则向上取第一个空闲端口，也可以指定（1024–65535）。`out` 转发须指明送出去的本地服务，机器端口缺省与之同号。
