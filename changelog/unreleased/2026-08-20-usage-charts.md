...
- The share pie and the per-model progress bars were replaced by one full-width **requests + success-rate combo**: request bars from the bottom on the left axis, the success-rate line on top against its own right-hand 0–100% axis with a dot at the line's end, and a legend underneath. A by-Agent / by-Model toggle plus an entity dropdown in the card header switch between the all-entities view — bars stacked by entity (top entities keep their own CVD-checked hue, the tail folds into a neutral "other") under the overall success line — and one entity's own bars and success line, which is where a single model's success rate now lives, labeled.
