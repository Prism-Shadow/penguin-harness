...
Both command tools are excluded, not just the obvious one: the tool that polls a backgrounded command reaches the same exit-code mapping, so excluding only the foreground path would record the same command's non-zero exit depending on how it happened to finish. The remaining failure paths of the polling tool — an unknown process id, an interrupt mixed with other input — are the model misusing the tool, which it likewise sees and corrects on its own.

The exclusion happens where errors are captured rather than where they are queried, so the noise never reaches the table, the row cap, or the deduplication key.
