...
上面那条在机器上验证时发现的：修好的插件确实随推送的产物到了机器上，跑的却仍是旧的。插件宿主会复用上一个 App 已经 import 的对象，好让模块标识跨 swap 存活——但它只按 specifier 匹配，而一次推送会把内置插件写进**新的 assets 目录**。于是推送之前持有的条目会永远继续跑上一版的代码。

现在复用要求 specifier 仍然解析到**同一个文件**。文件不同即代码不同，重新 import。
