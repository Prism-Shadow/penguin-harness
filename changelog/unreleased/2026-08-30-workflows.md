...
The `penguin-sdk` skill documents the layout and the contract. The new-chat page's *Build and optimize agents* examples gain **Mission control for your agents** — a workflow that dispatches one task to several of the Project's Agents in parallel and shows every Session's live status, with a second tab of statistics and a fill-the-app view — in place of the Claude Code docs RAG example.
