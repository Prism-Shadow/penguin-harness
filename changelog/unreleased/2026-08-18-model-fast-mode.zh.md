...
模型对话框新增「快速模式」开关，默认关闭，且只对路由后的 AgentHub client 能提供该档位的模型渲染——`fastModeState` 依表单的实时取值重算，因此编辑 id、协议或 base URL 时，开关会随输入即时增减。它与视觉开关采用同样的行内 switch 形态；处于开启状态时带一行提示（同时作为标签的悬停 title），说明这是以溢价换更快的输出，而条目记录的价格仍按标准档。
