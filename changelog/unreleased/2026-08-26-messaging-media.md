...
- An image that is too large or cannot be downloaded answers with one bilingual notice and starts no Task — a model asked about a picture it never received would answer confidently about nothing.
...
- When a run ends, the files its reply MENTIONED follow the text into the chat: inline-code paths that resolve inside the Workspace and actually exist — the Web App's file-card rule, applied to the chat, so what arrives is the output the Agent pointed at rather than everything the run happened to write.
