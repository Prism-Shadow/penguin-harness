...
- Discoverability: `penguin server` starts the service directly, so nobody browses its subcommand help — the login page's footer gains a second hint line naming the command (the one place a locked-out admin is guaranteed to be looking), and the `server` command description mentions the subcommand so it shows in top-level `penguin --help`.
