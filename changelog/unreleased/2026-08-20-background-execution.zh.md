...
- 轨迹观测页把完成回报自动发起的一轮归并进启动后台工作的那一轮(仅展示层——文件与分析结果不动;Token/时长求和,上下文快照取较晚者;归并轮渲染为连续单条时间线),每轮时间线新增用户行:用户侧消息以与模型条同族的极小柱子呈现——真人较深、机器注入较浅,tooltip 注明来源。
...
- 完成回报的聊天渲染改为思考块同款的可折叠一行:折叠只显示结论(「后台命令完成」/「后台任务失败」),展开可见句柄、退出详情与输出尾部。
