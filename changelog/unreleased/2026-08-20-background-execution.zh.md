...
- 完成回报的聊天渲染改为与思考块共用同一折叠行外壳的可折叠一行(抽出单一样式源 `disclosure-row.tsx`——宽度、内边距、折叠/展开两态颜色与 chevron 交互;工具卡与工作分组同样引用其类常量):折叠只显示结论(「后台命令完成」/「后台任务失败」),展开以工具卡输出样式呈现句柄、退出详情与输出尾部。轨迹观测页上,回报触发的任务保持独立一轮。
