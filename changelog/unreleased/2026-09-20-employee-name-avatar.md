...
- CLI: `penguin org employee avatar [<agent_id>] (--file <image> | --clear)` sets or removes the picture from a PNG, JPEG or WebP file, checked for format and size before anything is sent. The id defaults to `PENGUIN_AGENT_ID`, so an employee sets its own from its desk; the employee skill lists the command, and `agent-company` is `2026.09.25.1`.
