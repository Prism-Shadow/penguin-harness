...
Only the indexes, through the template's `{{MEMORY}}` placeholder: it expands to `memory.prompt` — what Memory is for, the save mechanics in template-example form, and both scopes with their indexes (`{{MEMORY_USER_DIR}}` / `{{MEMORY_USER_INDEX}}`, and `{{MEMORY_DIR}}` / `{{MEMORY_INDEX}}` inside the prompt's `[workspace_memory]` region, which only persistent-Workspace Sessions keep — a temporary Workspace is never told about a directory it does not have). The prompt is per-agent config, editable on the Memory tab. Injected index content is fenced by `[user_memory_index]` / `[workspace_memory_index]` marker pairs, a blank index injects an explicit "nothing saved yet" note, and injection is capped at 200 lines per scope — past the cap a truncation note tells the model to open the full `MEMORY.md` itself.

A template without `{{MEMORY}}` injects nothing — an agent created before Memory, for instance. The Memory tab reports this and offers inserting the placeholder as an explicit one-click action; nothing is ever spliced in automatically. The assembled prompt is recorded in `session_meta`.
...
The API is under `/api/projects/:p/agents/:a/memory` — overview, per-scope file listing, file read, file delete, and the idempotent placeholder insert; deliberately no content write or rename, since content changes go through chat. The memory prompt rides the ordinary config route. It never accepts a path: a file is addressed by agent, scope key and a name inside that scope, each validated and then re-checked for containment after resolution.
