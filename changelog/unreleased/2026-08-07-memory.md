...
A blank index injects an explicit "nothing saved yet" note, and injection is capped at 200 lines per scope, then at ~25k characters as a backstop for long-line indexes — past a cap a truncation note tells the model to open the full `MEMORY.md` itself. Both caps are declared in the default Memory prompt, so the model keeps the index short before ever hitting them.
