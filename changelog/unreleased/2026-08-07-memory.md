...
Only the indexes, through the template's `{{MEMORY}}` placeholder: it expands to `memory.prompt` — what Memory is for, the save mechanics in template-example form, the user scope and its index (`{{MEMORY_USER_DIR}}` / `{{MEMORY_USER_INDEX}}`) — plus `memory.workspace_prompt` (`{{MEMORY_DIR}}` / `{{MEMORY_INDEX}}`) when the Session runs in a persistent Workspace, so a temporary Workspace is never told about a directory it does not have. Both prompts are per-agent config, editable on the Memory tab. Injected index content is fenced by `[user_memory_index]` / `[workspace_memory_index]` marker pairs, a blank index injects an explicit "nothing saved yet" note, and injection is capped at 200 lines per scope — past the cap a truncation note tells the model to open the full `MEMORY.md` itself.
...
Each group header also carries an **Import** action — the edit modal's shape with a required content-or-source field (pasted text, a file path or a URL; the agent reads sources itself) — bridging into a new chat where the agent organizes the content into that scope's files and index. And the agents list cards' stat line shows the memory count next to the other counts, deep-linking to the Memory tab.
