...
设置里新增 **沙盒** 页（管理员）：封禁模式（关闭 / 仅工作区可写 / 只读）、是否断开网络、以及对被封禁命令屏蔽的路径。`GET|PUT /api/admin/sandbox`；改动对下一次命令启动生效，无需重启。设置与服务器的其他设置存在一起（`server_settings` 的 `sandbox` 键），热更新与重启后都依然有效。
