...
- **谁在实施封禁、谁没有及原因，都明说。** 卡片列出正在使用的后端及各自实现的隔离维度；加载**失败**的后端点名并给出原因：bwrap、Seatbelt 与 MXC 现在在加载时检查自己能否在本机工作——SDK 装没装、运行器能否通过探测，MXC 还会在容器里真正启动一次 Shell：MXC 以 AppContainer 实施封禁，而 MSYS2 Shell（Git Bash，本平台在 Windows 上的首选）会在执行任何指令之前以 STATUS_DLL_INIT_FAILED 死掉——AppContainer 不开放全局对象命名空间，它却需要在那里建立 `\BaseNamedObjects\msys-*`，策略与文件权限都够不着；原因里会这样说明，并指向 `PENGUIN_SHELL`——不能时带着原因拒绝加载，因此不会有后端悄无声息地缺席，Windows 主机也不会把策略派给 bwrap。面向其他平台的后端只是让出：一套部署按平台各装一个后端本来就没有问题，因此只在无人可用时才点它的名，那时它正是原因。当保存的模式需要的隔离没有任何可用后端实现时，卡片警告 Agent 的每条命令都会被拒绝；一个可用后端都没有时，说明选择模式不会产生约束。
