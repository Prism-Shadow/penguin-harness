...
- **Create scheduled tasks** (new folder, no pinned skills) — three briefs that check for the `{{SCHEDULES}}` section first, read the server's date and UTC offset from `date`, and write TOML into `<app_data_dir>/agents/<agent_id>/agent_state/schedule/`:
  - *A 9am daily planning check-in* — one `24h` task whose `start_at` is the next 09:00 local, bound to the current Session's `session_id`, so every morning lands in the same conversation as the progress review.
