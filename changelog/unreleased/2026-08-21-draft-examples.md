...
- **Conversational investment analyst** (Build and optimize agents, `penguin-sdk` + `web-design`) — a stock-market Copilot on the Penguin SDK that refreshes the market and sector picture every five minutes from startup, states the evidence behind each trend call, and gives the assistant a stock-lookup tool for asking about a specific ticker. It is framed as analysis of public data, not investment advice.
