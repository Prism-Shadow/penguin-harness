...
`exec_command` 与 `run_subagent` 正在执行时，对话里的工具调用卡片新增一个按钮，点击即把这次调用的工作
交还为后台任务：调用带着 `process_id` / `subagent_id` 正常结束，进程不会被杀掉，主对话不必等命令跑完就
继续进行。任务完成后仍以一贯的后台任务通知送回，与 `run_in_background` 启动的任务一致；转入后台的进程
...
- 调用转入后台后，工具卡片会带上后台标记；该标记读自调用返回的说明文字，因此刷新页面后依然保留。
