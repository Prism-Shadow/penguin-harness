# Agent settings: a dashed empty block, and skills import above the list
...
MCP Server list — traded their bare line of placeholder text for one shared block: rounded,
dashed-bordered, the message centered on both axes.
...
- The import action kept its modal untouched — the recommended chat install, the zip upload,
  ...
  changed, and it stays disabled until the installed list has loaded. The row renders in all
  three list states — loading, empty and populated — so the button never shifts and the empty
  block keeps a header above it. With the button gone from the tab header, the tab's
  description fold spans the full width of the panel, matching the other tabs.
