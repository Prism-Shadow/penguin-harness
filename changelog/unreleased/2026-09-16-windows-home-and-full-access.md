...
- **A confined command's home was remapped** to `C:\ProgramData\penguin\sandbox-home`, so `~/.gitconfig`, `~/.ssh`, tool caches and the harness's own config were all invisible, and a "write your home" test wrote to a decoy and reported success. The home is now the real one. Write-confinement never needed the home at all: a command runs as an account that owns nothing, so it is already denied everywhere, and the launcher grants it write on the **Workspace alone** — small, and fast. What the setup adds is READ on a **curated set of config paths** (the profile root, its own dotfiles, and `.ssh`, `.config`, `.aws`, `.gnupg`, `.docker`, `.kube`, `.azure`), so `~/.gitconfig` and friends resolve. Deliberately not the whole profile: a developer's profile holds millions of files in caches, `node_modules` and `AppData\Local`, and stamping every one would take an age and hand the sandbox their browser data for nothing. The full-access accounts get **modify** on that same set. The one thing still redirected is the temp directory, to a shared writable folder, and only when "temp writable" is on.
