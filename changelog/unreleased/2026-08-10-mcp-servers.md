...
- The connect + discovery wait is bracketed by `mcp_connect_begin` / `mcp_connect_end` events carrying per-server transport/status/duration/tool-count and the total wall time (emitted only when servers are configured). The web chat shows a connecting banner (spinner, then a one-line result listing unavailable servers), the CLI prints paired `[mcp]` lines, and the Trace records the pair — as event rows and as a synthetic "mcp connect" span on the analysis timeline, attached to the Task that follows.
