...
- `session_meta` **drops its `tools` field**; the full schemas follow as a new `session_tools_ready` event once discovery completes, rewritten next to the meta on every post-compaction Trace file. **Explicitly incompatible with pre-split Traces** (deliberate, no compat code kept): their embedded tool record is no longer read or displayed — everything else about old Traces renders unchanged, and no action is required.
- The connect + discovery wait is bracketed by `mcp_connect_begin` / `mcp_connect_end` events carrying per-server transport/status/duration/tool-count (the phase's total wall time is the pair's timestamp difference) (emitted only when servers are configured; servers connect in parallel). The web chat shows a connecting banner (spinner, then a one-line result listing unavailable servers), the CLI prints paired `[mcp]` lines, and the Trace records the pair — as event rows and as an "mcp connect" span on the analysis timeline under its own "other" legend category (not a tool execution), attached to the Task that follows. The connect is abortable: stopping mid-connect closes the pair `aborted`, the run ends, and the connect completes in the background — the next send reuses it instead of reconnecting; continuing a conversation never reconnects (the engine survives across tasks).
