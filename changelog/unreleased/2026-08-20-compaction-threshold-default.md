# Compaction threshold default raised to 512000
...
Newly created Agents are seeded with `compaction.max_context_length: 512000` instead of
`128000`. What bounds compaction in practice is the model's own `context_window`: the
effective threshold is capped at `context_window − COMPACTION_HEADROOM` (2048) at every use,
so a larger configured number can never push compaction past a window it would overflow.
Raising the seed lets a big-window model use the context it actually has, while a
small-window model compacts exactly where it did before.
...
- With the new seed, a 32768-token window still fires compaction at 30720 and a
  200000-token window fires at 197952. Only a window larger than 512000 lets the configured
  threshold be the number that fires.
...
- Changing a built-in default advanced `KERNEL_VERSION` to generation `2026-08-20`, with
  ...
  generation's default advances to 512000; a value the user edited is kept and reported.
