...
- Web: the App Center page (`/apps`, between Agents and the plugin library in the nav and the collapsed rail): a search box, a status segment, rows with a kind glyph, a meta line and a status pill, Open / Restart / Stop, a menu with go-to-session / edit / unregister, a re-probe every 20 seconds, and the kit's two create buttons — "Create with AI" hands the description to the Project's default agent with the app-building tail and leaves the prompt prefilled in a new conversation for the user to send, while "Create manually" opens a form against one of the Project's recent Sessions. The chat folds an `[app_center]` block into a one-line "App Center: restart …" notice, and input history skips such messages.
