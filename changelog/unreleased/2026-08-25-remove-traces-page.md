...
- **Importing a Trace moved to System settings → General**, below the CLI-sessions filter — the one
  capability that lived nowhere else. Choose the receiving Agent and pick a `.jsonl` file; the pick
  is the confirmation, and the conversation list refreshes so the imported Session appears in the
  sidebar. The endpoint is per-Agent for the same reason the old dialog asked: a Trace file's own
  `session_meta` cannot name a local Agent, because its `agent_state` path belongs to the machine
  that exported it.
