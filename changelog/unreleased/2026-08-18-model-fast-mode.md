...
The model dialog gained a "Fast mode" toggle, default off, rendered only for models whose routed AgentHub client can serve it — `fastModeState` recomputes from the live form, so editing the id, protocol or base URL updates it as it is typed. It takes the same inline-switch shape as the vision toggle, with a hint line while ON (and as the label's hover title) saying it is faster output at premium pricing and that the recorded prices stay standard.
