...
- **配置只在设置的「插件」页里。** 插件的模块在路由旁声明自己的设置分组——`bot_token`（密钥）、`project`（Project id）、`agent`（缺省 `default_agent`）与 `enabled`——管理员在那里填写；插件自己读取并 watch 这个分组，保存即按新值重启机器人，无需重启服务端。机制见[插件配置条目](2026-09-07-plugin-configuration.zh.md)。凑不成机器人的值连同原因报在插件只读的状态路由上（`GET /api/discord-bot`，仅管理员，经 `HttpModule.routes` 槽贡献、自带一份 Hono）。没有环境变量，也没有要手改的文件。唯一的其他状态是聊天 → Session 表，存在服务端设置的 `discord-bot:chats:<projectId>` 下。
- 插件随每个构建一起发布、列入内置索引、默认不安装。
- **接口生成器现在认得跨包引用的接口。** 这是第一个 `@Use` 服务端自有接口的插件；构建出的 `.d.ts` 里抽象类还在，但标记它的 `@Interface()` 装饰器被擦除了。生成器改为从声明方自己生成的接口表里取这个标记，包外的插件因此可以引用它的接口。
