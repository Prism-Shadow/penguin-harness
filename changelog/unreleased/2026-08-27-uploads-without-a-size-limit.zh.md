...
- **请求体上限固定为 384MB**，不再由设置推导。它标记的是传输真正走不通的位置——请求体被整体缓冲并
  作为单个字符串 JSON 解析，而 V8 对字符串长度的上限约为 512MB——而不是一条关于「允许发多大」的策
  略；这个数字也高于此前那条推导上限所能达到的任何取值。
...
- 不再抛出 `413` 错误码 `file_too_large`（针对输入框附件）与 `image_too_large`；阈值超出范围时
