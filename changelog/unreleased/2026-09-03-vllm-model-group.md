...
models AgentHub's `vllm-openai-chat` client carries a per-model thinking switch for:
...
- adding a model to the group preselects `vllm-openai-chat` rather than the generic
...
`vllm-openai-chat` does not exist in `@prismshadow/agenthub` 0.4.9, the version this repo
depends on and the newest published release; it is on AgentHub's unreleased branch (agenthub
...
with AgentHub's `"vllm-openai-chat is not supported"`.
