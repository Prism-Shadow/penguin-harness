...
- 状态：`drafting` → `ready` → `approved` → `merged`，或 `rejected`；请求整改把 `ready` 退回 `drafting`。认可只覆盖一个修订：其后的 publish 把提案退回 `ready`，告诉实施者等新的认可，页面逐节显示自认可的修订以来的改动（`GET …/revisions[/:rev]` 可读回任一修订；`show` 打印 `approved: r<M>`）。
