...
- 一份提案有组织内的编号、作者（点名的员工，否则是发起的员工）、可选的实施者（缺省为作者自己，除非点名同事）、发起它的主体（`user:<id>` 或 `agent:<id>`），一份由 `<文件, 可选名称模式>` 对组成的 `scope`——文件路径只出现在这里——以及各节（改动 / 目的 / 一个测试例），每一段都是评论的锚点。发布修订时未变的段落保留 id，评论随之保留。正文里出现文件链接即被拒绝。
