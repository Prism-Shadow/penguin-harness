...
- 保存凭据不再跨 Session 冲突。任意多个 Session 都可以同时保存同一个飞书应用或 Telegram 机器
  人，各自持有自己的配置与自己记住的会话。两个 PUT 都去掉了各自的 `_in_use` 409，
  `MessagingBindingsRepo.upsert` 也不再有失败分支。
- 仍有一种保存会被拒绝，因为它实质上就是一次启用：把**已启用**的绑定改指到另一个 Session 已启
  用的账号上，返回同样的 409 `account_enabled_elsewhere`。这类保存会用新凭据重启连接器，若不做
  这道检查，两个 Session 就会在从未经过启用闸门的情况下连到同一个账号上。已停用的绑定不受此限
  ——那正是这套模型的用意所在。
- 已启用绑定的 Session 若已不存在（随 Project 或 Agent 一起删除，二者都不会顺带清理绑定），现在
  会在下一次有人申请该账号时就地清理，而不必等到下次服务端启动。拒绝信息不点名任何 Session，否
  则这样一条残留记录指向的将是一条谁也够不着的连接。
