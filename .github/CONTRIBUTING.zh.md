...
| [`packages/plugins`](../packages/plugins) | `@prismshadow/penguin-plugins` | 插件库 loader：依赖全部插件包并解析各自目录 |
| [`plugins/*`](../plugins) | `@prismshadow/penguin-plugin-<name>` | 内置插件，一插件一 npm 包：Skill（软件开发、模型开发、Agent 开发/调优……）与会话钩子（目标模式、技能沉淀） |
