 ...
 *     [goal]
 *     round: 2
 *     …automation preamble, GOAL.yaml path + rules + its current content, working audits…
 *     [/goal]
 *
 *     make all tests pass
 *
 ...
 *
 * The block embeds the GOAL.yaml the goal hook has just written — round, tokens used and
 * budget included, so the file is the one place the numbers live. The embedded `objective`
 * value is user data, which is why the closing tag is matched line-anchored (see
 * markers/goal-block.ts).
 */
...
import { serializeGoalFile } from "./goal-file.js";
import type { GoalFile } from "./goal-file.js";

export interface GoalPromptArgs {
  /** The file as just written for this round (its `round` is the block's first field line; the frontend's round hint). */
  goal: GoalFile;
 ...
 * Task; afterwards the goal hook reads the file's status to decide whether to continue.
 */
export function goalRoundMessage(args: GoalPromptArgs): string {
  const block = markerBlock(
    MARKER_TAGS.goal,
    [
      `round: ${args.goal.round}`,
...
/**
 * The user message of the final wrap-up round after the budget is exhausted: the goal hook
 * ends the goal as `budget_limited` when this round ends (unless the model can truthfully
 * complete it).
 */
