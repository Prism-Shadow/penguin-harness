...
const TOOL_SEARCH_DEFINITION = {
  ...
  description:
    "Search hidden configured tools. Catalog text is usually English; translate non-English " +
    "requests into concise English capability or service keywords. Results provide tool_ref and " +
    "input_schema for call_tool. On no match, retry once in the catalog language with different " +
    "capability terms, then stop.",
  parameters: {
    type: "object",
    properties: {
      query: {
        type: "string",
        description:
          "Concise capability, service, tool, or parameter keywords in the catalog language (usually English).",
      },
      limit: {
        type: "integer",
        minimum: 1,
...
export interface McpToolProviderOptions {
  ...
  /** Auto-mode MCP Schema threshold; invalid values fall back to the default. */
...
/** Prevents one noisy Server from turning list-changed notifications into a tight request loop. */
const MIN_CATALOG_REFRESH_INTERVAL_MS = 500;

/** Search/dispatch never waits indefinitely for every connected Server's catalog to go quiet. */
const CATALOG_REFRESH_WAIT_BUDGET_MS = 1_500;

async function waitForPromisesWithin(
...
export class McpToolProvider {
  ...
  private readonly toolExposureThresholdTokens: number;
  ...
  constructor(entries: MCPServerConfig[], options?: McpToolProviderOptions) {
    ...
    const configuredThreshold = options?.toolExposureThresholdTokens;
    this.toolExposureThresholdTokens =
    ...
    this.searchTool = {
      ...
      execute: async function* (
        ...
        const catalogFresh = await provider.awaitCatalogRefresh();
        ...
        const result = {
          query,
          ...(!catalogFresh ? { catalog_refresh_pending: true } : {}),
          matches: matches.map(({ ref, sourceId, definition, tool }) => ({
            tool_ref: ref,
          ...
          note: !catalogFresh
            ? "A catalog refresh is still pending. These matches come from the last complete snapshot; search again before dispatching."
            : matches.length > 0
              ? "Call call_tool with a returned tool_ref, tool_name, and schema-valid arguments."
              : "No matching tools were found. If the query language differs from the catalog, retry once with concise catalog-language capability terms; otherwise do not retry without new information.",
        };
        yield partialToolCallOutput({
          eventType: "delta",
  ...
  toolPermission(name: string, rawArguments?: string): ToolPermission | undefined {
    ...
    if (this.usesGateway() && name === TOOL_CALL_NAME) {
      ...
      if (this.catalogRefreshPending(target.sourceId)) return undefined;
  ...
  /** Resolves a gateway approval label from the private binding, never from model display data. */
  toolApprovalTarget(name: string, rawArguments?: string): ToolApprovalTarget | undefined {
    if (this.usesGateway() && name === TOOL_CALL_NAME) {
      ...
      return {
        ...
        ...(!this.catalogRefreshPending(target.sourceId)
          ? { permission: target.tool.definition.permission ?? "rw" }
          : {}),
      };
    }
    if (this.usesGateway()) return undefined;
    const def = this.byName.get(name)?.definition;
    return def ? { name: def.name, permission: def.permission ?? "rw" } : undefined;
  }
  ...
  async resolveDispatch(
    ...
    const sourceId =
      this.byRef.get(parsed.toolRef)?.sourceId ?? this.staleByRef.get(parsed.toolRef)?.sourceId;
    if (!(await this.awaitCatalogRefresh(sourceId))) {
      return dispatchFailure(
        "catalog_refresh_pending",
        "The target tool catalog is still refreshing. Search again before dispatching so the tool contract and permission are current.",
        { retryable: true },
      );
    }
  ...
  private async connectAll(): Promise<void> {
    ...
    if (this.exposure === "auto") {
      ...
      this.autoUsesGateway = estimatedSchemaTokens >= this.toolExposureThresholdTokens;
  ...
  /** Waits within a fixed budget for a complete catalog snapshot; false means callers must stay conservative. */
  private async awaitCatalogRefresh(sourceId?: string): Promise<boolean> {
    if (!this.usesGateway()) return true;
    const relevant = this.connections.filter(
      (conn) => sourceId === undefined || sourceId === `mcp:${conn.server.name}`,
    );
    const deadline = performance.now() + CATALOG_REFRESH_WAIT_BUDGET_MS;
    for (;;) {
      for (const conn of relevant) {
        if ((conn.catalogDirty || conn.refreshRequested) && conn.refreshPromise === null) {
          this.queueCatalogRefresh(conn);
        }
      }
      if (!this.catalogRefreshPending(sourceId)) return true;
      const pending = relevant.flatMap((conn) =>
        conn.refreshPromise ? [conn.refreshPromise] : [],
      );
      if (pending.length === 0) return false;
      const remainingMs = deadline - performance.now();
      if (remainingMs <= 0) return false;
      if (!(await waitForPromisesWithin(pending, remainingMs))) return false;
    }
  }

  private catalogRefreshPending(sourceId?: string): boolean {
    return this.connections.some(
      (conn) =>
        (sourceId === undefined || sourceId === `mcp:${conn.server.name}`) &&
        (conn.catalogDirty || conn.refreshRequested || conn.refreshPromise !== null),
    );
  }

  /** Coalesces notification bursts into rate-limited per-connection refresh passes. */
  private queueCatalogRefresh(conn: McpConnection): void {
    conn.catalogDirty = true;
    conn.refreshRequested = true;
    if (!this.usesGateway() || this.closed) return;
    if (!conn.catalogReady || conn.refreshPromise) return;
    conn.refreshPromise = this.refreshCatalogOnce(conn).finally(() => {
      conn.refreshPromise = null;
      if (conn.refreshRequested && !this.closed) this.queueCatalogRefresh(conn);
    });
  }

  private async refreshCatalogOnce(conn: McpConnection): Promise<void> {
    const waitMs = Math.max(
      0,
      conn.lastRefreshStartedAt + MIN_CATALOG_REFRESH_INTERVAL_MS - Date.now(),
    );
    if (waitMs > 0) await new Promise((resolve) => setTimeout(resolve, waitMs));
    if (this.closed || !this.usesGateway()) return;

    // Notifications received during the cooldown are represented by this refresh. A notification
    // received after the request starts sets the flag again and schedules exactly one later pass.
    conn.refreshRequested = false;
    conn.lastRefreshStartedAt = Date.now();
    try {
      const listed = await conn.client.listTools(undefined, {
        timeout: conn.server.connectTimeoutMs,
      });
      if (!this.closed) {
        this.applyGatewayCatalog(conn, listed.tools);
        // A notification received during listTools belongs to a later revision; only a pass
        // with no concurrent notification establishes a current snapshot.
        if (!conn.refreshRequested) conn.catalogDirty = false;
      }
    } catch (err) {
      if (!this.closed) {
        this.warn(
