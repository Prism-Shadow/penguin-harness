...
/** Splits a letter/number run when it crosses between Han and non-Han scripts. */
function splitOnHanBoundary(value: string): string[] {
  const parts: string[] = [];
  let current = "";
  let currentIsHan: boolean | null = null;
  for (const char of value) {
    const isHan = HAN_CHARACTER.test(char);
    if (current !== "" && isHan !== currentIsHan) {
      parts.push(current);
      current = "";
    }
    current += char;
...
function terms(value: string): string[] {
  ...
  const runs =
    normalized(value)
      .replace(/[_-]+/g, " ")
      .match(/[\p{L}\p{N}]+/gu) ?? [];
  return runs.flatMap(splitOnHanBoundary).flatMap((term) => {
    const chars = [...term];
    if (!chars.every((char) => HAN_CHARACTER.test(char)) || chars.length < 2) return [term];
    return [term, ...chars.slice(0, -1).map((char, index) => char + chars[index + 1])];
  });
}
...
export function searchToolCatalog<T extends ToolCatalogEntry<unknown>>(
  ...
  const queryTerms = [
    ...new Set(
      terms(phrase).filter((term) => {
        const chars = [...term];
        const minimumLength = chars.every((char) => HAN_CHARACTER.test(char)) ? 2 : 3;
        return (
          chars.length >= minimumLength && !/^\p{N}+$/u.test(term) && !GENERIC_QUERY_TERMS.has(term)
        );
      }),
    ),
  ];
