 ...
 * generated by the installer from plugin.json: identity and display fields, one script list
 * per hook point (`stop`, `pre_tool_use`, `user_prompt`), and the switch that turns the
 * package off without uninstalling it. The plugin's icon is not part of it: the installer
 * writes `icon.svg` beside the manifest instead.
 */
export interface HookManifest {
  ...
  /** `false` switches the package off: it stays installed but no Session consults it. The installer never writes the field — an absent one means enabled (hookPackageEnabled is the one reader). */
  enabled?: boolean;
}

/** Whether a Session consults an installed hook package: only an explicit `enabled: false` switches it off; an absent field means enabled. */
export function hookPackageEnabled(manifest: Pick<HookManifest, "enabled">): boolean {
