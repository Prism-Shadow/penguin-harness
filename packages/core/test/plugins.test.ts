...
describe("comparePluginVersions", () => {
  it("orders by date, then by sequence number numerically; non-versions sort before every version", () => {
    expect(comparePluginVersions("v2026.08.29.1", "v2026.08.29.1")).toBe(0);
    expect(comparePluginVersions("v2026.08.29.2", "v2026.08.29.10")).toBeLessThan(0);
    expect(comparePluginVersions("v2026.09.01.1", "v2026.08.29.9")).toBeGreaterThan(0);
    expect(comparePluginVersions("", "v2026.08.29.1")).toBeLessThan(0);
    expect(comparePluginVersions("7", "v2026.08.29.1")).toBeLessThan(0);
    expect(comparePluginVersions("", "")).toBe(0);
  });

  it("reads the legacy spelling as the same version a copy installed earlier carries", () => {
    expect(comparePluginVersions("2026-08-29.1", "v2026.08.29.1")).toBe(0);
    expect(comparePluginVersions("v2026.09.10.2", "v2026.09.10.1")).toBeGreaterThan(0);
    expect(comparePluginVersions("v2026.09.10.1", "2026-09-09.9")).toBeGreaterThan(0);
    expect(comparePluginVersions("2026-09-09.9", "v2026.09.09.10")).toBeLessThan(0);
  });
});
