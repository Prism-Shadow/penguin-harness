...
describe("context compaction", () => {
  ...
  it("records the opener publishes are yielded live and written at the rotated file's head after the meta; a context that publishes none keeps the previous records", async () => {
    ...
    const engine = new ContextEngine({
      ...
      bootstrapRecords: [],
      compaction: settings(),
      createContext: async (_tokens, { emit }) => {
        opened += 1;
  ...
  it("mid-task: abandonment with a trailing tool-calling rejection stashes the repair for the next run", async () => {
    ...
    const engine = new ContextEngine({
      ...
      environment: fakeEnvironment,
      compaction: settings(),
      createContext: () => ({ llm: new ScriptedLLM([], "llm2") }),
      compactionMaxReconnects: 4,
      reconnectBackoffMs: 1,
    });
  ...
  it("no compaction capability (createContext missing) means thresholds never fire and compact() is a no-op", async () => {
    const llm1 = new ScriptedLLM(
  ...
  it('compactability(): reports each specific "cannot compact" reason instead of a blanket false', async () => {
    ...
    // The compaction request goes to the **current** LLM (the script's second entry); createContext
    ...
    // (1) No compaction capability configured (no compaction / createContext).
  ...
  it("a resumed engine restores the turn count and the just-compacted reason", async () => {
    ...
    const withTurns = new ContextEngine({
      ...
      environment: fakeEnvironment,
      compaction: settings(),
      createContext: () => ({ llm: new ScriptedLLM([]) }),
  ...
  it("a resumed Session compacts on request instead of silently doing nothing", async () => {
    ...
    const session = new Session({
      ...
      createContext: () => ({ llm: newContextLLM }),
      compaction: settings(),
