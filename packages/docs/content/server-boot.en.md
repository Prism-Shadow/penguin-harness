...
```
A plugin is a set of modules — the unit the harness itself is built from, written the same way: `@Component` / `@Module` classes with `@Use` / `@Provide` / `@Bind` fields. Its manifests are generated, not written — the package's build runs `gen-ifaces` over its own tsconfig and ships the `ifaces.json` beside its `package.json` (marked `"penguin": {}`); the default export is `{ modules: [<class>, …] }`, each class checked against its manifest in that table at load. Split by frequency:

| Moment                | Frequency        | What happens                                                                                                                                                       |
| --------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Load                  | Once per process | Boot step ④ `loadPlugins`: resolve and import each specifier in `plugins.json`, read the `ifaces.json` beside its `package.json`, check each class the default export names against its manifest there. A package without the table (never built), or one whose class and table disagree (a stale build), is skipped with its reason; an unreadable or malformed `plugins.json` fails the boot |
...
| Plugin host          | built by ④ `loadPlugins`, published to the registry in ⑤ | a `package.json` marked `penguin`, its generated `ifaces.json`, a default export `{ modules: [<class>, …] }`; the configuration surface is `<root>/plugins.json` |
