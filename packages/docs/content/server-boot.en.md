...
```
A plugin is a set of modules — the unit the harness itself is built from, written the same way: `@Component` / `@Module` classes with `@Use` / `@Provide` / `@Bind` fields. Its manifests are generated, not written — the package's build runs `gen-ifaces` over its own tsconfig and ships the `ifaces.json` beside its `package.json` — the table is the package's module payload, and a plugin is a plugin by being listed; the default export is `{ modules: [<class>, …] }`, each class checked against its manifest in that table at load. Split by frequency:
...
| Plugin host          | built by ④ `loadPlugins`, published to the registry in ⑤ | an npm package: its generated `ifaces.json` (the module payload), a default export `{ modules: [<class>, …] }`; the configuration surface is `<root>/plugins.json` |
