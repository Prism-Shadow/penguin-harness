...
The extension contract (`Extension` / `ExtensionContext` / `ExtensionEvents` / `PenguinInterface` / `PenguinContext`, and the sandbox vocabulary a backend is written against) is declared in the SDK, at `@prismshadow/penguin-core/extension`. `PenguinContext` and `PenguinInterface` are closed: they name every member an extension may rely on, and are not reopened by declaration merging. `terminals` cannot be among them — it is a harness class — so it is named on `HarnessContext`, which extends the contract and is the whole of `@prismshadow/penguin-server/extension`; an extension reading it writes `ctx as HarnessContext` and thereby states where it is used that it depends on this embedder. An extension that needs no harness-only member names core alone: each sandbox backend in this repo does, and none depends on the server package. Both subpaths emit types only. Which extensions exist is the deployment's `<root>/extensions.json`; the harness itself imports no extension.
