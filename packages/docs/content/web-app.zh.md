...
- 「+」菜单收纳输入附加项：**上传图片**、**上传文件**与目标模式。附件不限类型（可多选，单个 ≤ 10MB），已选文件以可移除的小卡片显示在文本框上方；只带附件、没有正文也可发送。发送时文件写入该 Session 的 scratchpad（随 Session 一并删除），消息里每个文件追加一行 `[attached file: <path>]`，对话中渲染为一条「附加文件」提示：文件内容不进入对话，模型用普通文件工具按路径读取；
