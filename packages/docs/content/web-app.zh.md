...
- 筛选条件：Agent、模型、日期范围（近 7 / 30 / 90 天快捷选项或自定义起止）与时间精度（按小时 / 天 / 周 / 月，可选项随范围变化）；
- 概览卡片：今日 / 近 7 天 / 累计用量；
- 图表：全部按所选范围与精度展示的时间序列——各 Agent 调用次数（平滑折线，颜色区分 Agent）、请求成功率（0–100% 平滑折线）、Token 变化（三段堆叠柱 + 前置的缓存命中率曲线）、成本变化（平滑折线 + 面积）；图表自适应卡片宽度，不出现滚动条；
