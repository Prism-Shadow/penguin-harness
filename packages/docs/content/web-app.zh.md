...
- 「+」菜单收纳输入附加项：**上传图片**、**上传文件**与目标模式。附件不限类型（一次最多 20 个，默认单个 ≤ 100MB、合计 ≤ 120MB——两个尺寸均可由管理员在用户菜单的**上传限制**中调整，见下文；超限的文件在读取前即被拒绝，不会先上传再报错，提示中会写出当前生效的上限），已选文件按选择顺序以可移除的小卡片显示在文本框上方；只带附件、没有正文也可发送。发送时文件写入该 Session 的 scratchpad（随 Session 一并删除），消息里每个文件追加一行 `[attached file: <path>]`，对话中渲染为一条「附加文件」提示：文件内容不进入对话，模型用普通文件工具按路径读取；
...
- 筛选条件：Agent、模型、日期范围（最近一小时 / 最近一天滑动窗口、近 7 / 30 / 90 天、或自定义起止）与时间精度（按分钟 / 小时 / 天 / 周 / 月，可选项随档位变化——最近一小时以分钟桶呈现）；
- 概览卡片：今日 / 近 7 天 / 累计用量；
- 图表，全部按所选范围与精度展示的时间序列：并排的「按 Agent 拆分」与「按模型拆分」的请求与成功率图（单个时间桶的请求数按对象分段堆叠、走左轴，每个对象的成功率各自成一条虚线、走右侧 0–100% 轴；请求数最高的若干对象具名呈现，其余折叠为一条标注了数量的中性色序列，某个桶内没有计入成功率的请求时折线断开）、Token 变化（三段堆叠柱 + 前置的缓存命中率虚线曲线——连续绘制，无缓存流量的桶记 0）、成本变化（折线带点 + 面积）。图表自适应卡片宽度，不出现滚动条；空桶显示 0 而非「—」；
...
仅管理员可见：侧边栏用户菜单的「上传限制」打开一个服务器全局的表单，可调整两个尺寸——**单个附件上限**与**单条消息附件合计上限**，均以整数 MB 填写，默认 100MB 与 120MB。两者的取值范围为 1–200MB，且合计不得低于单个上限（否则一个本身合法的附件将无法发送）；超出范围的值——例如把「100GB」按 MB 填成 102400——会被服务端明确拒绝并在输入框下方给出原因，不会被接受。校验先于写入：一次 PUT 中只要有一个字段非法，其余字段也不会落库。
