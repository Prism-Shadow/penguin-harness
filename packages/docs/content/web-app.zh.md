...
- 有两种情况图画不出来。**没有比率**的桶——该对象在这个桶里一次请求都没发，或该桶没有缓存流量——折线沿坐标轴顶部横穿过去，只为让线保持连续；悬停表格在该处显示「—」而非百分比，而真正的 0%（发了请求且全部失败）仍然显示 0%。**什么都没记录**的桶会被 Token 与成本两张图直接丢弃：它们只画有值的区间、从左往右依次排开，因此横轴会跳过时间——跳过处在坐标轴上画有断轴标记，图下方另有一行说明跳过了多少个区间、并提示此横轴与请求图表的横轴不一致（后者保留范围内的每一个桶）；
