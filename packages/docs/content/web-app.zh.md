...
- 需要人工审批时，工具调用在消息流中内联显示“允许 / 拒绝”按钮；修改审批模式会更新当前用户的所有已打开客户端，不影响其他用户；
