...
| Memory | Workspace 记忆：Agent 级开关与统一索引 `memory/AGENTS.md`，其下是 Workspace 选择器与该 Workspace 的主题文件列表，以及 Markdown 编辑器 |
...
该页自上而下是逐层收窄的作用域，与记忆的存储方式一致：开关与统一索引属于 Agent 级，位于选择器之上；选择器以下的内容都属于单个 Workspace；打开索引会定位到所选 Workspace 的分组标题。它写入的就是 Agent 自己维护的那批文件。重命名或删除主题文件时，索引中指向它的链接会一并改写或清除，索引不会列出已不存在的文件。关闭记忆不会删除任何文件，该页仍可正常管理——只是记忆不再进入 Agent 上下文，也不再为新 Session 准备目录。
