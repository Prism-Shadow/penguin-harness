...
- 子 Agent 在消息流中只留下一条通栏行，样式与其他折叠步骤行一致（头像、名称、短会话号、运行中转圈，子会话内有待审批时带琥珀色圆点）；点击打开右侧智能体面板——上方是该行所属 Task 派生关系的调用图（节点按运行时权威状态显示运行中 / 停止中 / 已完成与耗时，点节点切换），下方是所选子会话的实时对话（含子会话自己的用户消息），嵌套工具卡片与审批与主对话完全一致。选中的子 Agent 带停止按钮与紧凑输入框：运行中发送是纠偏，空闲时发送是在原上下文中继续；停止只结束当前轮、保留上下文以便之后继续，而停止主 Task 也会级联停止仍在运行的子 Agent。面板的显示以任务为界：发送消息开启新任务时默认关闭（进入会话时同样默认关闭），仅在手动打开、或（桌面端）当前任务派生子智能体时重新显示（每个任务自动打开一次，任务内手动关闭后保持关闭到下个任务，且不抢占已打开的工作区面板）；从工具条打开面板或切换 Session 时默认展示最新 Task 的调用图，点旧轮次的行则回看那一轮的调用图；智能体面板与工作区面板不会同时显示，打开一个即关闭另一个；上下文压缩与首次 MCP 连接以统一样式的步骤行提示（与运行分组头同款外壳，含吸顶标题栏，连接中/成功/失败仅切换图标与单行详情，结果行含耗时；MCP 行主显发现的工具数并点名不可用的 Server，展开后按 Server 分组——每组一行状态、工具数与该 Server 的连接耗时，点开组内是工具列表或失败原因）；
