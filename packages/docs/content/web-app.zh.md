...
- 概览卡片：今日 / 近 7 天 / 累计用量；
- 图表，全部按所选范围与精度展示的时间序列：整行宽的请求与成功率组合图（请求柱在下走左轴，成功率折线走右侧 0–100% 轴、线末带圆点；「按 Agent / 按模型」切换加对象下拉，可在全部对象堆叠视图与单个对象的柱与成功率之间切换）、Token 变化（三段堆叠柱 + 前置的缓存命中率虚线曲线——连续绘制，无缓存流量的桶记 0）、成本变化（折线带点 + 面积）。图表自适应卡片宽度，不出现滚动条；空桶显示 0 而非「—」；
