...
- 每个 Task 结束后显示统计行：Token 用量、TPS、耗时、费用、复制以及**分叉**操作。分叉会在该条已完成的模型回复处新建 Session，沿用 Agent、模型、Workspace、审批模式和保留范围内的历史；新旧会话的 Trace 与 scratchpad 是独立快照，即使删除源对话，本地图片和附件仍可显示；
