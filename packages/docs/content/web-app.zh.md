...
侧边栏用户菜单会显示当前运行版本及「最近更新日期 7 月 26 日」样式的本地化日期——取发布版的构建日期；更早的安装（v0.1.2 及以前）没有构建日期，则由新版本查询解析其 Release 发布日期；新对话页在品牌下方也有同样的版本行。菜单首次打开后会向 GitHub 查询是否有新版本：有新版本时用户按钮上会出现提示圆点，菜单内提供更新说明链接；管理员还可点击"立即更新"，在服务器上执行 `penguin update`（数据目录不受影响）。更新完成后需要重启服务才会生效。设置 `PENGUIN_UPDATE_CHECK=off` 可完全关闭新版本检查——见[配置参考](/configuration)。
