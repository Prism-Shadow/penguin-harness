...
- 概览卡片：今日 / 近 7 天 / 累计用量；
- 图表，全部按所选范围展示的时间序列：并排的「按 Agent 拆分」与「按模型拆分」的请求与成功率图（单个时间桶的请求数按对象分段堆叠、走左轴，每个对象的成功率各自成一条虚线、走右侧 0–100% 轴并绘制在柱子上方；请求数最高的若干对象具名呈现，其余折叠为一条标注了数量的中性色序列，某个桶内没有可计入成功率的请求时该点记 0，悬停文案会与「确实全失败」区分开）、Token 变化（三段堆叠柱 + 前置的缓存命中率虚线——连续绘制，无缓存流量的桶记 0）、成本变化（折线带点 + 面积）。每张图都支持同样的悬停：悬停整列看气泡、悬停单段柱突出该段、悬停折线突出该线。所有折线都是直连线段，不做平滑。图表自适应卡片宽度，不出现滚动条；空桶显示 0 而非「—」；
