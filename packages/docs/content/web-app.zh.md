...
侧边栏用户菜单会显示当前运行版本及「最近更新日期 7 月 26 日」样式的本地化日期——即当前运行版本的发布日期，由发布流程在构建时打入、直接显示、无需联网（开发构建以及打入机制之前的发布版，v0.1.2 及以前，只显示版本号）；新对话页在品牌下方也有同样的版本行。菜单首次打开后会向 GitHub 查询是否有新版本：有新版本时用户按钮上会出现提示圆点，两处版本行的末尾会出现「有新版本可用」上标小徽标（管理员点击进入更新对话框，其他用户跳转 Release 页面），菜单内提供更新说明链接；管理员还可点击"立即更新"，在服务器上执行 `penguin update`（数据目录不受影响）。更新完成后需要重启服务才会生效。菜单内还提供手动「检查更新」按钮：点击立即向 GitHub 查询（绕过缓存结果），已是最新时以提示告知。设置 `PENGUIN_UPDATE_CHECK=off` 可完全关闭新版本检查——见[配置参考](/configuration)。
