...
把既不属于 Project 也不属于 Agent 的设置收在一页，左侧子导航切换子页面。入口是侧边栏用户菜单的「系统设置」，每个子页面都可直接寻址（`/settings/proxy`）。

子导航按组划分。**个人**组存放属于当前账号的偏好，改动即刻生效，没有保存按钮，也就不存在离开页面丢失草稿的问题；**服务器**组存放经 `GET|PUT /api/admin/settings` 写入的服务器全局设置，仅管理员可见：非管理员既看不到该组、也看不到其中的条目，直接输入其中某个子页面的路径只会落到自己的第一个子页面，不会因此得知这些设置的存在；无论前端渲染成什么，该接口对非管理员的两个动词一律返回 403。

### 通用

个人设置。**显示 CLI 会话**（默认关）：关闭时会话列表只列出在 Web 端创建的 Session，直接由数据库返回；开启后会额外扫描轨迹目录，把 CLI 创建的会话一并列出。按用户存储，轨迹观测页遵循同一偏好。
