...
- 也可以把文件**拖拽到窗口**上传：文件拖到页面上方时全窗显示「松开以添加附件」浮层（拖离窗口即消失，拖动文本等非文件内容不触发），松开即把整批文件附加到输入框——图片走粘贴图片的同一管线，其余文件成为附件，类型与大小校验及报错与「+」菜单一致。任务运行中同样可以拖入（附件按下述运行中发送方式送出）；目标模式下拖入的图片照常附加、文件会以提示拒绝。在应用的其他位置放下文件则被直接忽略，不会让浏览器跳转打开该文件；
