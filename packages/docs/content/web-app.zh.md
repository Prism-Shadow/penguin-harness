...
- Task 运行期间输入框保持可用，工具条只保留一个操作按钮：输入框为空时是**停止**，一旦输入内容即变为**发送**，其行为遵循工具条「更多设置」弹出分组中的**运行中发送方式**（一个可扩展的设置面板，草稿态同样可设，选择会被记忆）：**插话**（默认）把文字**连同已附加的图片**以 `[user_steering]` 用户消息随下一轮送达运行中的 Agent（一张没有配文的图片也能单独发出，送达后渲染为一枚内嵌图片的插话气泡）；**排队** 把整条消息暂存在服务端，本轮结束后自动作为普通新消息发出（期间在输入框附近显示「N 条已排队」提示；队列存放在服务端，刷新页面不丢失）。插话确实带不走的草稿——只勾了技能而没有文字和图片，或带着 `@` 交接目标——该次发送回落到排队通道，按钮因此不会在顶掉「停止」之后又变成死键；
- 输入 `@` 提及其他 Agent，将会话交接给它；
- 「+」菜单里的**目标模式**把草稿变成一个由系统循环推进的目标。附加的图片可以一起发出，且一律以 scratchpad 路径的形式送达（目标每轮都作为文本重新注入，见[目标模式](/docs/goal-mode)）；文字仍是必需的——一张图说明不了目标。第一轮气泡下方完整展示附图，后续轮次收成一枚 chip，点击展开；
