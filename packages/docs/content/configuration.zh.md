...
| `compaction.max_context_length` | `512000` | 触发压缩的上下文 Token 阈值；生效阈值不超过模型 `context_window` − 2048，因此除非窗口比它还大，决定何时压缩的是模型窗口而非该数值（条目未配置 `context_window` 时按 128000 的假定窗口计） |
