...
The service runs at http://127.0.0.1:7364 and opens your browser (`--no-open` to skip). The account is `admin`, and it has no password yet: the server prints a one-time first-login link in a framed notice, which opens the browser signed in so you can set one. The link is reprinted on every start until you do. `penguin server` starts the same process headless.
