...
| GET | /api/admin/sandbox | The sandbox settings and what can enforce them: `{settings: {mode, network?, maskPaths?}, backends: [{name, dimensions}]}` — answered once the backends have loaded, so an empty list means none is mounted |
| PUT | /api/admin/sandbox | Replace the sandbox settings: `{mode, network?, maskPaths?}` — `mode` one of `read-only` / `workspace-write` / `danger-full-access`, `network` `"none"` or null, `maskPaths` trimmed and de-duplicated (at most 64); 400 refuses the whole body. Stored in the server settings and applied to the next command spawn |
