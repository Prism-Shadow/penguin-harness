...
| GET | /api/projects/:projectId/models/keys/health | Process-local masked key-health registry. With `?provider=…&modelId=…` (or `?modelRef=provider/model`) returns one `{modelRef,totalKeys,healthyCount,cooldownCount,evictedCount,keys}` report; with no selector returns `{reports}` for configured models |
...
Both key-health routes require Project membership. They expose only masked keys (`****` for values of eight characters or fewer, otherwise the first three and last four characters). The registry is in memory, is recreated after server restart, and is separate from the rotators owned by live Session runtimes. Reset clears the registry's cooldown/eviction state, counters, last-used times, and rotation position; it neither changes the Project config nor rebuilds or resets live runtimes.
