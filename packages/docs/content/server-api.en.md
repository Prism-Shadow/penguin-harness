...
| GET | /api/admin/plugin-config | Every settings entry: first the groups modules contribute (the sandbox, then each mounted sandbox backend's own options), then every loaded plugin that declares options (`package.json#penguin.configuration`): `{plugins: [{name, configuration, values, parent?, notices?}]}` — the schema, the stored values merged onto its defaults (secrets masked), the entry whose card it is drawn inside, and live status lines. Field types: `string`, `secret`, `boolean`, `number`, `project`, `enum` (with `options`), `list` (lines, optional `maxItems`) |
| PUT | /api/admin/plugin-config | Save one package's options: `{name, values}` — fields the request omits keep their value, `null` or `""` clears one, a secret sent back as its mask keeps the stored one; 400 `plugin_config_invalid` names the refused field, 404 `plugin_config_unknown` for a name no entry answers to. The owner picks the change up through its watch, no restart — a sandbox save applies to the next command spawn |
