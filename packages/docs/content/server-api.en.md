...
| POST | /agents/:agentId/sessions | Create a Session: `{modelId?, provider?, workspace?, approvalMode?, client?, surface?}` → 201. `client` is the creating-client hint stored on the row (`"cli"` from the CLI; default `"web"`) — informational provenance, never a list filter. `surface` creates a **surface Session** of that kind (one a plugin contributes, see `GET /api/contributions`; 400 `unknown_surface` otherwise): no model reference, drawn by the surface instead of the conversation, open it with the route below |
