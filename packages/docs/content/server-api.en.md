...
| POST | /steer | Mid-run steering: `{text, images?, files?}` queues a message for the running Task (delivered between turns as a standalone `[user_steering]` user message; uploaded files use the same scratchpad-backed `{fileName, dataUrl}` shape and limits as Task file parts) → 202; any part can carry the message on its own, but an empty request is a 400; 409 `not_running` when no Task is in progress |
