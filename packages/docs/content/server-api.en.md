...
| PUT | /messaging/telegram | Save the credential: `{botToken?, clearBotToken?}` — the whole credential is the one `<bot id>:<secret>` token from @BotFather (omitted/blank keeps the stored one; 400 `telegram_token_invalid` when the numeric id cannot be read; the clear flag works like Feishu's and the cleared config keeps its bot identity). Same save/enable split, and the same absence of a cross-Session conflict on save |
