...
| POST | /api/projects/:projectId/suggest-id | A semantic id proposed for the display name of an object being created: `{name, kind, taken?}`, `kind` being `project`, `agent`, `benchmark`, `org` or `channel` and `taken` the ids the proposal must avoid → `{id, source, reason?}`. The default model of the Project in the path translates the name into one English id in the kind's spelling (`source: model`) — the New Project dialog borrows the Project it was opened from — and is asked a second time with the format spelled out when the first answer yields no id; an ASCII slug of the name answers when no model is configured or neither answer is usable (`source: fallback`); a name neither path can name gets a dated placeholder (`source: placeholder`) with a `reason` of `no_default_model`, `model_failed`, `unusable_answer` or `no_ascii`. The route never fails for a name it cannot translate — a dialog that asked for an id always gets one. Kinds differ only in the id's shape, the ids the server avoids on its own, and who may ask: `project` is snake_case for the admin and `<username>-<suffix>` for anyone else, avoiding every Project id on the server; `agent` is snake_case, avoiding the Project's Agents; `benchmark` is kebab-case, avoiding the Project's Benchmarks, owner only; `org` and `channel` carry a `co_` / `ch_` prefix, never doubled when the model's answer already has it, and answer 404 `company_mode_off` while company mode is off. Every other caller must be a member of the Project. The ids the server avoids on its own are every name the kind's create route refuses as taken — a leftover folder no list shows, such as a Benchmark directory with no config, included — and never reach the prompt, only a `_2` / `-2` suffix on a collision; an id the kind's rule rejects for starting with a digit or being one character long gets the kind's name in front (`agent_3d_viewer`); the placeholders are `project_<yyyymmdd>`, `agent_<yyyymmdd>`, `benchmark-<yyyymmdd>`, `co_org_<yyyymmdd>` and `ch_channel_<yyyymmdd>` (a non-admin's Project with the username in front). Every model dead end is recorded as an `id_suggest_failed` error, under the source `organization` for `org` / `channel` and `id_suggest` for the rest; the completion runs with thinking off and the shared meta budget, belongs to no Session and is not metered |
