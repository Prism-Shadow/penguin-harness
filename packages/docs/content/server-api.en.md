...
| POST | /api/projects/:projectId/suggest-id | A semantic id proposed for the display name of an object being created: `{name, kind, taken?}`, `kind` being `project`, `agent`, `benchmark`, `org` or `channel` → `{id, source, reason?}`. The three steps (model → ASCII slug → dated placeholder) and the reason codes are those of the organizations' `/suggest-id` below, asked of the default model of the Project in the path — the New Project dialog borrows the Project it was opened from. Kinds differ only in the id's shape, the ids the server avoids on its own, and who may ask: `project` is snake_case for the admin and `<username>-<suffix>` for anyone else, avoiding every Project id on the server; `agent` is snake_case, avoiding the Project's Agents; `benchmark` is kebab-case, avoiding the Project's Benchmarks, owner only; `org` and `channel` go to the organization service and answer exactly what the organizations' `/suggest-id` answers, 404 `company_mode_off` while company mode is off. Every other caller must be a member of the Project. The ids the server avoids on its own never reach the prompt, only a `_2` / `-2` suffix on a collision; an id the kind's rule rejects for starting with a digit or being one character long gets the kind's name in front (`agent_3d_viewer`); the placeholders are `project_<yyyymmdd>`, `agent_<yyyymmdd>` and `benchmark-<yyyymmdd>` (a non-admin's Project with the username in front). Every model dead end of a Project, Agent or Benchmark proposal is recorded as an `id_suggest` / `id_suggest_failed` error; the completion belongs to no Session and is not metered here either |
