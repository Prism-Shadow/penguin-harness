...
| GET | /api/projects/:projectId/models/keys/health | Process-local masked key-health registry. With `?provider=…&modelId=…` (or `?modelRef=provider/model`) returns one `{modelRef,totalKeys,healthyCount,cooldownCount,evictedCount,keys}` report with per-key `activeLeases`; with no selector returns `{reports}` for configured models |
| POST | /api/projects/:projectId/models/keys/reset | Reset one registry entry with `{provider,modelId}` or `{modelRef}`; returns `{ok:true,report}`. With no selector it is a no-op returning `{ok:true}` |
...
Both key-health routes require Project membership. They expose only masked keys (`****` for values of eight characters or fewer, otherwise the first three and last four characters) alongside active concurrency leases (`activeLeases`). The registry is in memory, is recreated after server restart, and is separate from the rotators owned by live Session runtimes. Reset clears the registry's cooldown/eviction state, counters, last-used times, active leases, and rotation position; it neither changes the Project config nor rebuilds or resets live runtimes.
...
| POST | /subagents/:childSessionId/resume | Resume an interrupted or failed subagent child run: `{text?}` (optional continuation guidance) → `{outcome: "steered" \| "continued" \| "revived"}`; 404 `subagent_gone` when the child session no longer exists and could not be revived, 409 `subagent_busy` when currently running |
