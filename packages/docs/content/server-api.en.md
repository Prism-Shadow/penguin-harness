...
| POST | /api/projects/:projectId/machines/:machineId/connect | Bring that machine's server up and hold the one connection to it — an `ssh -T -D` session; `202` with the same body, the connect job now running |
...
| GET | /api/projects/:projectId/machines/:machineId/dirs?path= | The subdirectories of `path` on that machine, over the shared shell — what the workspace picker browses. Addressed by the machine's OWN id, like the proxy |
