...
- What `permission` is: it decides which of that Server's tool calls PenguinHarness stops for approval, and nothing else. It does not sandbox the Server, does not restrict what its tools do when they run, and is not verified against the Server. Marking a Server `r` that can in fact write only removes the prompt that would have caught it.
