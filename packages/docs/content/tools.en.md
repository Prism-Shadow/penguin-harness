...
A deny produces a synthetic `tool_call_output` for the model to react to: `aborted` with `Tool call denied by user.` for a bare `deny`, or the refusal's own message and stop reason when the approval boundary states one — which is how a [command policy](/configuration#command-policy) rejection reads as `failed` rather than as a person cancelling. See [ApproveFn](/interfaces#approvefn). Every decision is written to the Trace as an `approval_decision` event, forming a complete audit record. Approval happens in the tool-execution phase of the [Agent Loop](/agent-loop).
