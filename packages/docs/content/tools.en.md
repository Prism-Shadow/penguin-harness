...
- Connecting is **lazy**: Session creation returns instantly, and the first `run()` connects all Servers in parallel and discovers tools once — the wait streams as `mcp_connect_begin` / `mcp_connect_end` events (frontends show a connecting status; the Trace timeline records per-Server durations), and the full tool definitions follow as a `session_tools_ready` event (see [OmniMessage](/omni-message)). The result is a Session-lifetime snapshot and `tools/list_changed` notifications are ignored. An unreachable Server or invalid entry only produces a stderr warning and is skipped — **the session is never blocked**.
