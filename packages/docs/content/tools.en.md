...
- The first `listTools()` (at Session assembly) connects all Servers in parallel and discovers tools once; the result is a Session-lifetime snapshot and `tools/list_changed` notifications are ignored. An unreachable Server or invalid entry only produces a stderr warning and is skipped — **Session creation is never blocked**.
