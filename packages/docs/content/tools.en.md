...
// input_subagent
{
  ...
  prompt?: string;         // steering interjection while the child runs; follow-up round while idle; optional guidance when resuming; empty = poll only
  ...
  resume?: boolean;        // true = resume an interrupted or failed subagent run from where it stopped
