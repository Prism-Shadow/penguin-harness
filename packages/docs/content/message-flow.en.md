...
| Compaction | `compaction_begin` → the compaction request runs against the old context (its raw messages are written to Trace only; the summary text being generated is forwarded as `compaction_delta` events) → that request's `token_usage` → `compaction_end(status)` |
