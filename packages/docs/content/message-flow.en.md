...
| Automatic reconnect | `request_end(timeout \| malformed)` → a fresh `request_begin` (up to 5 times; `timeout` covers transport disconnects and transient provider quota errors too); the `[turn_retried]` block is model-visible only |
