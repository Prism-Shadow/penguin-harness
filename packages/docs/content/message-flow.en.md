...
The Server pushes this exact output stream verbatim (single-line JSON) onto the per-Session SSE channel: monotonically increasing event ids, a bounded replay buffer for reconnects, `resync_required` when the replay window is gone. Event order: on reconnect the replayed gap (or `resync_required`) comes first, then the authoritative `task_state` and `subagent_state` snapshots followed by pending approvals; a fresh connection skips replay, so those two snapshots are its first events. Details — including the bundled Web App's connect-first + dedup consumption pattern — are on the [Server API](/server-api) page.
