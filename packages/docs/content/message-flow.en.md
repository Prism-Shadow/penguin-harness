...
| Automatic reconnect | `request_end(timeout \| malformed)` → a fresh `request_begin` (up to 5 times, exponential backoff with a 30s ceiling; `timeout` covers every LLM failure except an authentication error); the `[turn_retried]` block is model-visible only |
