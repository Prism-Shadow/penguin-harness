...
For developers: `kernel_version` advances manually, and only on a substantive change to the built-in defaults (using that day's date). The pinned-hash test in CI (`core/test/kernel-version.test.ts`) recomputes every default leaf hash against `KERNEL_HISTORY.current` in `kernel-history.ts` and fails on drift, naming each leaf that moved and the edit it needs: bump `KERNEL_VERSION`, write the recomputed hash into `current`, and append the leaf's previous hash to `superseded` (a leaf that is new to the defaults gets an `addedIn` entry instead). Several changes on the same day may reuse that day's version. The superseded hashes are frozen forever — they are what identifies "still the old default".
