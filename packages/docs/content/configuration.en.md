...
| `PENGUIN_PINNED_AGENT` | `<projectId>/<agentId>`: serve that one Agent and refuse every route that would create, import, delete or redefine an Agent (`403 agent_pinned`, admin included) — see [Server API](/server-api). Set by the pinned Docker export's entrypoint; an existing data root keeps its other Projects and Agents, they are simply never listed or served | unset — an ordinary multi-agent server |
