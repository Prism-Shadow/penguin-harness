...
| `compaction.max_context_length` | `512000` | Context Token threshold that triggers compaction; the effective threshold is capped at the model's `context_window` − 2048, so unless the window is larger still it is the model's window that decides when compaction fires (an entry with no `context_window` assumes a 128000 window) |
