...
模型对话框从服务端健康注册表读取脱敏状态，显示**可用**、**冷却中**或**已剔除（401）**。该注册表只存在于当前服务进程中：查询时根据已保存的模型凭据初始化，服务重启即丢失，并且与真正执行推理的各 Session 轮换器相互独立。因此，接口中的计数与状态只是注册表遥测，并不是活跃 Session 已发请求的持久或权威视图。**重置密钥**会清除注册表条目的剔除标记、冷却、成功/失败计数、最后使用时间与轮询位置；它不会修改已保存凭据、重建 Session 运行时，也不会重置活跃 Session 的轮换器。要让后续 Task 重建缓存运行时，仍需编辑并保存模型凭据。
