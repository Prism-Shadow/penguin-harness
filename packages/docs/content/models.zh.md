...
「新增分组」对话框也能一步把全新分组按端点填满：填分组名与 base URL（API key 可选），点「检测并导入」——先跑同一套协议检测，再向端点询问它服务的全部模型 id（`POST /api/projects/:id/models/list`，仅 owner——在检测出的协议客户端上调用 AgentHub 的 `listModels()`，限时 20s）。列表中的每个 id 都在一次整表写入中成为新分组的条目——base URL、检测出的协议与所填 key 内联在每条上——顺序保持端点返回的顺序，重复项被跳过并计入 toast。key 留空时沿用各处一致的按协议环境变量回退。失败（未检测到兼容协议、协议不支持列出模型、列表为空）只在对话框内呈现且不落盘；手动路径——只填分组名进入新增模型对话框——保持不变。
