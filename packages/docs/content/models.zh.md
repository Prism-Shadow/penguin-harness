...
一个模型条目可通过有序数组 `api_keys` 保存多份凭据。Web 模型对话框的**多密钥轮换**模式支持每行一个 key，也支持逗号或分号分隔；空值会被移除，重复项保留第一次出现的位置。为保持兼容，解析后的第一个 key 也会保存在 `api_key`。程序调用方可向 `createSession` / `resumeSession` 传入 `apiKeys`；显式 `apiKeys` 数组优先，其次是条目的 `api_keys`，最后才解析单个 `api_key`。

Session 在模型运行时创建时建立自己的内存轮换器。每次请求按轮询顺序选择下一个可用 key。请求完成会增加该 key 的成功次数；HTTP 429 会增加失败次数并进入 60 秒冷却，随后由引擎的常规重试路径选择其他可用 key；HTTP 401 会增加失败次数并把该 key 从当前运行时中剔除。仍有其他未剔除 key 时该失败可重试；所有 key 都被剔除时，请求以鉴权失败结束。其他可重试错误只增加失败次数，不会剔除或冷却。当所有未剔除 key 都在冷却时，轮换器会选择最早结束冷却的 key，而不会等待。

模型对话框从服务端健康注册表读取脱敏状态，显示**可用**、**冷却中**或**已剔除（401）**。健康遥测中的每个密钥条目还会汇报其当前承载的活跃子代理租约数（`activeLeases`）、成功/失败计数以及最后使用时间。该注册表只存在于当前服务进程中：查询时根据已保存的模型凭据初始化，服务重启即丢失，并且与真正执行推理的各 Session 轮换器相互独立。因此，接口中的计数与状态只是注册表遥测，并不是活跃 Session 已发请求的持久或权威视图。**重置密钥**会清除注册表条目的剔除标记、冷却、成功/失败计数、最后使用时间、活跃租约与轮询位置；它不会修改已保存凭据、重建 Session 运行时，也不会重置活跃 Session 的轮换器。要让后续 Task 重建缓存运行时，仍需编辑并保存模型凭据。

### 子代理密钥分配策略与并发租约

当父级 Agent 派生子代理（Subagent）并发执行任务时，若所有子代理集中调用同一个 API key，极易触发服务商的并发与速率限制（Rate Limit）。PenguinHarness 提供了针对子代理运行时的密钥智能分配与租约管理机制：

- **密钥分配策略**：
  - `auto`：智能优先选择未冷却且活跃租约最少的密钥，并在租约均衡时回退至轮询。
  - `least_busy`：选择当前绑定并发子代理租约数最少的健康密钥。
  - `round_robin`：在可用未冷却密钥之间按顺序依次轮换。
  - `dedicated`：如果存在闲置密钥，为该子代理分配独占专属密钥。
  - `inherit`：直接继承并共用父会话的密钥轮换器。
- **活跃租约追踪（`activeLeases`）**：每个子代理会话启动时均会在所选密钥上注册一个租约，租约计数同步反映在密钥健康遥测中。子代理执行完毕、被中断或被淘汰时，租约会自动且安全地释放。
- **429 速率限制故障转移与退避**：当模型请求收到 HTTP 429 响应时，系统会自动执行带抖动的指数退避重试，并无缝故障转移至下一个可用健康密钥，同时将受限密钥置入 60 秒的冷却队列。

健康与重置路由见[服务端 API](/server-api#模型)。
