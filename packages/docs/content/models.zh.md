...
| `client_type` | 协议提示(如 `openai-chat`)；缺省由 AgentHub 按 model id 推断。0.4.2 之前的旧写法 `openai` 为已废弃别名，读取配置时归一化为 `openai-chat` |
| `display_name` | 显示名 |
...
预置目录中的部分模型：deepseek-v4-pro / deepseek-v4-flash、MiniMax-M3、gemini-3.7-flash、claude-opus-4-8 / claude-sonnet-5、gpt-5.5、glm-5.3、kimi-k3、qwen3.8-max 等(非完整清单)。DeepSeek 直连分组的价格记录官方低谷时段档(高峰时段——北京时间 9:00–12:00、14:00–18:00——按双倍计费)。
