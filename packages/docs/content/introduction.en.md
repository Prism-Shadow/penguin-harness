...
- **A minimal toolset**: dedicated file tools (`read_file` / `edit_file` / `write_file`) for precise reading and editing, native `web_search` for current-source discovery, and the shell (`exec_command`) as the general-purpose fallback. See [Tools & Approval](/tools).
