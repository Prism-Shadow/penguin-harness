...
`penguin-go` is a built-in model group, like TokenDance. New Projects contain the current eleven catalog models immediately; existing Projects add them through **Sync presets** on the Models page. The built-in rows provide a stable initial route to `https://token.penguin.ooo/api`, while the platform catalog returned during authorization or **Sync** can add model ids that are missing locally, together with their protocol, endpoint, display name, context window, maximum output per request, vision capability, and platform price. Existing rows refresh their three platform price buckets, client protocol, and flat-promotion metadata; their endpoint and other Project-owned configuration are not overwritten, and rows are never deleted. Pure embedding rows are excluded. The platform reports peak rates in USD per million tokens. The current built-in Penguin Go DeepSeek rows declare the same Beijing off-peak schedule as the direct DeepSeek group, so their model cards and cost records use half price outside weekday 9:00–12:00 and 14:00–18:00.

Before the group has a platform key, its **Authorize key** action creates a one-time request only after the user clicks it, then opens Penguin Go in a new tab while the local server polls. The browser receives only an authorization URL and local flow id; the device secret and delivered API key remain server-side. On success PenguinHarness validates `connection.apiKey`, the endpoints, and the model catalog. It writes the key to every existing group model, adds newly advertised models with their recommended protocol, endpoint, name, context window, maximum output per request, vision capability, and price, and refreshes existing platform-model prices, client protocols, and flat-promotion metadata. Synchronization preserves existing endpoints and other Project-owned configuration and never deletes models. Account and balance data are neither saved nor displayed.
