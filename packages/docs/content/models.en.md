...
| `client_type` | Protocol hint (e.g. `openai-chat`); inferred by AgentHub from the model id when omitted. The pre-0.4.2 spelling `openai` is a deprecated alias and is normalized to `openai-chat` when the config is read |
...
Some models in the preset catalog: deepseek-v4-pro / deepseek-v4-flash, MiniMax-M3, gemini-3.7-flash, claude-opus-4-8 / claude-sonnet-5, gpt-5.5, glm-5.3, kimi-k3, qwen3.8-max (not exhaustive). DeepSeek's direct-group prices record the official off-peak tier (peak hours, Beijing 9:00–12:00 and 14:00–18:00, bill double).
