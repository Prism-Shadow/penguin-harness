...
Before the group has a platform key, its **Authorize key** action creates a one-time request only after the user clicks it, then opens Penguin Go in a new tab while the local server polls. The browser receives only an authorization URL and local flow id; the device secret and delivered API key remain server-side. On success PenguinHarness validates `connection.apiKey`, the endpoints, and the model catalog. It writes the key to every existing group model, adds newly advertised models with their recommended protocol, endpoint, name, context window, vision capability, and price, and refreshes existing platform-model prices, client protocols, and flat-promotion metadata. Synchronization preserves existing endpoints and other Project-owned configuration and never deletes models. Account and balance data are neither saved nor displayed.
