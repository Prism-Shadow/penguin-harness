...
`penguin-go` is a built-in model group, like TokenDance. New Projects contain the current eleven catalog models immediately; existing Projects add them through **Sync presets** on the Models page. The built-in rows provide a stable initial route to `https://token.penguin.ooo/api`, while the platform catalog returned during authorization or **Sync** can add model ids that are missing locally, together with their protocol, endpoint, display name, context window, maximum output per request, vision capability, and platform price. Existing rows refresh only their three platform price buckets; their routes and other configuration are not overwritten, and rows are never deleted. Pure embedding rows are excluded. The platform reports current peak rates in USD per million tokens; this group does not separately apply DeepSeek's off-peak discount.

Before the group has a platform key, its **Authorize key** action creates a one-time request only after the user clicks it, then opens Penguin Go in a new tab while the local server polls. The browser receives only an authorization URL and local flow id; the device secret and delivered API key remain server-side. On success PenguinHarness validates `connection.apiKey`, the endpoints, and the model catalog. It writes the key to every existing group model, adds newly advertised models with their recommended protocol, endpoint, name, context window, maximum output per request, vision capability, and price, and refreshes existing platform-model prices. Synchronization does not overwrite existing configuration other than price and never deletes models. Account and balance data are neither saved nor displayed.

After a platform key is configured, a separate **Sync** action appears immediately before **Add model**. **Authorize key** remains available so the owner can replace the key with one from another platform account. Sync uses the stored key to request the latest catalog, adds new models, refreshes existing prices, and leaves the config file untouched when neither models nor prices changed. If the platform reports that the key is invalid or revoked, the Models page automatically opens authorization again. Penguin Go's browser session and API key are separate: an expired browser session does not prevent a valid key from synchronizing; reauthorization is needed only when the key itself is invalid.
