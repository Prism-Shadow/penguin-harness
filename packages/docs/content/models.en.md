...
The add-group dialog can also fill a brand-new group from its endpoint in one step. Give the group a name, a base URL, and optionally an API key, and **Detect & import** runs the same protocol detection and then asks the endpoint for every model id it serves (`POST /api/projects/:id/models/list`, owner only — AgentHub's `listModels()` on the detected protocol client, bounded at 20s). Every listed id is saved as an entry of the new group in one table write — base URL, detected protocol, and the typed key inline on each — in the endpoint's own order, with duplicates skipped and counted in the toast. An empty key follows the same per-protocol environment fallback as everywhere else. Failures (no compatible protocol, a protocol with no models endpoint, an empty listing) stay inside the dialog and persist nothing; the manual path — name only, then the add-model dialog — is unchanged.
