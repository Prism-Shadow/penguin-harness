...
Some models in the preset catalog: deepseek-flash / deepseek-v4-pro (`deepseek-flash` is V4.1 Flash and reads images; `deepseek-v4-pro` is the V4 Pro 0813 release and is text-only. DeepSeek still accepts the retired `deepseek-v4-flash` and `deepseek-v4-flash-vision-exp` names and serves them from V4.1 Flash. They are no longer presets, but the catalog keeps them — and TokenDance's `deepseek-v4-flash-vision-exp` — as retired rows, so a Project that still has them keeps their names and their off-peak pricing: send an image to `deepseek-flash`), MiniMax-M3, gemini-3.8-flash, claude-opus-5 / claude-opus-4-8 / claude-sonnet-5, gpt-6-astra / gpt-5.6 / gpt-5.5, glm-5.3 / glm-5.3-flash, kimi-k3, qwen3.8-max / qwen3.8-flash, seed-2.1-pro / seed-2.1-turbo / seed-evolving, dots-3-note-preview (free on TokenDance, 512K context) (not exhaustive). The whole OpenAI line-up is listed twice — directly (your own OpenAI key, list prices) and on OpenRouter as `openai/<id>` (the gateway's rates, which follow its running promotions). DeepSeek's direct-group rows record the official peak tier and declare its off-peak schedule: outside Beijing weekday 9:00–12:00 and 14:00–18:00 every bucket is halved, which the models page marks with a `50% off` badge and the cost center bills at. Two resold rows follow the same schedule, because their sellers pass DeepSeek's own windows through: TokenDance's `deepseek-v4.1-flash` and OpenRouter's `deepseek/deepseek-v4.1-flash`. Qwen bills the DeepSeek models it sells on a schedule of its own — half price from 22:00 to 8:00 Beijing time, every day — so `deepseek-v4.1-flash` in both Qwen groups and the Token Plan's `deepseek-v4-pro-0813` declare that one instead, and the badge's tooltip names the windows of whichever schedule the row follows. The stored price is always the peak one, so what is on disk does not depend on the hour a Project was created or re-synced in. GLM-5.3 Flash appears five times — directly as `glm-5.3-flash`, under the same id on TokenDance, and as OpenRouter's `z-ai/glm-5.3-flash`, Fireworks AI's `accounts/fireworks/models/glm-5p3-flash` and Qwen Pay-As-You-Go's `ZHIPU/GLM-5.3-Flash` — and every row accepts images: AgentHub's GLM client forwards image parts for this one GLM id (every other GLM id refuses them), while the gateway rows go through the generic OpenAI-compatible clients, which carry them for any id. What the rows do not share is the price: each records what its own seller charges, so they disagree while a promotion is running.

TokenDance rows record the gateway's list price and, where a promotion is running, the rate off it. Nine models are discounted today — `kimi-k3` at 40% off, `deepseek-v4-flash-0731`, `deepseek-v4-pro-0813`, `glm-5.3`, `glm-5.3-flash` and `qwen3.8-max` at 10%, and the three Doubao Seed rows (`seed-2.1-pro`, `seed-2.1-turbo`, `seed-evolving`) at 50%. One further row, `deepseek-v4.1-flash`, carries no flat discount and instead follows DeepSeek's own off-peak schedule, described above. Their model cards show the price being billed right now, with the rate as a badge, and a Project is preset with the **discounted** price, so the cost center charges what the gateway charges. Prices you edit yourself keep the discount decoration off the card: the figure is then yours, not the gateway's.
