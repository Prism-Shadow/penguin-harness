...
| | `use-codex` | 使用 ChatGPT 订阅通过 Codex ACP 适配器委托编程任务：通过 MCP 提供项目级设备登录、模型发现、进度、取消和人工答复。任务具有工作区写入权限。安装插件后，按照 `codex` Skill 配置 Agent MCP 并连接账户。包含 Codex 运行时，需要 Node 24+。不预装，也不是模型选择器中的提供商。 |
