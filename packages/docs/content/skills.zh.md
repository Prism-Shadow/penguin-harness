...
| Agent 公司 | `agent-company` | [公司模式](/company-mode)的整套工具箱——七个 Skill：`company-setup`（与用户一起创建组织：一次只问一个问题，确认摘要后执行 `penguin org create`；它自己不招募、不建工单）、`company-employee`（每个工位会话与工单会话都遵循的协议：触发块、工单看板、阻塞、重负载或不可逆的事先向董事会请示、频道礼仪、预算）、`company-ceo`（使命拆解为工单、招募、划分工作区、审核、向董事会汇报）、`company-hr`（保证日程有安排、招募与离任、评估）、`company-finance`（预算、每日巡检、告警与暂停处置）、`company-research`（科研组织的作者与审稿人：先固定评测与指标，在董事会批准的资源额度内跑实验循环、只保留有提升的改动，每个结论都交给一个设法推翻它的审稿人）、`company-mirror`（数字分身公司：为每位现实同事建一个分身、绑定到他的机器人、只传话不开工单）（不预装：组织创建 CEO 与招募员工时自行安装；想让某个 Agent 能创建组织，从技能库装上它即可） |
