...
| | `use-codex` | 使用 ChatGPT 订阅将编程任务委托给官方 Codex app-server：通过 MCP 提供项目级设备登录、模型发现、任务进度、取消和人工审批转发。安装插件后，按照 `codex` Skill 配置 Agent MCP 并连接账户。需要 Codex CLI；不预装，也不是模型选择器中的提供商。 |
