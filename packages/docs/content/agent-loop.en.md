...
The Task ends when a turn produces no `tool_call`. A denial produces a synthetic tool output the model reacts to: `aborted` with "Tool call denied by user." for a plain `deny`, or the refusal's own message and stop reason when the approval boundary states one (see [ApproveFn](/interfaces#approvefn)).
