...
The Task ends when a turn produces no `tool_call`. A denial produces a synthetic `aborted` tool output the model reacts to — "Tool call denied by user.", or "Tool call denied by policy." when the approval boundary names the [command policy](/configuration#command-policy) as the decider (see [ApproveFn](/interfaces#approvefn)).
