...
The compaction request keeps the session's toolset **unchanged** — the request prefix (tool list included) stays byte-identical to ordinary turns, so the provider's prompt cache remains valid at the moment the context is largest. Compaction still succeeds only with a valid summary: a response that calls a tool or whose extracted summary is empty counts as one more failed attempt — any tool calls are answered with synthesized failed outputs (keeping `tool_use`/`tool_result` pairing intact), and the resent request carries a corrective note ahead of the compaction Prompt (committed history can only be appended to; rewriting it would invalidate the prompt cache). Every failure — unusable summaries and the `failed`/`timeout`/`malformed` transport statuses alike — draws on the one reconnect budget and backoff ladder described under "Automatic reconnect" above; only `auth` stops the compaction at once. Once the budget is exhausted the compaction ends `failed`, keeping the original context and Trace file until the next trigger; `compaction_end` reuses the unified retry detail block — `attempt` (the final attempt's ordinal, failed attempts included) and, on failure, the last `error_message` detail (shown on the chat banner and the CLI line) — and a failed compaction also lands in the cost center as a `compaction_failed` error record. The first **committed** attempt — adopted or rejected — also absorbs whatever turn input was folded into the compaction request (the carry-over a manual `/compact` folds in, or — for LLM implementations without `appendExchange` — mid-task tool results) into the old context's history: retries resend only the repairs and the Prompt, and the continuation after an abandoned compaction never resends the absorbed input.

When compaction triggers **mid-Task** — the model has just made tool calls and their results are ready — the engine first **closes the pending exchange**: the turn's tool outputs are committed to the old context as their own completed exchange, closed by a short synthetic assistant reply, and the compaction Prompt then rides **alone** as a fresh user turn. The model summarizes instead of continuing the task mid-exchange, and a rejected attempt can no longer absorb the turn's outputs. After a successful mid-Task compaction the summary itself is the new context's first input and the Task continues on it; after a failed one the Task now **always resumes too** — when the absorbed outputs leave nothing task-bearing to send, the engine synthesizes a `[compaction_failed]` continuation note (sent to the model only, like every synthetic carry-over) instead of ending the run.

While the summary is being generated, the engine streams it as `compaction_delta` events between the paired compaction events: the Web App's compaction banner shows the summary being written live, and folds the full text into an expandable body once the compaction settles. Deltas are stream-only — never written to Trace — and a history rebuild reconstructs the same text from the compaction span's recorded output, so a reload shows exactly what the live viewer saw.

A compaction the process never got to finish (a crash or kill mid-request leaves a `compaction_begin` with no matching end) is **healed on resume**: the session appends an aborted `compaction_end` before continuing the Trace file, so stateless readers — the Web reducer, the server's history windows — never mistake the conversation that follows for compaction-internal messages. Readers additionally close a stale span themselves on the unambiguous signals (another `compaction_begin`, an `abort` event, a rotation's `session_meta`), keeping traces damaged before the heal readable.
