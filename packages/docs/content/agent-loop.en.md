...
Every LLM-side failure except an authentication error becomes `timeout` and triggers an in-run reconnect, as does `malformed` (truncated streams, JSON parse failures). The classification used to be an allowlist of known network codes, statuses and message vocabulary, and every gateway that phrased a transient failure its own way — `Upstream HTTP/2 stream failed`, for one — fell through it and killed the turn. The two mistakes cost wildly different amounts: wrongly retrying a permanent error costs the ladder below and then fails with the same message, while wrongly aborting a transient one destroys the turn. So the default inverted; deterministic 4xx bodies (bad parameters, context length exceeded, content policy) are knowingly retried too. A reconnect the engine re-sends the original input plus a `[turn_retried]` block carrying the previous partial output, so tools are never re-executed. Default limit is 5 reconnects with exponential backoff under a ceiling (base 250ms, cap 30s: 250ms, 500ms, 1s, 2s, 4s ≈ 7.75s of total patience — one shared schedule growing toward the slower retryable classes, with the first steps as fast as a transport blip needs); beyond that the turn settles as `failed`, and its `abort` event carries `code: "retry_exhausted"` — the structural signal that separates a turn the ladder failed to save from routine retry noise, which the Cost center files as needing a human (`llm_retry_exhausted`, unexpected) rather than as one more expected `llm_timeout`. Each failure's `request_end` announces the planned wait as `retry_in_ms` (same formula as the sleep), which the Web App renders as a live countdown with "retry now" (skips the remaining wait via `Session.skipReconnectWait` — the attempt counter is unchanged) and "give up" (the ordinary abort; the engine's abort-during-backoff path ends the turn) controls. Compaction requests use their own tighter cap (3 retries): a failed compaction keeps the original context and retries on the next trigger, so failing fast beats stalling the session. Authentication errors are classified before any retry heuristic and never retry: the request ends with its own terminal status `auth` (only the model reference is fixed at Session creation — credentials are read from the current Project config when the Session loads), and the Web App disables that Session's composer until the model's credential is updated (which auto-unlocks it) or the notice is dismissed for a retry. Tool errors are never retried — they are fed back to the model as `tool_call_output` and the model decides what to do next.
