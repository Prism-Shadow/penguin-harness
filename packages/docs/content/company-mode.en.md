...
5. **People decide in the chat and on the board.** Only `@<employee>` and `@all` deliver a message to a desk; a message that reaches the mention-chain limit is recorded but delivers nothing, so two employees cannot ping-pong forever. Accepting, rejecting and reviewing tickets is yours or the CEO's, as the handbook says.
