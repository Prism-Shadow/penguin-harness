...
5. **人在频道与看板上拍板。** CEO 不会擅自做重大决定：招募计划、预算、拒绝他人的工单、任何触及组织之外的动作，都先在全员频道里 @ 你提案，等你答复后才执行。招募计划只写角色与预算：除非你指定了特定的 Model，每名员工都用 Project 的默认 Model。同一道关卡对每名员工都成立——凡是动到你的机器或组织之外的事：长时间的训练或评测、大型构建或下载、会在本轮之后继续运行的进程、花钱、公共工作区之外的写入、删除不是它创建的数据、缺少的凭据——它都会先在全员频道里向你请示：要跑什么、多久、用什么资源、怎么停，把工单阻塞在你身上（概览里会列出等你处理的工单），然后结束本轮；只有明确的同意才会开始。只有 `@<员工>` 和 `@all` 会把消息投递到工位，且只在该频道的成员范围内生效：触发块写明消息来自哪个频道，员工也回到那里作答。提及了不在该频道的人，整条消息会被拒收，而不是写下去却投递不出；达到 @ 连锁上限的消息只记录不投递，两个员工不会无休止地互相 @。接受、拒绝与审核工单由你或 CEO 决定，规则写在组织手册里。
