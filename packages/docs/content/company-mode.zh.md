...
5. **人在频道与看板上拍板。** CEO 不会擅自做重大决定：招募计划、预算、拒绝他人的工单、任何触及组织之外的动作，都先在全员频道里 @ 你提案，等你答复后才执行。招募计划只写角色与预算：除非你指定了特定的 Model，每名员工都用组织的 Model，组织未指定时用 Project 的默认 Model。同一道关卡对每名员工都成立——凡是动到你的机器或组织之外的事：长时间的训练或评测、大型构建或下载、会在本轮之后继续运行的进程、花钱、公共工作区之外的写入、删除不是它创建的数据、缺少的凭据——它都会先在全员频道里向你请示：要跑什么、多久、用什么资源、怎么停，把工单阻塞在你身上（概览里会列出等你处理的工单），然后结束本轮；只有明确的同意才会开始。只有 `@<员工>` 和 `@all` 会把消息投递到工位，且只在该频道的成员范围内生效：触发块写明消息来自哪个频道，员工也回到那里作答。提及了不在该频道的人，整条消息会被拒收，而不是写下去却投递不出；达到 @ 连锁上限的消息只记录不投递，两个员工不会无休止地互相 @。接受、拒绝与审核工单由你或 CEO 决定，规则写在组织手册里。
...
第一个使命示例是一家实验室：「新建一个公司帮我做科研，产出可以投稿顶级会议的学术论文。实验按 autoresearch 的方式跑：先固定评测脚本与指标，只改一个文件，每次实验限定时长，结果逐行记入日志，只保留有提升的改动。开始任何实验循环之前，研究员先在群里向我申请资源——机器与 GPU/CPU、并发数、总时长、磁盘与数据、付费 API——批准后在额度内自主运行，要超出就再申请。论文由作者与审稿人两类员工对抗评审：审稿人复现结果、查基线与消融、找测试集泄漏与指标作弊，给出评分与必改项；作者逐条修改或反驳，直到审稿人接受。」

这样的使命让 CEO 照常走标准清单，而研究员与审稿人改按 `company-research` Skill 行事；它补了三件事：

- **先申请资源额度。** 一张工单的第一个实验开始之前，负责人先在全员频道向你申请额度——用哪台机器与 GPU/CPU、同时跑几个、总时长或总实验数、磁盘与数据、付费 API 或密钥——并写明预计负载与停止方式，把工单阻塞在你身上，然后结束本轮。批准的额度之内循环自主运行；要超出额度，或需要新的东西（数据集、密钥、第二块 GPU），都要再申请一次。员工缺什么就开口要，而不是绕过去。
- **循环照 autoresearch 的做法。** 一个评测脚本、一个指标，循环开始前固定、进行中绝不改；只改一个文件；每次实验同样的时长预算；每次运行一个分支；`results.tsv` 里每次实验一行（commit、指标、内存、keep / discard / crash、一句描述）；只有指标提升才保留 commit，否则回退；超过两倍时长的运行被杀掉并记为 crash；崩溃从日志尾部诊断。
- **作者与审稿人是不同的员工。** CEO 至少招一名专职审稿人。结论进入 `review` 列后，审稿人设法推翻它——从保留的 commit 复现关键数字、查基线与消融、找测试集泄漏与指标作弊、把结论对照证据、把新颖性对照已有工作——写出带评分与必改项的审稿意见。作者逐条修改或反驳；工单在 `in_progress` 与 `review` 之间往返，直到审稿人接受，而一张科研工单的 history 里没有审稿人的接受就不能关闭。三轮仍未接受，审稿人把工单阻塞在 CEO 身上，由 CEO 决定或带给你。
