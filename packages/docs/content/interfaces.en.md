...
- a provider rejecting a thinking-mode request because the history behind it doesn't bring its `reasoning_content` back is a verdict on the **context**, not on the turn: retrying it unchanged can only fail the same way, so thinking goes off the wire for the rest of that context (an explicit `none`, outranking both the per-request override and the construction default) and the rejection is still reported as `failed` for the engine to retry — the committed history is never rewritten to synthesize the missing reasoning. See [Models & Providers](/models);
