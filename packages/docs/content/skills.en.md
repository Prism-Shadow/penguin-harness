...
| | `use-codex` | Delegate coding tasks to the official Codex app-server using a ChatGPT subscription: project-scoped device sign-in, model discovery, task progress, cancellation, and human approval relay through MCP. Install the plugin, follow the `codex` skill's Agent MCP setup, and connect your account. Requires the Codex CLI; not preinstalled and not a model-picker provider. |
