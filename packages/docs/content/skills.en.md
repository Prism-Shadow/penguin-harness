...
| AI App Development | `agent-development` | Agent development on PenguinHarness — five skills: `penguin-sdk` (build agent/AI/RAG apps on the SDK), `unified-llm-api` (call model APIs through `@prismshadow/agenthub`), `penguin-config` (manage model keys, defaults and Vault secrets), `penguin-orchestration` (drive agents, sessions, costs and schedules from a shell) and `agent-porting` (import an agent from a Claude Code, Codex or Pi setup or an exported bundle; export one as an API bundle for a coding agent) |
