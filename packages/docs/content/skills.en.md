...
| Software Development | `software-development` | Software development end to end — three skills: `software-engineering` (investigate, implement and validate with minimal scope), `web-design` (the Penguin visual language for generated web UIs) and `app-center` (run a finished app in the background, register it with the [App Center](/web-app#app-center-apps) through `penguin app`, and carry out its restart / stop requests) |
