...
plugins.json 加载 + PluginHost（⑤ 发布）  插件 onCreateApp 投递 · workflow 实例 · "create" 事件
```
...
├─ plugins.createApp(iface)          # 每个插件的 onCreateApp，按注册顺序同步执行
├─ sandbox = new SandboxService(已注册的后端)   # 由寄存的设置重新水合
├─ plugins.emit("create", {          # 目前平台发出的唯一事件
...
| 时机           | 频率        | 发生什么                                                                                             |
| -------------- | ----------- | ---------------------------------------------------------------------------------------------------- |
| 模块加载       | 每进程一次  | 启动步骤 ④ `loadPlugins`：解析 `plugins.json` 里的 specifier 并 import；单条失败被隔离并记日志，不阻断启动 |
| onCreateApp    | 每 App 一次 | 插件向全新的 `iface` 注册 workflow factory 与 sandbox 后端（`tool` 槽位保留未用）                    |
...
| 插件宿主             | ④ `loadPlugins` 构建，⑤ 发布进资源注册表      | `RawPlugin` / `PenguinInterface` / `PenguinContext`；配置面是 `<root>/plugins.json`                          |
