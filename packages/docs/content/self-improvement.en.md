...
Self-improvement in PenguinHarness uses Skills to orchestrate the ordinary Agent machinery: evaluations are ordinary Sessions, optimization is ordinary file editing, and promotion validation is another isolated top-level Session. The user starts only two prompts, Builder and Optimizer. After the Optimizer submits a structured nomination, the server validates it and creates the third-stage Session automatically. Individual evaluations are delegated through the built-in `run_subagent` tool. Top-level prompts provide the Agent, Benchmark, capability, score, and round settings; Skills and the server-side promotion state machine own call relationships, calibration, Freeze, protocol, repair, rollback, and reporting.
...
| Promotion Validator | New top-level Agent created automatically after optimization; evaluates only the final Candidate's held-out matrix |

The Builder and Optimizer directly follow their Skills in their own top-level Sessions. The Promotion Validator runs in a third top-level Session created by the server. A minimal fixed prompt orchestrates `agent-evaluation`; it writes only the complete held-out Evaluation and does not decide whether to retain or restore. The server deterministically compares scores, writes `promotion_decision`, and retains or rolls back Agent State, without returning held-out evidence to the Optimizer. Evaluators are created through `run_subagent`; each follows `agent-evaluation` and uses the Penguin CLI to launch the specified Target Agent in an isolated Workspace identified by an absolute path. The Penguin CLI launches the Target Agent for the requested Case run.
...
The highest-scoring Reference retained when the Optimizer stops is only a **Development-accepted Candidate**. It is already the active Agent State, but it must not be reported as production-promoted until it passes phase 3. If the batch produced a new Candidate, the Optimizer must atomically create, and must not overwrite, `promotion_requests/<optimization_session_id>.yaml`. The file contains only protocol version, Optimizer Session id, Target Agent id, Development Benchmark id, production Reference version, and final Candidate version; it must not contain a Promotion id, held-out evidence, Runtime, or free-text diagnosis. The final report must include those batch fields, the Development evaluation Runtime, the recovery archive `snapshots/v<production_reference_version>.tar.gz`, and whether the automatic promotion request was submitted. The server rejects a request without a complete Development Evaluation, the active Candidate, or a verifiable production Reference Snapshot.

### Phase 3: Decide promotion on the held-out Benchmark

After the Optimizer Task ends, the server discovers the matching nomination file and validates its immutability, ownership by the Optimizer Session, bidirectional Benchmark pairing, the final Development Evaluation's complete matrix and Runtime, the active Candidate version, and both production Reference and Candidate Snapshots. The server derives the Promotion id and frozen Runtime from the verified pair and production held-out Reference; the Optimizer does not provide them. Only after every check passes does the server create a new top-level `source: promotion` Session and give it the Target Agent, `optimization_session_id`, final `candidate_version`, pre-optimization `production_reference_version`, Promotion Benchmark id, and frozen Runtime. It must not inspect the Development Benchmark, the original Optimizer's diagnosis, or private Traces, and it must not modify the Candidate.

Before dispatching the first held-out cell, the server ensures that `snapshots/v<candidate_version>.tar.gz` exists, its archived version matches the current Candidate, and no existing same-version Snapshot was overwritten. This Snapshot preserves the exact state of a failed Candidate for diagnosis and reconstruction by a later batch; it must not let the old Candidate bypass new Development evaluation and be nominated directly. The server does not create the Promotion Session if the Snapshot cannot be created or validated.
...
The Candidate passes promotion if and only if the matrix is complete and valid, the Candidate remains unchanged throughout evaluation, and its held-out top-level average is no lower than the latest valid Evaluation for `production_reference_version` in the Promotion Scoreboard. On the first promotion that comparison target is the Formal Baseline; afterwards the current production version's held-out record is the Evaluation written when it was itself promoted. The symmetric one-Run matrix bounds cost but keeps run-to-run noise — a known trade-off, and the reason the gate is no-lower-than rather than strictly-higher. The Promotion Validator appends the complete held-out Evaluation and Session ids with no decision. The server reparses and validates the unique matrix, deterministically retains or restores, and then adds `promotion_decision` (`promoted` or `restored`) only to that record. The record also carries `evaluation_kind: promotion_candidate`, `optimization_session_id`, and `production_reference_version`; `summary_title` and `summary` remain human-readable. The server trusts the stored aggregate for the gate and does not recompute it.
...
- **Validation cannot complete:** do not treat missing or invalid cells as zero or claim promotion. The server marks the run failed and attempts a verified restore only while the active State is still this batch's Candidate. If another process has changed State, automation does not overwrite that unknown version and a human must inspect it; the failure category remains in run state and server error records.
...
- The Web evaluation pages read these files and Promotion run state directly; the trend chart shows Score only, while the detail table separates workflow status, model ID, and thinking level. A Development header distinguishes active from production versions and shows waiting, queued, running, promoted, restored, or failed state, with a link to the server-created Promotion Session. The frontend does not trigger promotion or mutate Agent State, request files, or the Scoreboard. See the [Web App Guide](/web-app).
...
The current Promotion Validator does not yet correspond to another built-in Skill. A server-owned fixed prompt only orchestrates existing `agent-evaluation` workers and asks them to write the held-out Evaluation; the deterministic gate, state convergence, and Snapshot restoration remain in runtime code. Extract an `agent-promotion` Skill only when the model-side protocol needs reuse from another entry point, avoiding a thin Skill that duplicates the server state machine as a second source of truth.
