...
The Candidate passes promotion if and only if the matrix is complete and valid, the Candidate remains unchanged throughout evaluation, and its held-out top-level average is no lower than the Promotion Benchmark's Formal Baseline for `production_reference_version`. Whether promotion passes or fails, append the complete held-out Evaluation and Session ids to the Promotion Benchmark's own Scoreboard. Record the source version, Candidate version, and promotion decision in `summary_title` and `summary` without changing the current Scoreboard schema.
...
- **Fail:** retain Development Benchmark scores and Traces as experimental evidence, but restore and verify `production_reference_version` from the pre-optimization Snapshot. Treat `candidate_version` as consumed and never reuse it.
