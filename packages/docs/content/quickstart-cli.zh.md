...
服务运行在 http://127.0.0.1:7364 并自动打开浏览器（`--no-open` 跳过）。账号是 `admin`，此时它还没有密码：服务端会以边框提示打印一条首次登录链接，打开即以登录态进入，随后设置密码即可。在密码被设置之前该链接一直有效，可以重复打开，并且每次启动都会重新打印。`penguin server` 启动同一进程的 headless 版本。
