...
| Memory | The Agent-level switch, then every memory grouped by scope — user memory first, then one group per Workspace, each group collapsible with an import entry — with view / delete / edit-via-chat actions per row |
