...
The setting exists because an inline image is not an attachment. An attachment goes to the Session scratchpad and the model opens it by path, so its size is paid once and nothing else scales with it — which is why file attachments have no size limit at all. An image placed inline enters the conversation and the Trace, where its size is paid again on every history page and every session resume. The model side is lower still (providers commonly cap around 5MB, and the `read_image` tool at 5MB), so shrinking a large image usually makes it more likely to be read, not less.
