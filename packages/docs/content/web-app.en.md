...
| Tools | Built-in tool table (incl. per-tool call_description switches) and MCP Server management: a table plus an add/edit form whose fields follow the transport (http by default) with a connectivity-test button (changes save immediately; full transport validation server-side) |
