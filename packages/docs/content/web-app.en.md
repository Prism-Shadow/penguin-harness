...
- Typing `/` opens the slash menu: trigger context compaction (`/compact`), hand the conversation over to another Agent (`/agent`), switch the model (`/model`), or toggle installed Skills — chosen Skills are sent along with the message in a `[use_skills]` block;
