...
Admin only, server-global: a switch, **Compress large images**, and the size above which it applies — **Compress images larger than**, in whole MB, defaulting to on at 4MB. The threshold may be set between 1 and 64MB; a value outside that range — "100GB" typed into a MB field as 102400, say — is refused with the reason shown under the input rather than accepted, and a rejected PUT writes neither field. Saving takes effect immediately, with no restart.

Above the threshold an image is fitted inside a 2048px box and re-encoded in the browser, keeping its own format, before it is read into a data URL — so the upload shrinks with the picture rather than after it. Below the threshold, and for any animated or vector image (GIF, SVG), the file is sent byte-for-byte: a canvas round trip would return a first frame or a rasterization, which is a different picture, not a smaller one. A re-encode that comes out no smaller than the source is discarded.

The setting exists because an inline image is not an attachment. An attachment goes to the Session scratchpad and the model opens it by path, so its size is paid once and nothing else scales with it — which is why nothing limits how large an attachment, or a request, may be. An image placed inline enters the conversation and the Trace, where its size is paid again on every history page and every session resume. The model side is lower still (providers commonly cap around 5MB, and the `read_image` tool at 5MB), so shrinking a large image usually makes it more likely to be read, not less.

One number is deliberately not exposed: the per-message file **count** stays at 20. It bounds how usable the composer's chip row is, not what the server can survive.

## Version and Updates

Checking for updates sits outside the settings dialog: one row in the sidebar user menu, directly under the **System settings** entry, so it is reachable without opening anything. The row has two states — it reads "Check for updates" and runs the manual check until a newer release is known, then turns in place into "New version vX available" and opens the update dialog instead. The running version sits muted on its right.

The running version's release date is stamped into the build by the release workflow and displayed without any network access: the new-chat page shows it under the brand as `vX.Y.Z · Last updated Jul 26` (dev builds and releases that predate the stamping, v0.1.2 and earlier, have no date). The app checks GitHub for a newer release once the sidebar user menu has first been opened, and immediately — bypassing the cached result — when the manual action is clicked; the latter reports "You're on the latest version" when nothing newer exists. When a newer release is found, a dot appears on the user button and the draft page's version line gains a small superscript "New version available" badge linking to the release. The update dialog carries the release-notes link, plus an "Update now" action for admins that runs `penguin update` on the server (the data directory is untouched). The service must be restarted afterwards for the update to take effect. Set `PENGUIN_UPDATE_CHECK=off` to disable the update check entirely — see the [Configuration Reference](/configuration).

**In the desktop app** that row becomes a client-update row — the shell updates itself, and the server check never appears in desktop mode — in the shell's own window only. It drives the app's own updater end to end: "Check for updates", then a background download with its percentage in the label, then "Restart to install vX", which asks for confirmation first because restarting interrupts running Tasks. The installed client version sits muted on the right, and the app also checks on its own schedule, so the row may already offer the restart when you open the menu. A browser signed into the same desktop-mode server with a password gets neither row: it can neither read that machine's updater state nor restart its window, and it cannot run the CLI self-update on that server either. Forms that cannot replace themselves — a Linux install owned by the system package manager (`.deb`), and a development run — render the row disabled with the reason as its tooltip; the [Desktop app guide](/quickstart-desktop) covers the installers.
