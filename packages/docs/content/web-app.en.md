...
| Memory | The Agent-level switch, then every memory grouped by scope — user memory first, then one group per Workspace — with view / delete / edit-via-chat actions per row |
...
The Memory tab's switch writes immediately rather than joining the tab-level Save, so turning Memory off never drags an unrelated half-finished edit along with it. Turning it off keeps every file and the tab fully usable: it only stops Memory from entering the agent's context and from preparing directories for new Sessions. Rows are read-only on purpose — **View** opens the rendered body in a drawer, **Delete** confirms and also removes the file's `MEMORY.md` index lines, and **Edit** jumps to a new chat with this agent, prefilled with the memory's file path and an instruction to keep the index and `updated_at` in step; a Workspace memory's edit chat also pins that Workspace, so the editing Session reads the very index it is changing. The tab also edits the two memory prompts (`memory.prompt` / `memory.workspace_prompt`) in a collapsible section, and when the prompt template carries no `{{MEMORY}}` placeholder (an agent created before Memory) it shows a hint with a one-click insert — an explicit, idempotent config write. For the storage model, see [Configuration](/configuration#memory).
