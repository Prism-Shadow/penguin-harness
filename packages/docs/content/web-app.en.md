...
The sidebar user menu shows the running version with a localized "Last updated Jul 26"-style date — the running version's release date, stamped into the build by the release workflow and displayed without any network access (dev builds and releases that predate the stamping, v0.1.2 and earlier, show the version alone); the new-chat page carries the same version line under the brand. Once the menu has been opened, the app checks GitHub for a newer release: when one exists, a dot appears on the user button, both version lines gain a small superscript "New version available" badge (admins jump to the update dialog, others to the release page), and the menu gains a release-notes link, plus an "Update now" action for admins that runs `penguin update` on the server (the data directory is untouched). The service must be restarted afterwards for the update to take effect. The menu also offers a manual "Check for updates" action that queries GitHub immediately, bypassing the cached result, and reports "You're on the latest version" when nothing newer exists. Set `PENGUIN_UPDATE_CHECK=off` to disable the update check entirely — see the [Configuration Reference](/configuration).
