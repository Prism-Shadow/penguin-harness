...
- After each Task, a stats line shows tokens, TPS, elapsed time, cost, copy, and a **Fork** action. Fork opens a new Session at that completed assistant reply with the same Agent, model, Workspace, approval mode, and retained transcript. Its Trace and scratchpad are independent snapshots: local images/files keep rendering even after the source chat is deleted;
