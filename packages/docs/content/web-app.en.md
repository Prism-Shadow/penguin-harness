...
The sidebar user menu shows the running version with a localized "Last updated Jul 26"-style date — the release's build date, or, for installs that predate date stamping (v0.1.2 and earlier), the publish date resolved by the update check; the new-chat page carries the same version line under the brand. Once the menu has been opened, the app checks GitHub for a newer release: when one exists, a dot appears on the user button and the menu gains a release-notes link, plus an "Update now" action for admins that runs `penguin update` on the server (the data directory is untouched). The service must be restarted afterwards for the update to take effect. Set `PENGUIN_UPDATE_CHECK=off` to disable the update check entirely — see the [Configuration Reference](/configuration).
