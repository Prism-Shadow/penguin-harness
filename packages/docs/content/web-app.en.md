...
- Charts, all time series over the selected range: requests + success rate broken down by Agent and, beside it, the same by Model (a bucket's requests stacked one segment per entity on the left axis, each entity's success rate as its own dashed line on a right 0–100% axis, drawn above the bars; the top few entities are named and the rest fold into a neutral series labelled with how many, and a bucket an entity had nothing to rate in plots 0, which the hover text distinguishes from a bucket it actually failed), Token trend (three-segment stacked bars with a dashed cache-hit-rate line in front — continuous, a bucket without cache traffic reads 0), and cost trend (line + points + area). Every chart takes the same hovers: a column for its bubble, one bar segment to single that segment out, a line to single the line out. Lines are straight segments, never smoothed. Charts fit their card and never scroll; empty buckets display 0 rather than a dash;
