...
工具 schema **不在 meta 里**：工具集要等 MCP Server 连接完成才可知，而 meta 不应等待——完整工具定义在首次 run 时以独立的 `session_tools_ready` 事件下发（见 event_msg）。拆分前的旧版 Trace 在 meta 里内嵌 `tools` 字段，该字段已明确不再读取（旧 Trace 的工具记录不再展示）。
...
interface SessionToolsReadyPayload {
  ...
  tools: ToolDefinition[];    // 发给模型的完整工具 schema;首次 run 时发出一次(MCP 发现
                              // 完成后),压缩分卷时随 session_meta 一并重写到新 Trace 文件
}
...
interface McpServerConnectResult {
  ...
  status: "ok" | "failed";    // 失败逐 Server 且不致命:该 Server 被跳过,run 继续
  duration_ms: number;
  tools?: number;             // 发现的工具数(ok 时)
