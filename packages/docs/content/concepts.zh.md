...
插件是插件库安装的单位，包含 Skill、钩子包，或两者都有。把插件装到某个 Agent 上，这些内容就会放进这个 Agent 的 Agent State。
...
公司模式是 Web App 的一种 Beta 工作模式，让一个 Project 的 Agent 像一家公司那样自行运转。它默认关闭，需要管理员在**系统设置**里开启。
