...
| POST | /api/admin/settings/proxy-probe | 沿服务端出站链路探测上述目标，不携带任何凭据：`{proxyForApp, proxyUrl, probes: [{provider, url, outcome, ms, status?}]}`；`outcome` 在收到任意 HTTP 响应时为 `reachable`，否则为 `timeout` / `dns` / `refused` / `tls` / `network` |
