...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | /api/machines | 本机，加上**服务端自身** `~/.ssh/config` 中的主机别名、本服务端会安装的版本，以及正在运行或最近一次的安装任务：`{machines: [{id, alias, installed, local, status}], imageVersion, job}` |
| POST | /api/machines/probe | 重探已安装机器的服务端状态（每台一次 ssh 往返，最多并发五台），随后返回同样的响应体 |
...
`machineId` 是该机器**自身**的 id——由运行在那里的服务端铸造的 UUID（`<数据根目录>/machine-id`），跨改名、改别名与重装都保持不变，是所有存储引用应当指向的东西。该机器上的服务端尚未启动过时为 `null`，因为还没有任何东西铸造过它；它与 `status` 在同一次往返中获取，并记录在安装记录旁。同一主机的两个别名会报告相同的 `machineId`。
