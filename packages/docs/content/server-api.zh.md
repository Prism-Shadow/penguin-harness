...
| POST | /steer | 运行中插话：`{text, images?, files?}` 为运行中的 Task 排队一条消息（作为独立的 `[user_steering]` 用户消息随下一轮送达；上传文件复用普通 Task 文件附件的 `{fileName, dataUrl}` 形状、scratchpad 落盘方式和限额）→ 202；任一部分非空即可成消息，全部为空则 400；无 Task 运行返回 409 `not_running` |
