...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | /api/settings/approval-mode | 读取当前登录用户的模式：`{approvalMode}` |
| PUT | /api/settings/approval-mode | 修改当前登录用户的模式：`{approvalMode}` |

每个用户有一个跨 Project、Agent 与 Session 持久化的审批模式。一轮 Task 在启动时绑定发起用户，并在每次工具决策时重新读取该用户的当前值，因此修改会立即作用于该用户正在运行的 Task 的下一次工具调用，而不会影响其他用户；已经处于等待状态的审批请求仍需按原请求执行一次允许或拒绝。排队 Follow-up 保留其提交用户，定时任务使用任务创建者，子 Agent 继承父 Task 的用户。
...
| POST | /agents/:agentId/sessions | 创建 Session：`{approvalMode?, modelId?, provider?, workspace?}` → 201 |
| GET | /dirs?path= | 服务器端目录浏览（Workspace 选择器数据源） |

创建 Session 时，`modelId` 与 `provider` 要么成对给出、要么都不给：给出完整二元组即指定模型，两个都省略则取 Project 默认模型，只给一个返回 400。Workspace 默认自动创建临时目录；Session 不拥有审批设置，每轮 Task 使用其发起用户的当前模式。`approvalMode` 仅为兼容旧客户端而保留；传入时会先更新当前登录用户的设置。
...
| PATCH | / | 更新：`{approvalMode?, archived?, title?}`；其中 `approvalMode` 为兼容旧客户端而保留，修改的是当前登录用户的设置，而非当前 Session |
...
| approval_mode_updated | 当前用户的审批模式已变更；该用户已打开的客户端同步更新拾取器与已加载的 Session 行 |
| hello | 用户通道连接握手 |
