...
- **错误收敛为消息**:LLM 与 Environment 从不向引擎抛异常；结果携带五值 `stop_reason`(`completed | failed | aborted | timeout | malformed`)，仅 LLM 侧的 `timeout / malformed` 触发引擎内重连(至多 8 次、指数退避设上限——`timeout` 涵盖网络超时、传输层断连与瞬时的供应商额度错误)。
