...
某一轮以中断结束（用户停止、LLM 故障）时整个目标随之结束、不再续推——磁盘上的状态保持 `active`，工作区与目标文件就是干净的断点。Web App 中常规停止按钮即中止整个循环；CLI 中是 Ctrl-C。被单 Task 轮次上限（`max_turns`）掐断的轮则不同：`max_turns` 只约束单个 Task、不约束整个目标，因此循环把这次掐断当作轮次边界，用全新的轮次预算开启下一轮。被掐断那轮未提交的工具输出会作为补发保留、在下一轮重放，进度因此得以延续。
