...
Use the normal evidence-grounded mode unless the user explicitly requests black-box adaptation.
In black-box adaptation, precommit a finite set of candidate mechanisms and the selected private
mechanism before testing. Public evidence must make the result under each candidate mechanically
computable, but need not reveal which candidate was selected. Keep the selected mechanism and
Gold answer private, freeze the Rubric and points before evaluation, and reuse each learnable
mechanism across more than one Case so score feedback can support a general hypothesis.
...
When calibration stops after a complete valid matrix, write the final baseline to a temporary sibling, parse it as YAML, then atomically rename it over `scoreboard.yaml`. Include the sorted Case set and sorted runs, a real UTC ISO-8601 time, the tested version and Model pair, and a privacy-safe summary. A complete valid matrix is still a usable baseline when it misses a requested score interval: record it, report the calibration limitation, and stop when the requested refinement budget is exhausted. Leave `evaluations` empty only when no complete valid matrix exists.

Report the Benchmark path, aggregate and Case scores, Test Session ids, refinements, stop reason, and limitations.

Stop after recording and reporting the baseline. Do not modify the Test Agent or begin optimization.
