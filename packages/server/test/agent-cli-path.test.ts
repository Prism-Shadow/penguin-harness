 ...
 * process.env — see packages/server/src/hmr/agent-cli-path.ts's own doc comment for
...
import { ensureCliOnPath, findCliBinDir, prependToPath } from "../src/hmr/agent-cli-path.js";
...
describe("findCliBinDir", () => {
  it("returns null (no-op) when no candidate's launcher file exists", () => {
    const result = findCliBinDir({
      ...
      modulePath:
        "/opt/PenguinHarness/resources/app/node_modules/@prismshadow/penguin-server/dist/hmr/agent-cli-path.js",
    });
  ...
  it("does not mutate the deps object it is given", () => {
    const deps = {
      ...
      bundledShell: undefined,
      modulePath:
        "/opt/app/resources/app/node_modules/@prismshadow/penguin-server/dist/hmr/agent-cli-path.js",
    };
    const snapshot = { ...deps };
    findCliBinDir(deps);
  ...
  it("is not fooled by PENGUIN_BUNDLED_SHELL on non-Windows platforms", () => {
    ...
    const result = findCliBinDir({
      ...
      modulePath: "/some/dev/checkout/dist/hmr/agent-cli-path.js",
    });
...
describe("platform boot extends process.env.PATH (integration)", () => {
  ...
  it("a platform-touching route boots the packaged platform, which extends the real process.env.PATH", async () => {
    ...
    try {
      ...
      // Any request outside /api/hmr goes through the platform's HTTP seam, which
      // calls hmr.ensure() — that is the lazy first boot (see hmr/http-seam.ts).
