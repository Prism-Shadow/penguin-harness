 ...
 * so an extension's workflow and an installed one are the same thing downstream, and an
...
describe("WorkflowRegistry", () => {
  ...
  it("instantiates what extensions registered before it existed", () => {
    factories.set("from-extension", () => ({ run: () => "extension" }));
    const registry = new WorkflowRegistry(factories);
    expect(registry.instanceView().names()).toEqual(["from-extension"]);
    expect(registry.instanceView().run("from-extension", null)).toBe("extension");
  ...
  it("refuses a name another workflow already owns, and lets a reinstall rename", async () => {
    factories.set("counter", () => ({ run: () => "extension" }));
    const registry = new WorkflowRegistry(factories);
    await store.install(ref, SCRIPT);
