...
import type { PenguinContext } from "../src/plugin/index.js";
import { PluginHost, PLUGINS_RESOURCE_ID } from "../src/plugin/index.js";
...
describe("sandbox settings ride the parked context across a swap", () => {
  ...
  async function bootObserved(doc: Json) {
    ...
    host.use({ activate: (pluginCtx) => pluginCtx.on("create", (ctx) => (seen = ctx)) });
