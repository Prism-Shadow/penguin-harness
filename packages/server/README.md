...
- Passwords use `node:crypto` scrypt (`scrypt$N$r$p$salt$hash`, timingSafeEqual). A session is a signed token verified against a key held only in process memory — nothing about it is stored unless it is revoked before expiry (`auth_revocations`); sessions run 30 days with sliding renewal, and a server restart signs everyone out.
