...
export function checkIfaces(
  ...
  for (const d of ts.getPreEmitDiagnostics(program)) {
    ...
    problems.push(
      `${label(q)}: ${
        q.direction === "requires"
          ? "this platform's interface does not fit the version the workflow was written against"
          : "the version the workflow was written against does not fit what this platform asks for"
      }\n    TS${d.code} ${text}`,
    );
  }
  return { problems, uncompared };
