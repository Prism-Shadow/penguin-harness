...
export interface SessionInfo {
  ...
  modelId: string;
  workspace: string;
  /** Current authenticated user's approval mode, repeated on Session rows for client compatibility. */
  approvalMode: ApprovalMode;
...
export interface SessionCreateRequest {
  /**
   * Legacy client compatibility: when present, updates the current authenticated
   * user's setting before the Session is created.
   */
  approvalMode?: ApprovalMode;
...
export interface SessionPatchRequest {
  /** Backward-compatible entry point: changes the current user's mode, not only this Session. */
  approvalMode?: ApprovalMode;
...
export type ServerEvent =
  ...
  /** The current user's approval mode changed; that user's open composers update immediately. */
  | { type: "approval_mode_updated"; approvalMode: ApprovalMode }
  /** Handshake on the user-level event channel. */
  | { type: "hello" }
