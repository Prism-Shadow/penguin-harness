...
export interface MeResponse {
  ...
  /**
   * How this server wants uploads handled. It rides `/api/me` rather than
   * `/api/admin/settings` because every user's composer needs it, not just an admin's.
   */
  uploadPolicy: UploadPolicy;
}

/**
 * The upload policy as the web app sees it. Sizes are whole MB, the same unit the admin form
 * uses, so the number a user sees is the number an admin typed.
 */
...
export type TaskInputPart =
   ...
   * Bounded only by the request body cap. An inline image is written into the Trace and read
   * back on every history page and every resume, so the Web App re-encodes a large one before
   * uploading it (see `uploadPolicy` on `GET /api/me`); an API client sends what it likes and
   * pays that cost as it stands.
   */
  | { type: "image_url"; imageUrl: string }
  /**
   * File attachment (the composer's "+" menu): `dataUrl` is a base64 `data:` URL of the
   * file's bytes. No per-file size limit — the request as a whole has to fit the body cap, and
   * one message may carry at most 20 files (413 `too_many_files`). The server writes it into
   * the Session scratchpad under a sanitized name and appends an `[attached file: <path>]` line
   * to the message text — the bytes never enter the conversation, the model opens the file by
   * path. `fileName` is the original name (no path separators, no `..`).
   */
