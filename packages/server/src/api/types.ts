...
export interface SessionInfo {
   ...
   * reloads and every later context of this Session opens at it, and an Agent-config change
   * no longer moves it. The level is part of a context's request prefix: a pin lands in the
   * next context — a Session that has not run yet starts its first context with it, a
   * running one takes it at its next compaction.
   */
  thinkingLevel?: ThinkingLevelName;
...
export interface SessionPatchRequest {
   ...
   * is a 400). It replaces the Agent-config fallback for this Session's model contexts:
   * a Session that has not run yet opens its first context at it, a running Session's next
   * compaction opens the next context at it — the context in flight keeps its level, since
   * the level is part of its request prefix. There is no unpin: the picker only offers
   * concrete levels.
   */
  thinkingLevel?: ThinkingLevelName;
