...
export class SessionsRepo implements SessionIndex {
  ...
  private runInsert(verb: "INSERT" | "INSERT OR IGNORE", row: SessionRow): void {
    this.db
      .prepare(
        `${verb} INTO sessions (session_id, project_id, agent_id, provider, model_id, workspace, approval_mode, title, client, has_trace, last_active_at, created_at, surface, sandbox)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, COALESCE(?, ?), ?, ?, ?)`,
      )
      .run(
        row.sessionId,
