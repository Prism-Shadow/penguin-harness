 ...
 * genuinely different shapes, so version 1 must tolerate finding its work already done and
 * uses IF NOT EXISTS for that reason alone. Migrations added from version 2 on face a
 * known prior state and should NOT: a bare CREATE TABLE that fails loudly is the point.
 */
