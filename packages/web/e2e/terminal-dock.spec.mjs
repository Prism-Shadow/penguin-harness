...
test("Detach hands the terminal to /terminal?id=… and the dock lets go", async ({ page }) => {
  ...
  await expect(popup.locator('[data-testid="terminal-status"][data-status="ready"]')).toBeVisible({
    timeout: 20000,
  });
  await expect
...
test("tab interactions: reorder by drag, live title, detach keeps the dock", async ({ page }) => {
  ...
  await expect(
    page.locator('[data-testid="terminal-tab"][data-active="true"]'),
  ).not.toHaveAttribute("data-terminal-id", titledId);
  await expect.poll(() => dockScreenText(page), { timeout: 15000 }).toContain("FIRST_TAB_MARK");
});
...
test("tabs are numbered; drag-out splits the tab into a new pane", async ({ page }) => {
  ...
  expect(
    await page.evaluate(
      () =>
        document.querySelector('[data-testid="terminal-dock"][data-position="bottom"] .xterm') ===
        window.__xtermB,
    ),
...
test("terminal clipboard: keyboard copy/paste, right-click, focus", async ({ page }) => {
  ...
  await expect.poll(() => dockScreenText(page), { timeout: 15000 }).toMatch(/^COPY_ME_TOKEN$/m);
...
test("hover menus: create opens on hover; terminal lists on both levels", async ({ page }) => {
  ...
  // Seed one live terminal through the API, with a marker on its screen.
  ...
  await page.request.post(`${BASE}/api/terminals/${seededId}/keys`, {
    data: { keys: "echo HOVER_PICK_MARKER", literal: true },
  });
