...
/**
 * What either transport reduces a response to: the status, the parsed body (null when empty),
 * and the two headers a caller reads — `Date` (the server's clock) and `Retry-After` (how long
 * a refusal asks it to wait).
 */
interface Answer {
  status: number;
  body: unknown;
...
/** `Retry-After` as whole seconds, when it is given in that form. */
...
async function callOverSocket(method: string, url: string, body: unknown): Promise<Answer | null> {
  try {
    ...
    return {
      ...
      retryAfterSeconds: retryAfterOf(res.headers["retry-after"]),
    };
  } catch {
...
async function callOverHttp(method: string, url: string, body: unknown): Promise<Answer> {
  ...
  if (response.status === 204) return { status: 204, body: null, date, retryAfterSeconds };
  const text = await response.text();
  if (!text) return { status: response.status, body: null, date, retryAfterSeconds };
  let parsed: unknown;
  try {
  ...
  return { status: response.status, body: parsed, date, retryAfterSeconds };
}
