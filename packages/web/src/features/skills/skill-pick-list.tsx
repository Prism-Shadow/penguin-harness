 ...
 * carries the same fields (name, descriptions, icon, version), so a plugin is a row too; one that
 * ships no skill has no icon and names the glyph to draw instead of the book (`fallbackIcon`).
 *
        ...
        {skills.length === 0 ? (
          ...
          filtered.map((s) => {
            ...
            return (
              ...
              >
                {/* Each row's custom icon (icon.svg, sanitized and inlined; falls back to the book icon, or the row's own glyph, if missing). */}
                <SkillIcon
                  icon={s.icon}
