...
export function ChatInput({
   ...
   *
   * A large one is re-encoded before it is read (lib/image-compress), not refused: an inline
   * image rides the conversation and the Trace, where its size is paid again on every history
   * page and every resume, and shrinking it keeps that cost down without keeping the picture
   * out. The threshold and the switch are the server's (uploadPolicy); an image under the
   * threshold, or of a type a canvas round trip would change, goes up byte-for-byte.
   *
   * The whole batch is compressed before any of it is staged, so the thumbnails — and therefore
   ...
   * tools, so nothing downstream scales with them, and the only ceiling left is the request
   * body's — which no ordinary pick reaches.
   *
