...
/** Agent-id suggestion from a bundle file name: `<agentId>-export.zip` → `<agentId>`; a bare `penguin-agent.json` suggests nothing. */
 ...
 */
export async function downloadAgentBundle(projectId: string, agentId: string): Promise<void> {
  const res = await fetch(api.agentBundleUrl(projectId, agentId));
  if (!res.ok) {
  ...
  anchor.href = url;
  anchor.download = `${agentId}-export.zip`;
