/**
 * A Project's Benchmarks, read off every machine this server holds.
 *
 * A Benchmark belongs to the Project, and the Project is the same on each machine it is lent
 * to. Its `benchmarks/` directory, though, is written where the Benchmark was created or
 * evaluated — so one Benchmark's cases and scoreboard can be spread over as many disks as it
 * has run on, while the Evaluation Center read only the disk it is served from.
 *
...
/** One Benchmark, as every machine together holds it. */
