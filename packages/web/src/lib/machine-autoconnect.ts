...
export interface AutoConnectApi {
   ...
   * server, so an attempt only counts as connected once the machine itself has answered.
   * Declaring success on the forward's word alone is what made this loop forever: the
   * listener re-ran the reachability probe, the silent machine read as offline again, and
   * the "successful" attempt had already been forgotten.
   */
