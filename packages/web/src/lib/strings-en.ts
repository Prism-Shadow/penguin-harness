  ...
  hooks: {
    ...
    importPromptTail: (projectId: string, agentId: string): string =>
      [
        ...
        'Then produce a PenguinHarness hook package: a hooks.json (name, description, description_zh, version in the vYYYY.MM.DD.N format, and one command list per hook point — stop / pre_tool_use / user_prompt — each entry { "command": "<script path relative to the package>", "timeout": <seconds> }) plus plain Node .mjs scripts using builtin modules only.',
