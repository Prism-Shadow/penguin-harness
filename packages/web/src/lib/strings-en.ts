...
- candidate_round_limit: \`5\``,
  ...
  company: {
    ...
    proposals: {
      title: "Proposals",
      info: "A proposal describes one change — the interfaces it touches, how, why, and one test case — written by an employee and built at the same time, its PR among the materials. Select a passage of the body to comment on it, then send the comments to the author in one batch with Request changes; approving requests the merge. Link one as proposal:<number>.",
      ...
      diff: {
        title: (approved: number, head: number): string =>
          `Changed since approved r${approved} → r${head}`,
        ...
        scopeChanged: (parts: string): string => `Scope: ${parts}`,
        sectionAdded: "added",
      ...
      sectionComments: (n: number): string => `${n} comment${n === 1 ? "" : "s"} on this section`,
      ...
      fromRevision: (n: number): string => `A passage of revision ${n}`,
