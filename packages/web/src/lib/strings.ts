...
- candidate_round_limit：\`5\``,
    ...
    showLess: "收起",
    /** Memory-change card below the file summary and the side panel's Memory view: titles, scope/op tooltips, diff labels, collapsed row. */
    memoryChangesTitle: (n: number) => `${n} 条记忆更新`,
    memoryScopeWorkspace: (key: string) => `工作区记忆（${key}）`,
    memoryOpWrite: "写入",
    memoryOpEdit: "编辑",
    memoryOpRewrite: "重写",
    memoryReplaceAll: "（全部替换）",
    memoryEventNth: (n: number) => `第 ${n} 次`,
    ...
    memoryRewrittenNote: "本会话整篇写入，无会话前版本可对比",
    memoryUnalignedNote: "文件另有本会话之外的改动，无法整体对齐差异",
    memoryPerCallToggle: "逐次调用的差异",
    memoryBodyUnchanged: "正文无变化（本会话仅更新了元信息）",
    memoryRowOpen: "查看差异",
    memoryBack: "返回列表",
