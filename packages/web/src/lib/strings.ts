...
- candidate_round_limit：\`5\``,
  ...
  company: {
    ...
    proposals: {
      title: "提案",
      info: "一份提案是一件改动的说明：拟改的接口、改法、目的与一个测试例，由员工来写并同时实施，PR 挂在关联材料里。选中正文里的一段文字即可评论，再点「请求整改」一次发给作者；认可即请求合并。链接写作 proposal:<编号>。",
      newProposal: "新建提案",
      newTitle: "新建提案",
      author: "作者",
      ...
      diff: {
        title: (approved: number, head: number): string =>
          `自认可的第 ${approved} 修订以来有改动（→ 第 ${head} 修订）`,
        approvedBy: (who: string, when: string): string => `${who} 认可于 ${when}`,
        hint: "认可只覆盖一个修订：作者又发了新修订，需要重新认可。",
        showChanges: "显示改动",
        titleChanged: (before: string, after: string): string => `标题：「${before}」→「${after}」`,
        scopeChanged: (parts: string): string => `范围：${parts}`,
        sectionAdded: "新增",
        sectionRemoved: "已删去",
        addedLabel: "新增",
        removedLabel: "已删去",
      ...
      sectionComments: (n: number): string => `本节 ${n} 条评论`,
      ...
      fromRevision: (n: number): string => `第 ${n} 修订的文字`,
