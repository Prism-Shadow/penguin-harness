...
export const zh = {
  ...
  chat: {
    ...
    exampleTasks: {
      game: {
        label: "示例：企鹅雪橇越野小游戏",
        desc: "可爱南极企鹅滑雪橇跳石头，难度由易到难，纯前端小游戏",
        prompt:
          "做一个可爱的南极企鹅滑雪橇越野小游戏：按空格键起跳，跃过冰面上迎面而来的石头；" +
          "开局简单，滑行速度与难度随时间逐渐上升，实时计分，撞上石头即结束并可一键重新开始。" +
          "纯前端实现（单个 HTML 文件即可），画面走可爱卡通风，界面遵循 web-design 技能。" +
