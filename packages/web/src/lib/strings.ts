...
完成后在浏览器打开 index.html 自测一次。`,
      ...
      rhythmRunner: {
        label: "音乐节奏跑酷小游戏",
        desc: "喵斯快跑式的双轨音游：踩点击打与闪避，判定分级、连击与结算评级",
        prompt: `做一个喵斯快跑（Muse Dash）式的横版音乐节奏跑酷小游戏：角色自动向前跑，音符随音乐从右侧推进，玩家在判定线上踩点击打。纯前端单个 HTML 文件，file:// 直接打开即玩，不依赖任何 CDN 资源。

## 玩法
- 上下两条轨道（空中 / 地面）：上轨按 F 或 ↑，下轨按 J 或 ↓；手机端左右半屏各控一轨。
- 三种物件：普通音符（到判定线时击打）、长按音符（按住到尾端）、障碍（不要打，切到另一轨躲开）。
- 判定分 Perfect / Great / Miss 三档（建议 ±45ms / ±90ms），实时显示连击数、准确率与分数，Miss 断连击。
- 至少两个难度：Easy 只有普通音符且密度低；Hard 加入长按与障碍，密度更高。
- 开局 3 秒倒计时，支持暂停与一键重开。

## 音乐与同步（决定手感的部分，别省）
- 用 Web Audio API 现场合成音轨（振荡器 + 噪声打击乐），不要外挂音频文件——file:// 下 fetch 本地文件会被浏览器拦截，而 CDN 又不允许用。
- 谱面与音乐共用同一份节拍网格：先定 BPM，再按拍位排布音符，音画天然对齐。
- 判定时间一律取 AudioContext.currentTime，不要用 requestAnimationFrame 累加，也不要用 audio.currentTime；requestAnimationFrame 只负责渲染。
- 提供延迟校准：设置里可调 offset（毫秒），并做一个「跟着节拍按键」的校准页测出建议值。
- 准备 2–3 首 BPM 不同的曲目（例如 100 / 128 / 160），在选曲界面切换。

## 表现与结算
- 可爱卡通风、以深色为主，遵循 web-design 技能：命中有粒子与判定文字弹出，连击数放大，背景随节拍轻微律动。
- 结算界面显示 Perfect / Great / Miss 计数、最大连击、准确率与评级（S / A / B / C），可重开或换曲。

## 收尾
- 在浏览器里从 file:// 打开自测：至少完整打完一首，确认连打 30 秒后判定依然准（音画不漂移）、暂停与重开正常、窄屏布局不塌。
- 告诉我怎么打开、按键是什么、延迟校准在哪里调。`,
      },
      investmentCopilot: {
        label: "对话式投资分析助理",
        desc: "用 Penguin SDK 做能看盘的 Copilot：启动即周期刷新大盘与板块，每个判断都给出依据",
        prompt: `用 Penguin SDK 做一个对话式投资分析助理（股市 Copilot）：实时抓取行情、周期性分析大盘，指出哪些板块和走势较强，并且每个结论都带上依据。

## 数据层
- 选一个免费、无需密钥（或密钥容易拿到）的公开行情源，例如 Stooq 的 CSV 接口、Yahoo Finance 的 chart 接口。写代码前先用 curl 打一次，确认接口还活着、字段名到底是什么；打不通就换一个，不要照记忆里的字段结构硬写。
- 封装成统一的数据模块：指数、板块、个股的最新报价与历史 K 线，带本地缓存与失败重试；每次抓取都记录时间戳和来源 URL。
- 前端不直连外部接口，一律走本应用的后端，避免密钥外泄与 CORS 问题。

## 周期性分析（启动就开始跑）
- 应用启动时立刻跑一次全量抓取与分析，随后按固定周期（默认 5 分钟，可配置）持续刷新首页走势——没人提问它也在更新。
- 每轮产出：主要指数的涨跌与量能、板块涨跌排行、领涨与领跌个股、以及基于可计算指标（均线、涨跌幅、成交量变化等）的趋势判断。
- 首页显式展示「最近更新时间」和距下次刷新的倒计时；抓取失败时保留上一份数据并标注「已过期 + 失败原因」，绝不清空页面或显示空白。
- 每轮结果落盘（JSON 或 SQLite 都行），对话与 CLI 读同一份分析，而不是各算各的。

## 对话（Penguin SDK）
- 用 @prismshadow/penguin-core 内嵌一个 Agent：createSession + session.run() 流式输出，Web 聊天界面按 web-design 技能实现。
- 把数据层包装成 tools/ 下的命令行小工具，并写进内嵌 Agent 的 AGENTS.md，让它通过 exec_command 取数，而不是凭记忆报行情。
- 要能回答这类问题：「今天大盘怎么样」「哪些板块强」「600519 现在什么情况」「帮我盯着新能源」。

## CLI
- 提供命令行入口查单只股票，例如 \`npm run stock -- 600519\` / \`node cli.ts AAPL\`：打印最新价、涨跌幅、成交量、当日与区间高低、所属板块表现，以及与 Web 端同一套逻辑给出的趋势判断和依据。
- CLI 直接复用同一个数据层与分析模块，不允许重写一份；支持 \`--json\` 输出，便于接管道。

## 依据与口径（硬性要求）
- 每个结论必须附依据：具体数值、数据时间戳、来源 URL、用到的指标口径。说不清依据的判断就不要输出。
- 数据缺失、延迟或互相矛盾时明说，不要用推测填补。
- 全部输出定位为「基于公开数据的分析」，不是投资建议；界面与 CLI 都要写明这一点，不要出现「买入 / 卖出」这类操作指令式结论。

## 收尾
- 跑起来自测：确认启动后无人操作也刷新过至少两轮、CLI 查一支股票能返回真实数据、聊天里问一个问题能流式回答并带出依据。
- 告诉我怎么启动、CLI 怎么用、刷新周期在哪里改。`,
      },
      rag: {
...
- candidate_round_limit：\`5\``,
      ...
      dailyPlan: {
        label: "工作日早 9 点的计划对话",
        desc: "工作日 09:00 在同一个会话里聊当天计划，并回顾昨天的进展",
        prompt: `建一个定时任务：工作日每天早上 9 点（我的本地时区）和我聊当天的工作计划，并且就在这个会话里进行，这样进展回顾才有上下文。

## 动手之前
- 先确认你的系统提示里有「Scheduled Tasks」小节；没有就停下来告诉我：去 Agent 设置 → 定时任务，把 {{SCHEDULES}} 占位符插进模板后再继续。
- 跑一下 \`date\`，拿到服务器当前的日期、星期和 UTC 偏移——换算首次触发时间这三样都要用。我的时区和服务器不一致的话，问我。

## 任务文件
- 定时任务是 \`<app_data_dir>/agents/<agent_id>/agent_state/schedule/\` 下的 TOML 文件，一个任务一个文件（目录不存在就创建）。\`period\` 只接受固定间隔（\`30m\` / \`12h\` / \`7d\`），没有「仅工作日」这种写法，所以工作日触发要拆成五个周任务，而不是一个日任务。
- 写 \`daily-plan-mon.toml\` 到 \`daily-plan-fri.toml\` 五个文件，每个都是 \`enabled = true\`、\`period = "7d"\`，\`start_at\` 设成下一个对应星期几的当地 09:00，写成带时区偏移的 ISO 8601 时刻。周末因此永远不会触发。
- 五个文件的 \`session_id\` 都填当前这个会话的 Session ID（在你的 Environment 小节里），让每天早上都落回这里，而不是每天新开一个对话。
- 如果我这边有夏令时，顺带提醒我：固定 7d 周期在换季后会偏一小时，那时需要改一次 \`start_at\`。

## 每天说什么
\`prompt\` 字段就是每天早上送达的那条消息。让它：先回看本会话上文，说清昨天定的计划完成了多少、哪些顺延、卡在哪；再问我今天要做什么，并主动给出排好序的候选清单，每条附一句理由，而不是丢一个空问题；我确认后把今天的计划写成清单。控制篇幅——这是晨会，不是日报。

## 收尾
- 定时任务只在 Web 服务运行时触发；新文件服务器约 30 秒内自动读到，不需要注册。
- 把五个文件的名字、首次触发时间和绑定的 Session 列给我，并告诉我 Agent 设置 → 定时任务 里可以查看、修改或停用它们。`,
      },
      githubDigest: {
        label: "每天汇总 GitHub 项目状态",
        desc: "定时跑一遍仓库的 Issue、PR 与 CI，日报结尾给出按优先级排序的建议",
        prompt: `建一个定时任务：每天汇总一个 GitHub 项目的状态，结尾给出按优先级排序的建议。

## 动手之前
- 先问我仓库（owner/repo）和汇总时间；我没意见就默认每天当地时间 09:30。
- 确认你的系统提示里有「Scheduled Tasks」小节；没有就先告诉我去 Agent 设置 → 定时任务 插入 {{SCHEDULES}} 占位符。
- 确认 \`gh auth status\` 正常且 \`gh\` 能访问该仓库；不行就告诉我怎么登录，然后停在这里。

## 任务文件
- 在 \`<app_data_dir>/agents/<agent_id>/agent_state/schedule/\` 下写 \`github-digest.toml\`（目录不存在就创建）：\`enabled = true\`、\`period = "24h"\`、\`start_at\` 为下一个约定时刻，写成带时区偏移的 ISO 8601 时刻。
- 不要写 \`session_id\`：每次触发新开一个 Session，让每天是一份独立日报，而不是一个越滚越长的会话。我本地有仓库检出的话，用 \`workspace\` 指向那个目录。

## 日报内容
触发时是全新会话、没有上下文，所以 \`prompt\` 字段要把活儿交代完整。让它用 gh CLI 抓：过去 24 小时新开、关闭与合并的 Issue 和 PR；待评审的 PR 及其已等待时长；主分支上失败的 CI 运行；里程碑与临近的截止；长期无人处理的 Issue。然后输出：一段状态概览、变化清单、风险项。结尾必须是「优先级建议」小节，按 P0 / P1 / P2 分级，每条写清做什么、为什么、对应哪个 Issue 或 PR（带链接）。gh 取不到的数据要明说，不许编。

## 收尾
- 交付前你自己先把那段 prompt 正文跑一遍，确认里面每条 gh 命令都能跑通，并把输出给我看。
- 告诉我文件名、下一次触发时间，以及在 Agent 设置 → 定时任务 里可以修改或停用。`,
      },
      memoryReview: {
        label: "每周五晚回顾并记录 Memory",
        desc: "周五傍晚一起过一遍这周值得长期记住的事，按 Memory 格式写进记忆目录",
        prompt: `建一个定时任务：每周五傍晚和我一起过一遍这周有什么值得长期记住的，然后由你记进 Memory。

## 动手之前
- 先确认你的系统提示里同时有「Memory」和「Scheduled Tasks」两个小节；缺哪个就停下来告诉我：去 Agent 设置 → 记忆 / 定时任务，把 {{MEMORY}} / {{SCHEDULES}} 占位符插进模板。
- 跑一下 \`date\` 拿到服务器当前的日期、星期和 UTC 偏移，再问我周五几点合适；我没意见就默认当地 18:00。

## 任务文件
- 在 \`<app_data_dir>/agents/<agent_id>/agent_state/schedule/\` 下写 \`weekly-memory-review.toml\`（目录不存在就创建）：\`enabled = true\`、\`period = "7d"\`、\`start_at\` 为下一个周五的约定时刻，写成带时区偏移的 ISO 8601 时刻。
- \`session_id\` 填当前这个会话的 Session ID（见你的 Environment 小节）：这个回顾需要我回话，所以要落在我本来就在的对话里，而不是每周新开一个。

## 回顾怎么做
让 \`prompt\` 字段安排你：先读 user memory 目录的 \`MEMORY.md\` 索引，列出已经记过的条目，避免重复；再用具体问题问我这一周——新的偏好和协作方式、做过的决定与背后的理由、踩过的坑与规避办法、下个月依然成立的目标与约束、常用资源放在哪，而不是只问一句「有什么要记的吗」。我确认一条，你就按 Memory 小节规定的格式写一条：一件事一个文件，frontmatter 写 \`name\` / \`description\` / \`updated_at\`，正文带 **Why:** 与 **How to apply:**，相关条目用 \`[[name]]\` 互链；同一轮里同步更新该目录的 \`MEMORY.md\` 索引，新增、合并、删除都要反映。这周的事推翻了已有记忆的，改写或删掉那条，而不是再叠一条。永远不要记：代码、配置或 git 历史里已经写着的事，任务进度，密钥，没确认的猜测，聊天记录原文。最后把本次新增、修改、删除的条目列给我。

## 收尾
- 告诉我文件名、首次触发时间、绑定的 Session，以及在 Agent 设置 → 定时任务 里可以修改或停用。`,
      },
    },
    sessionList: "Session",
