...
三个 Case 应共同考察从多个公开证据中提出可能的规律、用额外证据区分它们、处理规则优先级和例外，并在应用前验证所选规律的能力。难度应来自选择和验证正确规律，而不是直接套用一张完整规则表。每个 Statement 必须提供完成任务所需的全部信息。Rubric 可以包含 Gold 和评分方法，但不得使用无法从公开材料推导或学习的任意隐藏映射。

如果使用潜在规律，Statement 必须提供足够的公开历史样本，使该规律原则上可以被发现。难度应来自多步分析、噪声信息、跨材料关联、例外条件和验证过程，而不是缺失关键信息。Pilot 分数过高时，先根据 Test Trace 判断 Agent 使用了什么捷径，或哪一步推理没有被真正要求，再调整 Case。增加样本、字段或干扰信息，只有在迫使 Agent 执行新的推理或验证步骤时才算有效；单纯增加工作量不算提高难度。不得仅通过隐藏更多信息或改成任意规则来降低分数。
