...
export const zh = {
  ...
  models: {
    ...
    addGroupTitle: "新增分组",
    addGroupDesc:
      "自建分组与 Custom 同语义：接口协议可手动选择，也可按 base URL 检测。填写 Base URL 后可一键导入该端点的全部模型。分组由模型条目承载，保存首个模型后即出现。",
    groupAddManually: "手动添加",
    groupImportAction: "检测并导入",
    groupBaseUrlHint: "填写后可检测协议并导入端点全部模型",
    groupImportKeyHint: "留空按协议读取 OPENAI_* / ANTHROPIC_* 环境变量",
    groupImportDetecting: "正在检测协议…",
