...
- candidate_round_limit：\`5\``,
      ...
      dailyPlan: {
        ...
        prompt: `建一个定时任务：每天早上 9 点（我的本地时区）和我聊当天的工作计划，并且就在这个会话里进行，这样进展回顾才有上下文。

## 动手之前
- 先确认你的系统提示里有「Scheduled Tasks」小节；没有就停下来告诉我：去 Agent 设置 → 定时任务，把 {{SCHEDULES}} 占位符插进模板后再继续。
- 跑一下 \`date\`，拿到服务器当前的日期与 UTC 偏移——换算首次触发时间要用。我的时区和服务器不一致的话，问我。

## 任务文件
- 定时任务是 \`<app_data_dir>/agents/<agent_id>/agent_state/schedule/\` 下的 TOML 文件，一个任务一个文件（目录不存在就创建）。
- 写一个 \`daily-plan.toml\`：\`enabled = true\`、\`period = "24h"\`，\`start_at\` 设成下一个当地 09:00，写成带时区偏移的 ISO 8601 时刻。
- \`session_id\` 填当前这个会话的 Session ID（在你的 Environment 小节里），让每天早上都落回这里，而不是每天新开一个对话。
- 如果我这边有夏令时，顺带提醒我：固定 24h 周期在换季后会偏一小时，那时需要改一次 \`start_at\`。
...
- 把文件名、首次触发时间和绑定的 Session 告诉我，并说明 Agent 设置 → 定时任务 里可以查看、修改或停用它。`,
      },
      githubDigest: {
