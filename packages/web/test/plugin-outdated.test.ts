 ...
 *   behind on it (`AgentSummary.pluginUpdates`), since versions are `vYYYY.MM.DD.N` strings the
...
const skill = (name: string) => ({ name, description: "", version: "v2026.08.01.1" });
...
describe("pluginInstalled", () => {
  it("counts a plugin as installed once any part of it is there, so a partial copy can be updated", () => {
    const whole = installs(
      { plan: "v2026.08.01.1", run: "v2026.08.01.1" },
      { orchestration: "v2026.08.01.1" },
    );
    ...
    expect(
      pluginInstalled(
        FULL,
        installs({ plan: "v2026.08.01.1" }, { orchestration: "v2026.08.01.1" }),
      ),
    ).toBe(true);
    expect(
      pluginInstalled(FULL, installs({ plan: "v2026.08.01.1", run: "v2026.08.01.1" }, {})),
    ).toBe(true);
    expect(pluginInstalled(FULL, installs({ other: "v2026.08.01.1" }, {}))).toBe(false);
  });

  it("reads a skill-only plugin off the skills list and a hook-only one off the hooks list", () => {
    expect(pluginInstalled(SKILL_ONLY, installs({ "web-design": "v2026.07.30.1" }, {}))).toBe(true);
    expect(pluginInstalled(SKILL_ONLY, installs({}, { "web-design": "v2026.07.30.1" }))).toBe(
      false,
    );
    expect(pluginInstalled(HOOK_ONLY, installs({}, { goal: "v2026.08.29.1" }))).toBe(true);
    expect(pluginInstalled(HOOK_ONLY, installs({ goal: "v2026.08.29.1" }, {}))).toBe(false);
  });
...
describe("installedPluginVersion", () => {
  it("reads the hook package's version where there is one, else the first skill's", () => {
    expect(
      installedPluginVersion(
        FULL,
        installs(
          { plan: "v2026.07.01.1", run: "v2026.07.01.1" },
          { orchestration: "v2026.07.02.1" },
        ),
      ),
    ).toBe("v2026.07.02.1");
    expect(
      installedPluginVersion(SKILL_ONLY, installs({ "web-design": "v2026.07.30.1" }, {})),
    ).toBe("v2026.07.30.1");
  });
  ...
  it("is undefined where the plugin is not installed, and reads a partial copy's first installed skill", () => {
    ...
    expect(
      installedPluginVersion(SKILL_ONLY, installs({ other: "v2026.07.01.1" }, {})),
    ).toBeUndefined();
    expect(
      installedPluginVersion(
        { name: "pair", skills: [skill("plan"), skill("run")], hooks: [] },
        installs({ run: "v2026.07.01.1" }, {}),
      ),
    ).toBe("v2026.07.01.1");
  });
});
...
describe("outdatedAgentIds", () => {
  it("names the Agents the server lists as behind on that plugin, in list order", () => {
    const agents = [
      agent("stale", { name: "web-design", version: "v2026.08.01.1" }),
      agent("current"),
      agent("other", { name: "vllm", version: "v2026.08.01.1" }),
      agent(
        "also_stale",
        { name: "web-design", version: "v2026.08.01.1" },
        { name: "vllm", version: "v2026.08.01.1" },
      ),
    ];
...
describe("pluginUpdatePlan", () => {
  ...
  it("sends one request per Agent, carrying every plugin that Agent is behind on", () => {
    ...
    expect(
      pluginUpdatePlan([
        agent(
          "alpha",
          { name: "web-design", version: "v2026.08.01.3" },
          { name: "vllm", version: "v2026.08.01.2" },
        ),
        agent("beta"),
        agent("gamma", { name: "web-design", version: "v2026.08.01.3" }),
      ]),
    ).toEqual({
      perAgent: [
  ...
  it("counts distinct plugins, matching what the notice above the button says", () => {
    ...
    const plan = pluginUpdatePlan([
      agent("alpha", { name: "shared", version: "v2026.08.01.2" }),
      agent("beta", { name: "shared", version: "v2026.08.01.2" }),
      agent("gamma", { name: "shared", version: "v2026.08.01.2" }),
    ]);
