/**
 * benchmark-merge.ts unit tests: a Project's Benchmarks read off several machines and folded
 * into one list. A Benchmark is one identity, so the merge is by benchmark id — the machine is
...
describe("mergeBenchmarks", () => {
  ...
  it("names every Agent either scoreboard tested, in first-seen order", () => {
    const merged = mergeBenchmarks([
      ...
      { machineId: MACHINE, benchmarks: [benchmark("example", { agentIds: ["b", "c"] })] },
    ]);
    expect(merged[0]!.agentIds).toEqual(["a", "b", "c"]);
  });
  ...
  it("never mutates the answers it was given", () => {
    const mine = benchmark("example", {
      evaluations: [evaluation("2026-09-01T00:00:00Z")],
      agentIds: ["a"],
    });
    mergeBenchmarks([
      { machineId: null, benchmarks: [mine] },
      {
        ...
        benchmarks: [
          benchmark("example", {
            evaluations: [evaluation("2026-09-02T00:00:00Z")],
            agentIds: ["b"],
          }),
        ],
      },
    ...
    expect(mine.agentIds).toEqual(["a"]);
