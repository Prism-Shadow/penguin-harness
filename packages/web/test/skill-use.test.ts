...
describe("parseSkillsMessage legacy form (old Traces re-rendered)", () => {
  it("still recognizes the old angle-bracket <use_skills> block", () => {
    expect(
  ...
  it("build always emits the current square-bracket form", () => {
    expect(buildSkillsMessage(["solo"], "x").startsWith("[use_skills]\n")).toBe(true);
  });
});
