...
import { bulkOutcome, firstFailure, noticeCounts } from "../src/lib/bulk-update";
...
describe("the notice block is the one shape on every page that has one", () => {
  ...
  it("always carries a way down: the dismiss control is not optional", () => {
    for (const { file, attrs } of sites) {
      expect(attrs.has("dismissLabel"), `${file} must label its dismiss control`).toBe(true);
      expect(attrs.has("onDismiss"), `${file} must handle dismissal`).toBe(true);
    }
  });
...
describe("the bulk button confirms before it writes", () => {
  ...
  it("renders a ConfirmModal on every page that offers the action", () => {
    for (const { file, source } of withAction) {
      expect(source.includes("<ConfirmModal"), `${file} must confirm the batch`).toBe(true);
    }
  });
});
