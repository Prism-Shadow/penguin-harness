...
const updatedAt = (size: number): string =>
  en.company.handbook.updatedAt(formatDateTime(UPDATED), formatBytes(size));
...
describe("HandbookExplorer rows", () => {
  ...
  it("keeps the index's reason for being pinned out of the row and in its tooltip and name", () => {
    const [index] = rows();
    const expected = `${HANDBOOK_INDEX} · ${en.company.handbook.indexLabel} · ${updatedAt(512)}`;
    ...
    expect(visible(index!)).not.toContain(en.company.handbook.indexLabel);
  });

  it("leaves the index label out of every visible row", () => {
    expect(rows().map(visible).join("")).not.toContain(en.company.handbook.indexLabel);
  });
  ...
  it("gives a document row its path and write time, and no accessible name of its own", () => {
    ...
    expect(visible(row!)).toBe(`conventions.md${formatRelativeShort(UPDATED, "en")}`);
  });

  it("gives a folder row its path and how many documents it holds", () => {
    const row = rows().find((r) => attr(r, "aria-expanded") !== null);
    expect(attr(row!, "title")).toBe(`roles · ${en.company.handbook.documentsInFolder(2)}`);
    expect(attr(row!, "aria-label")).toBeNull();
