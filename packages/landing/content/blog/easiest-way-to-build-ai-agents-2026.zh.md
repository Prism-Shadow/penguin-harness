...
所以，Agent 还没在生产环境里做成任何有用的事，团队里就已经有人学完了两个库，接好了追踪 SDK，自建或订阅了一个观测平台，还手工攒了一套评估数据集。等 Agent 表现不好，去读 trace、调 Prompt 的还是这个人，因为这套技术栈里没有任何一环会替你做这件事。
...
同一份指南还提醒，框架往往多加一层抽象，把底层的 Prompt 和响应挡在后面，让 Agent 更难调试。
...
这个品类没有死，它只是承认了一件事：真正有价值的从来不是那些抽象。这也让上文那笔组装成本更难自圆其说。
...
安装后就有多 Session 对话、Agent 与 Skill 管理、模型配置、用量与成本统计、Trace 可观测，以及**评估中心**，彼此已经打通。不用订阅什么，也不用另外自建什么。每个请求、每次工具调用、每个审批决策都已经记录下来，Session 可以从 Trace 完整恢复。这里没有追踪 SDK 要接，因为产品之间根本不存在需要埋点的接缝。
...
这件事能成立，是因为 **Agent 是可编辑的数据，不是写死的常量**：Prompt、Skill 和配置都是磁盘上的普通文件。

### 调优循环也自动化了

这正好回应了 LangChain 那个例子的最后一步：由人去读 trace、调 Prompt。内置的 Benchmark 设计、评估与优化 Skill，让 Agent 自己给输出打分，找出丢分的地方，然后发布第 N+1 版。每一轮开始前都会留快照，每个请求都能在 Trace 视图里回放。

在别的技术栈上，*你*就是那个优化器：你读 trace，你调 Prompt，你重跑评估。在这里，这个循环归 Agent 管，你不必把自己练成这方面的行家。
...
- **团队写 Python。** PenguinHarness 的 SDK 是 TypeScript。CLI 和服务端与语言无关，但如果你的团队用 Python，还想继承、扩展这些类，上面的多数方案会更顺手。
...
- **文档**：[快速开始](https://penguin.ooo/docs/quickstart) · [Skills](https://penguin.ooo/docs/skills) · [Session 与 Trace](https://penguin.ooo/docs/sessions-and-traces)
