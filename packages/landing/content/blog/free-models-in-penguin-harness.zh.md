...
![模型库页面的 OpenRouter 分组：Ling 3.0 Flash (free)、Nemotron 3 Ultra (free)、Laguna M.1 (free) 和 Free Models Router 都带淡黄色的「免费」标签](/blog-assets/free-models-page-zh-light.webp)
...
- **仅支持文本。** 路由本身接受图片，但某次请求背后的模型未必支持。这一条特意标记为仅支持文本，所以 PenguinHarness 不会在这条路由上发送图片，而是改用平常的纯文本处理方式。

## 开启免费模型

1. 在 [openrouter.ai](https://openrouter.ai/) 创建 API key。免费档不需要绑定支付方式。
2. 打开**模型库**页面：
   - 新建的 Project 已经带有这些预置条目。在 OpenRouter 分组上点击**手动设置密钥**，粘贴一次 API key，整个分组都会生效。
...
可以把某个免费条目设为 Project 的默认模型；也可以不动默认模型，开始会话时在模型选择器里选一个免费条目。模型按会话选择，不绑定 Agent。免费条目在**模型库**页面和模型选择器里都带淡黄色的**免费**标签，很容易认出来。
...
- **数据政策。** 免费模型按 OpenRouter 的免费模型条款运行，上游供应商可能在自家条款允许的范围内使用你的 Prompt。不愿公开的内容不要发送。
...
然后打开**模型库**页面，填入 OpenRouter API key，选一个免费条目。
