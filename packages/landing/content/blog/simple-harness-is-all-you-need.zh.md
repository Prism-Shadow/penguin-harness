...
**Agent Harness** 是包在语言模型外面的那层软件，决定每次请求里放什么：系统 Prompt、工具定义和消息历史。按常见的直觉，Harness 越大，Agent 越强。工具更多、上下文更全、脚手架更厚，决策理应更好，整个品类就是建立在这个信念上的。

本文的观点正好相反。Databricks 用真实 Pull Request 做了一次评测，最简单的 Harness 拿到最高分，成本还只有一半左右；Databricks 把原因归结为它每轮发送的上下文更少。我们把 Harness 看作一份**上下文预算**，而不是一张功能清单。下文会说明这份预算花在了哪里，介绍 PenguinHarness 怎样围绕同一个判断构建，并指出精简必须在哪里止步。
...
精简的 Harness 不只是在价格上守住了阵地。在榜首，它赢了。
...
> 「Pi 每轮发送的上下文大约只有三分之一。它对上下文管理得更好，工作集更紧凑，完成任务所需的运行次数也更少。」

方法上还有一个细节，这么做的人比应有的少：Databricks 拒绝用模型来判分，理由是那样会「奖励听起来对，而不是真的对」。

## Harness 是一份上下文预算

Harness 只做一种决策：每一轮，模型的上下文窗口里放什么。

模型是 API 背后的一个函数：接收系统 Prompt、工具定义和消息历史，返回文本和工具调用。这份契约是固定的，也是公开的。除此之外的一切，也就是所有决定请求里放什么的软件，都是 Harness。
...
上下文膨胀从来不是一个糟糕的决定造成的，而是四个各自说得通的决定，在每一轮里不断叠加。
...
环境快照、状态块、反复重发的配置文件，都钉在每条消息上。单个很小，结构上却是永久的。

这些都不是错的想法，问题在于没人给它们标价。

## PenguinHarness 是怎么做的

这个赌注我们在那份评测出现之前就下了，它写在源码里，不在宣传页上。下面的数字描述的是本文发布时的 PenguinHarness，每一个在当时都能在仓库里核对。

### 六个工具，没有文件工具

PenguinHarness 内置[六个工具](https://penguin.ooo/docs/tools)，任一 Session 实际只看到五个，因为两个图像工具按模型类别互斥。
...
没有 read 工具、write 工具、edit 工具，也没有 glob 和 grep。读、写、编辑、搜索全部走 shell，因为 shell 本来就能做这些事，模型也本来就会用。一个四工具的极简内核要把三个名额（read、write 和 edit）花在文件系统上，PenguinHarness 只花一个。我们不宣称工具的绝对数量最少，Pi 的内核比我们还少一个。我们宣称的是，在 Agent 每天真正在做的事上，我们的 schema 面积最小。

### 72 行的系统 Prompt

默认模板在替换占位符之前是 72 行、约 6,600 字符（[源码](https://github.com/Prism-Shadow/penguin-harness/blob/main/packages/core/src/state/default-config.ts)）。内容包括角色、成功标准、约束、停止规则、文件系统布局，外加一小段建议工作流，然后就结束了。
...
系统里没有 Skill 工具。Prompt 只带每个 Skill 的名字和一行描述，需要时 Agent 再用一条普通的 shell 命令读取正文。用不到的 Skill，只占一行。
...
「上下文越多，决策越好」这个直觉并不离谱，只是在边际上不成立，原因有三个。
...
我们自己的评测成绩落在那个位置，也是同一个道理。这组评测里，每个 Harness 都搭配它平时常用的模型：PenguinHarness 配 DeepSeek V4 Pro，Claude Code 配 Claude Opus 4.8，Codex 配 GPT-5.5。
...
PenguinHarness 把安全和可观测性当作承重结构。它们在上下文里几乎不占开销，因为它们在运行时里实现，而不是写在 Prompt 里：
...
- 工具永远不向引擎抛异常。失败会变成工具输出，模型读到后据此应对。这也是精简的 Prompt 依然安全的原因：环境把自己的错误讲得足够清楚，Prompt 就不必替它预判。
...
- 工具输出的硬上限是多少？
- 你的系统 Prompt 里，有多少行在教模型预训练时就学会的事？
- 每条消息里都注入了什么？
...
- **读实现**：[工具与审批](https://penguin.ooo/docs/tools) · [Agent 运行循环](https://penguin.ooo/docs/agent-loop) · [Skills](https://penguin.ooo/docs/skills)
