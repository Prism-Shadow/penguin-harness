---
title: "PenguinHarness 0.2.12：公司模式、重建的评估中心、可管理的文件浏览、托盘图标"
date: 2026-09-15
category: news
excerpt: 一个 Project 可以变成一家公司。公司模式——内测功能，缺省关闭，需管理员开启——把 Project 里的 Agent 组织成公司：一位 CEO、一棵汇报树、一张日历、一块五列工单板和若干频道，每一样都是磁盘上的文件。此外，Benchmark 从 Agent 的附属品变成与 Agent 平级的对象，文件浏览长出右键菜单并去掉了体积上限，运行中的工具调用可以转入后台，桌面端关掉窗口后继续在托盘里干活。三个会让任务中途终止的网关问题也一并修掉。
---

PenguinHarness 0.2.12 是「Project 可以变成组织」的那一版：CEO 提议、人拍板，整家公司以文件的形式存在 Project 下。其余的改动是围着它的工作台——评估中心按它服务的那条闭环重建，文件浏览变成真的能管理 Workspace 的面板，桌面端进了系统托盘，以及三个曾经让 Task 停在原地的网关问题。
