...
const en: Messages = {
  ...
  compactionStop: (mode, status, tokens, cause) =>
    (status === "completed"
      ? mode === "discard"
        ? "[compaction] done; old context discarded"
        : "[compaction] done; continuing with the summarized context"
      : `[compaction] ${status}${cause !== undefined ? ` (${cause})` : ""}; keeping the current context`) +
  ...
  compactionStop: (mode, status, tokens, cause) =>
    (status === "completed"
      ? mode === "discard"
        ? "[压缩] 完成，旧上下文已丢弃"
        : "[压缩] 完成，已切换到摘要后的新上下文"
      : `[压缩] ${status === "aborted" ? "已中断" : `失败${cause !== undefined ? `（${cause}）` : ""}`}，保留当前上下文`) +
    (tokens ? ` · tokens ${tokens.total} (${tokens.delta})` : ""),
