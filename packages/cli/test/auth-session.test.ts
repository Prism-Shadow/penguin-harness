...
describe("the session file", () => {
  ...
  it("defaults the server to the one this data root is running", () => {
    expect(localServerUrl(root)).toBeNull();
    fs.writeFileSync(path.join(root, "server.lock"), JSON.stringify({ pid: 1, port: 7364 }));
    expect(localServerUrl(root)).toBe("http://localhost:7364");
  });
});
...
describe("call", () => {
  ...
  it("rejects a target that is not a URL rather than guessing one", async () => {
    await expect(call("not a url", { method: "GET", path: "/" })).rejects.toThrow();
  });
});
