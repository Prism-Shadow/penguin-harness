...
const toolTasks = [
  ...
  },
  {
    ...
    prompt: "请使用工具检查数据库中用户表的字段结构，不要自行猜测。",
...
async function runEvaluation() {
  ...
  if (process.argv.includes("--check")) {
    ...
    const chineseCatalogCase = "postgres-describe-table-zh";
    const chineseCatalogRuns = results.filter(
      (result) => result.case === chineseCatalogCase && result.search_calls > 0,
    );
    // The live fixture keeps this target's catalog contract English-only. A successful lazy
    // run must therefore finish with an English catalog query; a Chinese first attempt followed
    // by the one permitted catalog-language retry is also valid.
    const chineseTranslationPasses =
      !tasks.some((task) => task.id === chineseCatalogCase) ||
      (!evaluationModes.includes("lazy") && chineseCatalogRuns.length === 0) ||
      (chineseCatalogRuns.length > 0 &&
        chineseCatalogRuns.every((result) => {
          const finalQuery = result.search_queries.at(-1) ?? "";
          return (
            result.success && /[a-z]/iu.test(finalQuery) && !/\p{Script=Han}/u.test(finalQuery)
          );
        }));
    ...
    const passes = commonPasses && comparisonPasses && scenarioPasses && chineseTranslationPasses;
