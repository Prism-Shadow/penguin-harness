...
async function readNativeAssets() {
  ...
  // The compiler workflows are checked with: the target's program may predate it being a
  // dependency (or be a desktop bundle with no node_modules), and a platform that cannot
  // find it loads no workflow. Content-addressed like everything here, so it crosses once.
