...
**Changing it, and taking it away.** Everything about a workflow is files in your own Agent directory, so every change — including removing it — is a file operation you make yourself; list `workflows/` first to see what exists. The server notices within a moment and the user's tabs follow without a refresh.

- *Remove one tab:* delete its entry under `WebModule.sessionTabs` in `package.json` (and the page under `ui/` if nothing else uses it).
- *Keep the server code, drop all UI:* leave `"contributes": {}`.
- *Remove the workflow entirely — back to the chat with no custom UI:* delete the folder `workflows/<id>/` **and** its recorded versions `workflows-history/<id>/`. That is everything there is: the tabs disappear, nothing else of the Agent is touched. The user can do the same from the tab's bar (*Remove*, two clicks). Say plainly that this cannot be undone before you do it, and never delete a workflow you were not asked to remove.
- *Go back to an earlier version of the code:* the tab's bar has *History* with *Restore* per version; from your side, the versions are full copies under `workflows-history/<id>/<revision>/` (`versions.json` lists them, newest first) — copy that version's files over the folder and the server reloads it. `state.json` is never part of a version: restoring code leaves the data as it is.
- *The data is only ever changed through `host.setState`.* Do not edit or delete `state.json` by hand to "reset" something: there is no history of it, so what you overwrite is gone. If the user wants a reset, give the page a button that does it through the handler — and if it matters, keep what was there under another key so it can be brought back.
