...
/**
 * The plugin's one module: one entry per language on the LanguagesModule.grammars slot,
 * each grammar bound by its contribution id (the manifest is generated into ifaces.json
 * from here — the entries below are the static half, CONTRIBUTIONS the code half, and the
 * test asserts the two agree). Created per App, so a hot swap gets the grammars of the
 * plugins the push actually carried.
 */
@Component({
  contributes: {
    "LanguagesModule.grammars": [
      ...
      { id: "languages.dart", language: "dart", displayName: "Dart", extensions: ["dart"] },
    ],
  },
})
export class Languages {
  @Bind("languages.typst") typst!: LanguageGrammar;
  @Bind("languages.swift") swift!: LanguageGrammar;
  @Bind("languages.kotlin") kotlin!: LanguageGrammar;
  @Bind("languages.csharp") csharp!: LanguageGrammar;
  @Bind("languages.dart") dart!: LanguageGrammar;

  setup() {
    const bound = new Map(languageContributions().map((c) => [c.data.language, c.grammar]));
    const grammar = (language: string): LanguageGrammar => {
      const g = bound.get(language);
      if (g === undefined) throw new Error(`no grammar built for ${language}`);
      return g;
    };
    this.typst = grammar("typst");
    this.swift = grammar("swift");
    this.kotlin = grammar("kotlin");
    this.csharp = grammar("csharp");
    this.dart = grammar("dart");
  }
}

const plugin: Plugin = { modules: [Languages] };
