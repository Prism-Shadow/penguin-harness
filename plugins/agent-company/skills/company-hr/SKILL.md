...
3. `--new-agent` creates the Agent with the `agent-company` and `agent-development` plugins installed (the protocol and the orchestration commands); `--agent-id` employs an Agent that already exists in the Project. Ids match `^[a-z][a-z0-9_]{1,63}$`, prefixed `<org_id>_`. A hire runs on the Project's default model — `hire` takes none and you set none; a particular model on a role is a board decision the CEO carries (`company-ceo`).
...
3. **The model** — propose `penguin org employee set <id> --model-id <id> --provider <p>` to the CEO for a role the current model underserves (or, with finance, a cheaper one for a role it overserves). Every hire runs on the Project's default model, and a particular one is a cost decision the board confirms, so the command runs only after the CEO brings back the yes.
