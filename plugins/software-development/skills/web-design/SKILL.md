...
- **Card** — `border: 1px solid var(--border); border-radius: 12px; background: var(--surface); padding: 24px;` hover changes **only the border color** (one step darker) — no lift, shadow or scale. Cards in one grid share the same padding and stretch to equal height.
...
- **States** — loading: three pulsing dots in `--fg-faint` for the pending answer, skeletons for regions that load; error: 13px `#b91c1c` text on `#fef2f2` (dark: `#f87171` on `#450a0a`) in a rounded box with a retry affordance. Never leave a silent failure.
