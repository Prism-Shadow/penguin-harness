...
class World {
  ...
  managerDeps(): ManagerDeps {
    return {
      ...this.deps(),
      pluginConfig: {
        get: (name: string) => (name === CONFIG_GROUP ? this.config : {}),
...
describe("the manifest, the config table and the helpers", () => {
  it("contributes the routes its generated manifest declares", () => {
    ...
    const table = JSON.parse(readFileSync(path.join(PLUGIN_DIR, "ifaces.json"), "utf8")) as {
      modules: Record<
        string,
        {
          ...
          contributes: Record<
            string,
            Array<{ id: string; properties?: Record<string, { type: string }> }>
          >;
        }
      >;
      plugin: { modules: string[] };
    };
    const manifest = table.modules.DiscordBotPlugin;
    expect(manifest?.name).toBe("DiscordBotPlugin");
    ...
    // Its settings group is a contribution beside the route, so the table carries it too.
    const group = manifest?.contributes["PluginConfigProvider.groups"]?.[0];
    ...
    expect(group?.properties?.bot_token?.type).toBe("secret");
    expect(group?.properties?.project?.type).toBe("string");
    expect(table.plugin.modules).toEqual(["DiscordBotPlugin"]);
    expect(plugin.modules).toEqual([DiscordBotPlugin]);
  });
