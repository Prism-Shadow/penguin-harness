...
/**
 * The plugin's one module: the `discord` entry on the chat-bot host's slot, bound to the
 * seed read off the environment (its manifest is generated into ifaces.json from here).
 */
@Component({
  contributes: {
    "ChatBotsModule.bots": [
      { id: "discord", channel: "discord", label: "Discord", labelZh: "Discord" },
    ],
  },
  context: { version: 1 },
})
export class DiscordBot {
  @Bind("discord") bot!: ChatBotBinding;

  setup() {
    this.bot = { defaults: envDefaults() };
  }
}

const plugin: Plugin = { modules: [DiscordBot] };
