...
`agent-company-proposals` is the skill half of the company-proposals plugin pair (the code half, `@prismshadow/penguin-plugin-company-proposals`, is a module plugin a Project installs; see [PRFC-0016](https://github.com/Prism-Shadow/penguin-harness-design/pull/210)): `proposal-author` writes and revises a proposal — a short, abstract description of a change a person comments on passage by passage — and opens its build at once; `proposal-implementer` builds it (the author's own session, or a colleague's) on a `proposal/<n>-<slug>` branch, opens the PR against `dev`, feeds back where the code disagrees with the text and merges into `main` on approval; `proposal-tester` runs `dev` in batches and files a fix ticket for a merged proposal or a runtime feedback for one still open. Three protocols, not three positions: the plugin installs the package on whoever writes or builds a proposal, never by default.
